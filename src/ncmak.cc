...
comp *addcomp_extern(comp *pnt, comp *pnte);
comp *set_cachan_extern(comp *pnt, comp *pnte);
void coneset(int n);
void rodiset(int n);
void chr2set(int n);
...

{
   ...
   switch (src->ctype) {
	  ...
	  chdest->k2      = chsrc->k2;
	  chdest->n       = chsrc->n;
	  chdest->vfbg    = chsrc->vfbg;
	  chdest->vfbo    = chsrc->vfbo;
          break;
...

{
 ...
 if ((stpnt=(char *)emalloc(rndsiz)) == (char *)NULL) {
   ncfprintf (stderr,"makrand: No space left for %s rand state %d.\n", mesg,num);
 }
 cumrand++;
 return stpnt;
}
...

{
  ...
  if ((nrseed=napnt->rseed) >= 0) {
    ...
    if (spnt->vstate) ncinitstate (nrseed,spnt->vstate,RNDSIZ);
  ...
  if (spnt->vcov > 0) {
    if ((nrseed=napnt->rseed) >= 0) {
      ...
      if (spnt->gstate) ncinitstate (nrseed+11,spnt->gstate,RNDSIZ);
    }
  }
  spnt->vtint = gamdev(1);
  restorstate();
...
{
    int sf1,sf1h,sf2,sf3,sfm,nt,stop,secmsg;
    synap *spnt;
    attrib *apnt;
    chattrib *ntpnt;
    nattrib *napnt;
    double st, sn, sg, so, nkd, npow, ckd, chc, sc, rp, msgc, mx, re, trc, eg, vg, rrp, mrrp, ms, rrpg;
    double sf1hg, sf1ho, tf2, tf3, tfm, vsize, casens, vfbg, vfbo;
    synapse *oepnt;
    comp *comp3;
    void makexp(void);
    ...
    if ((casens=epnt->casens)==NULLVAL) casens = 0; /* presyn sensitivity */
    ...
    if (!epnt->timec1) epnt->timec1=makfiltarr(sf1, 0, (double*)NULL, dfta);
    if (epnt->timec1)
    ...
    if (!epnt->timec3) epnt->timec3=makfiltarr(sf3, 0, (double*)NULL, 1.0);
    if (epnt->timec3)
      if (epnt->timec3[0]==0) epnt->timec3[0]=dftc; /* tau of filters (msec) */
    if ((tf3=epnt->tfall3)==NULLVAL) tf3 = 0.0;	 /* falling tau of filt (msec)*/

    /* set filter for 2ndmsg (mglur2, gaba-b) */

    if ((sfm=(int)(epnt->nfiltm))==NULLVAL)sfm=0;/* number of filters, def=0, set in set2ndmsg() */
    if (!epnt->timecm) epnt->timecm=makfiltarr(sfm, 0, (double*)NULL, dftm);
    if (epnt->timecm)
      if (epnt->timecm[0]==0) epnt->timecm[0]=dftm; /* tau of filters (msec) */
    if ((tfm=epnt->tfallm)==NULLVAL) tfm = 0.0;	 /* falling tau of filt (msec)*/
	...
	further below.
      */
         ...
         case NUNIT:  unitsyn = (nattrib *)apnt;	/* remember N, unit */
			      break;
						/* default:        */ 
         case CCHNOISE: if (((nattrib*)apnt)->n!=0){ /* stop if nchan = 0 */
	 ...
	 case VFB:
	     comp3 = makcomp(epnt, 1e-14, 1e-10, NULLVAL, NULLVAL);	/* make new compartment */
	     spnt->compe = comp3;					/* allow synapse to set comp v */
	     spnt->maxcond = 1e-15;					/* remove synaptic cond */
             addcomp_extern(comp2, comp3);				/* connect new comp as ext to postsyn comp */
             set_cachan_extern(comp2, comp3);
	     chapnt= (chattrib *)apnt;
	     if ((vfbg=chapnt->vfbg)==NULLVAL) vfbg = dvfbg;
	     spnt->vfbg = vfbg;						/* voltage feedback gain */
	     if ((vfbo=chapnt->vfbo)==NULLVAL) vfbo = dvfbo;
	     spnt->vfbo = vfbo;						/* voltage feedback offset */
	     stop = 1;
          break;
    ...
    for (apnt=epnt->attpnt; apnt; apnt=apnt->attpnt) {/* set up ves noise parms*/
      ...
      spnt->vgain = vg;
      spnt->casens = casens;
      if (eg==0) eg = 1;		// allow caegain row to be zero in old nval.n files
      spnt->caegain = eg;
      spnt->ngain = sn;
      spnt->cgain = sg;
      spnt->coff = so;
    ...
    if (tf2) tf2 = 1.0 - calctau(tf2,stiminc/MSEC);	// optional falling time constant
    if (tf3) tf3 = 1.0 - calctau(tf3,stiminc/MSEC);
    if (tfm) tfm = 1.0 - calctau(tfm,stiminc/MSEC);
    ...
    spnt->filtm  = maksynfilt (LP,sfm, 0,    tfm,  0, epnt->timecm, stiminc/MSEC);
    spnt->secmsggain = 0;	/* gain for plot of synaptic mglur2 mod of Ca chans */
    spnt->secmsgthresh = 0;	/* thresh for plot of synaptic mglur2 mod of Ca chans */

/* ncfprintf (stderr,"sf1 %d sf1h %d ft1 %g sf2 %d ft2 %g sf3 %d ft3 %g sfm %g ftm %g\n",
				sf1,sf1h,ft1,sf2,ft2,sf3,tf3,sfm,tfm); /* */
...

{
 ...
 if (!makestim) {
   ...
   if (sepnt->casens!=NULLVAL)epnt->casens= spnt->casens= sepnt->casens;  /* presyn sensitivity */
   ...
   if ((sfi=(sepnt->nfilt3)!=NULLVAL)) {	/* modify nfilt in third filter */
	...
	if (napnt->vsize!=NULLVAL)
	    spnt->vsize = napnt->vsize;	
        if (napnt->rseed > 0) ncinitstate (napnt->rseed,spnt->vstate,RNDSIZ);
	break;
...

{
  ...
  rpnt->stimchan = epnt->stimchan;	/* can set to give private stimulus channel */
  rpnt->chanw[0] = 1.0;
  rpnt->iflux = 0.0;
  ...
  if (dnois>0) {
     ...
     if ((nrseed=epnt->dkrseed) >= 0) {		/* use default random if neg */
	  ...
	  nrseed = noduniq(epnt->node1a, epnt->node1b,
        ...
        if(rpnt->dstate)
            ncinitstate(((nrseed&IMASK)+1818213)&IMASK,rpnt->dstate,RNDSIZ);
     }
  }
  ...
  if (pnois) {
     ...
     if ((nrseed=epnt->phrseed) >= 0) {		/* use default random if neg */
	  ...
	  nrseed = noduniq(epnt->node1a, epnt->node1b,
        ...
        if(rpnt->pstate)
            ncinitstate(((nrseed&IMASK)+1098667)&IMASK,rpnt->pstate,RNDSIZ);
#undef IMASK
     }
  }
  rpnt->attf = attf;
...

{
 ...
 if (maxcond <= 0 && nozerochan) {
  ...
  chpnt->gfractab = NULL;
  chpnt->gvrevtab = NULL;
  chpnt->secmsgmod  = NULL;
  chpnt->secmsgtype = 0;
  chpnt->secmsggain = 1.0;
  ...
  for (stop=0,npnt=apnt->attpnt; npnt && !stop; npnt=npnt->attpnt) { 
	...
	if (nrseed >= 0) {
	  ...
	  if (chpnt->cstate) ncinitstate (nrseed,chpnt->cstate,RNDSIZ);
	}
...

{
 ...
 if (debug & NCMAK && debugz & 16)
  ...
  for (npnt=apnt->attpnt; npnt; npnt=npnt->attpnt) { /* set noise params */
	...
	if (napnt->rseed > 0) {
	   if (chpnt->cstate) ncinitstate (napnt->rseed,chpnt->cstate,RNDSIZ);
        }
	break;
...
1. presynaptic cell depolarizes  (horizontal cell)

2. external current stops flowing through hemi-channels into presynaptic cell

3. external compartment goes more positive -- towards zero

4. voltage seen by ion channel voltage sensor in postsynaptic cell looks more hyperpolarized  -- negative feedback

5. driving foce for non-selective channel conductance in postsynaptic cell increases      -- positive feedback (but small)

6. internal voltage driven by cell capacitance depolarizes                                    -- positive feedback (but small, due to small capacitance in synaptic cleft compared to whole cell capacitance)
...
/* Connect a compartment's channels to point to an external compartment */

{
    ...
    for (lpnt=pnt->clst; lpnt; lpnt=lpnt->next) {
        conpnt = lpnt->conpnt; 
	switch (conpnt->ctype) {
	   case CA: ((chan*)conpnt)->compe = pnte; 
		break;
	}
    }
    return pnt;
...
double vext(conn *chpnt, double atten) 

/* Return a channel's external comp voltage for ephaptic effects. */
/*   "atten" is set between 0 -> 1.0 to simulate effect of local vext, */
/*   e.g. in the cone/rod synaptic invagination, that doesn't affect the */
/*   entire cell. */
...
/*	ncfprintf 
    ...
    //       r = sqrt (area / (4.0*MPI)); /* radius in dm (decimeters) */
    // }
    // else if (epnt->ctype==CABLE) {
    //	   double len;
    //   if ((dia = ((cable *)epnt)->dia)!=NULLVAL) {
    //        if (apnt->cadia!=NULLVAL && apnt->cadia > 0) {
    //        dia *= apnt->cadia;	/* cadia is mult diameter factor */
    //        }
    //  }
    //  else if (apnt->cadia!=NULLVAL && apnt->cadia > 0) {
    //	     dia = apnt->cadia;		/* cadia is diameter (um) */
    //    }
    //    r = dia * 0.5 * DMUM;		/* radius in dm (decimeters) */
    //    len = dcacomplen * DMUM;	/* length of cacomp, 2.5 um */
    //    area = (2.0 * MPI * r*len);	/* shell surface (dm2) */
    //  }
    else if (epnt->ctype==CABLE) {
           area = carea * DMCM2;
           r = sqrt (area / (4.0*MPI)); /* radius in dm (decimeters) */
           if ((dia=apnt->cadia)!=NULLVAL && apnt->cadia > 0) {
	       r *= dia; 
	       area = 4.0*MPI*r*r;
	   }
    }
      ...
      if (carea>0) {			/* in this case area comes in as cm2 */
          ...
          r = CACOMPRAD * DMUM;		/* (calib in dm) make it 5 um diameter */
          area = (4.0 * MPI * r*r);	/* shell surface (dm2) */
       }
    }
    ...
    // mincoresiz = 3;
    mincoresiz = 1;
    dr = dcashd*DMUM;			/* thickness of shell in dm (1e-6) */
