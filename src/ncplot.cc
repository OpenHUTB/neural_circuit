...
int plotcolors[PLOTNODSIZ] = {0};	/* list of plot colors from comnd line */
...
static char plotlabeltext[200]={0};	/* label text */
...

{
   ...
   double *arr;
   int n,nn,pen,plpos,plposn,aplot,setpen;
   int ch;
   char *plframe;
   lpfilt *filt;
   Symbol *varpen;
...
#define LABELAX    .02
 ...
 setpen=lookcolor(plpos);		 /* plot color set on command line*/
 ...
 if (plsep) {
	...
	if (setpen>=0) pen = setpen;
	if (pen==NULLVAL) pen = colortab[plposn & 0xf];
        //if (pen > 0) cpen (pen);
        cpen (pen);
 }
	...
	pen = plotnod[n].ppen;
	if (setpen>=0) pen = setpen;
        if (pen == NULLVAL) pen = colortab[n & 0xf];
        // if (pen > 0) cpen (pen);
        cpen (pen);
/*      if (dashfl) dash ((long)(pen & 7)); /* */
 ...
 if (pen < 0 || pen==NOCOLOR) return;  		/* do nothing for negative pen */
 if (pen==NOCOLOR) return;
   ...
   switch (plotnod[plposn].charmode) {
    ...
    case   LINES:				/* plot chars with lines */
       if (!notinit(plotnod[nn].oldx)) {
         _move (plotnod[nn].oldx,plotnod[nn].oldy);  /* plot lines */
         if ((plotnod[nn].oldx > x) && !allowrev) _move (x,y);
         else                                     _draw (x,y);
         _move (plotnod[nn].oldx,plotnod[nn].oldy);  /* prev point */
    ...
    case   PLINES:				/* plot just lines */
       if (!notinit(plotnod[nn].oldx)) {
          /* if (!dashfl) _move (plotnod[nn].oldx,plotnod[nn].oldy);*/
          _move (plotnod[nn].oldx,plotnod[nn].oldy);
       if ((plotnod[nn].oldx > x) && !allowrev) _move (x,y);
       else                                     _draw (x,y);
       }
       else _move (x,y);			/* move to first point */
       break;
...
{
    int i,n, pmod2;
    int nnodpos,charfl,charmode,plotn,ppen,setpen,useframe; 
    const char *modech,*plframe;
    char fstr[40];
    static char modey[10];
...
if (!vidmode) {                                /* init non-graphics display */
   ...
   if (infile && srcfile) { 
    ...
    for (i=0; i<totplots; i++) {
      ...
      setpen=lookcolor(n);             /* plot color set on command line*/
	 ...
	 case NRECGI:
         ...
         case FREC:     
			...
			break;
	 case CREC_GLU:  case CREC_AMPA: case CREC_KAINATE: case CREC_NMDA: case CREC_ACH: 
	 case CREC_CNQX: case CREC_GABA: case CREC_BIC:  case CREC_PTX:  case CREC_GLY: case CREC_CHRC:
	 ...
	 case NRECC9: case NRECD0: case NRECD1: modech = "F"; break; 
         case NRECA8:
...
#undef NUMBUFSIZ
  ...
  switch (plotnod[n].pmod) {
	...
	case NRECH0: case NRECH1: case NRECH2: case NRECH3: case NRECH4:
    	case NRECB0: case NRECB1: case NRECB2: case NRECB3: case NRECB4:
    	case NRECC0: case NRECC1: case NRECC2: case NRECC3: case NRECC4:
	case NRECC9: case NRECD0: case NRECD1:
...
#define issmall(c)        ((c)=='-' || (c)=='-' || (c)=='i' || (c)=='I')
   ...
   if (pllabel!=NULL) {
     ...
     if (lablx < 0) lablx = 0;
     if (disp_ray) {
       rprintf(pllabel,lablx*dispsize*0.5,lably*dispsize*0.5,1,charsiz*dispsize,0,((bknd_color==BLACK)? WHITE: BLACK));
     } else {
       gcwidth (charsiz);
       gpen (WHITE);
       gmove (lablx, lably);
...

{
    ...
    int plmaxlen, plminlen, pllenmax, setpen;
 ...
 _move (xplotmargl,  ybase);                 /* draw x axis */
 _draw (1-xplotmargr,ybase);
...
/* if (hitics) calibtic = ticinc * 10.0;
 ...
 for (itici=itic,j=0; j<ntics; j++,itici++) {     /* draw x axis tics */
   _move (plticmin, ybase);
   if ((itici % lablint) == 0) 
        _draw (plticmin, ybase - LARGETIC);    /* always draw larger tics */
   else 
   if (hitics) _draw (plticmin, ybase - SMALLTIC); /* smaller tics */

   if ((itici % lablint) == 0){
     if (nolabltic && (ncabs(itici%10) == 5)) ;
     else {
        cpen (WHITE);
        pxnum = ((int)(axmin/ticinc * 1.000001) + j) * ticinc;
        if (ncabs(pxnum) < (range / 100)) pxnum = 0;
        if (calibms) pxnum *= 1000;
	_move(plticmin,ybase-XPLOTLABEL-charsiz*1.2); /* position of label */
 ...
 if (hitics && ntics < 61) {
    if ((itic % lablint) < lablint/2 && (itic%lablint) > 0) {
      _move (xplotmargl,ybase-XPLOTLABEL-charsiz*1.2);
 ...
 if (hitics && ntics < 61) { 
    if ((--itici % lablint) > lablint/2) {
 	_move (1-xplotmargr, ybase-XPLOTLABEL-charsiz*1.2); 
 ...
 if (!finfl) {
  if (plotnod[n].pmod != GREC) {
   _move ((1+xplotmargl-xplotmargr)*0.5, ybase-XPLOTLABEL-charsiz*3);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
 ...
 for (m=i=0; m<numyaxis; i++,m++) {        /* draw (maybe multiple) y axis */
   ...
   if (useframe) n = looklist(nplot);
      ...
      setpen=lookcolor(i);		 /* plot color set on command line*/
   }
   else if (i>=numlaxis) {
...
/* ncfprintf (stderr,"i %-2d n %-2d m %-2d aplot %-2d yax %-6.3g yb %-6.3g plotn %-2d pen %-2d size %g\n",
   ...
   if (axpen >= 0 && axpen != NOCOLOR) {
    _move (yaxisX, ybase);			/* draw y axis */
    _draw (yaxisX, ybase+yaxis);
   }
   ...
   if (axpen < 0 || axpen==NOCOLOR) continue;
   ...
   for (itici=itic,j=0; j<ntics; j++,itici++) {   /* draw y axis tics */
     _move (yaxisX, plticmin);
     if ((itici % lablint) == 0)
          _draw (yaxisX-LARGETIC*ticdir, plticmin); /* always draw larger tics */
     else
      if (hitics) _draw (yaxisX-SMALLTIC*ticdir, plticmin);  /* smaller tics */
     ...
     if ((itici % lablint) == 0) {
	   ...
	   if (ticypos==ticymax) ticymaxfl = 1;       /*label y tics*/
           _move (yaxisX-(LABELTIC+ycharsiz*LABELOFFS)*ticdir,ticypos);
   ...
   if (!ticyminfl) {
     ...
     if (hitics && ntics < 61) {
        if ((itic % lablint) < lablint/2 && (itic%lablint) > 0) {
     	  _move (yaxisX-(LABELTIC+ycharsiz*LABELOFFS)*ticdir, ticymin);
     ...
     if (!nonodes && !plsep) {
       _move (yaxisX-(LABELAX+ycharsiz*LABELOFFS)*ticdir,ticymin+0.035);
   ...
   if (!ticymaxfl) {
     ...
     if (hitics && ntics < 61) {
        if ((--itici % lablint) > lablint/2) {
	  _move (yaxisX-(LABELTIC+ycharsiz*LABELOFFS)*ticdir, ticymax);
    ...
    if (!finfl) { 
     if (nonodes) { 
       /*_move (yaxisX-LABELAX*ticdir,ticymax+YLABELPOSN); /* At top of Y axis,*/
       _move (yaxisX,ticymax+YLABELPOSN); /* At top of Y axis, */
       if (ticdir>0) prtext ("r",ycharsiz,calstr); /* draw y axis calib label */
       else          prtext ("l",ycharsiz,calstr);
     }
     else {
       //_move (yaxisX-(+LABELAX*ticdir),ticymax-YLABELPOS); 
       _move (yaxisX-(pllenmax*ycharsiz*ticdir),ticymax); 
       if (ticdir>0) prtext ("r",ycharsiz,calstr); /* draw y axis calib label */
       else          prtext ("l",ycharsiz,calstr);
     }
    }
 ...
 if (plsep)
  for (m=i=0; m<totplots; i++,m++) {		/* draw plot labels */
   ...
   if (plotnod[n].plname[0]) {			/* plot name label */
      ...
      plotlabl[aplot] = 1;
						/* display plot name */
      _move (XABSMARGL, ybase+yaxis/2+(nnplots[aplot]*.5-nplots[aplot]+.5) * 
    ...
    _move(XABSMARGL, ybase+yaxis/2+(nnplots[aplot]/2-nplots[aplot]+1) *
  ...
  for (i=0; i<totplots; i++) {                   /* draw node nums */
    ...
    pen = plotnod[n].ppen;
    if (setpen>=0) pen = setpen;
    if (pen == NULLVAL) pen = colortab[n & 0xf]; /* default pen is plot num */
    cpen (pen);                                  /* change pen for node num */
    ybase = yplotmarg;
    _move (XABSMARGL,ybase+yaxis/2+(totplots/2-i)*charsiz*1.8); /* disp node num*/
  if (plotnod[n].plname[0]) {
...
/* lookup plot in command line list and 
    return color of possibly different plot number.
*/

{
   ...
   if (i<0) i = 0;
   if (listplots[0]) {
	retval=plotcolors[i+1];	 		/* possibly different order */
  	// if (retval<0) retval = 0;		/*   from command line */
   }
  return retval;
}
...
#define INBUFSIZ 10000
   ...
   int xpos,lchar,charmode,graphmem,setpen;
     ...
     if (!setauto)
      switch (inbuf[1])  {
       ...
       case 'd':
                ...
                if (nplot>=PLOTNODSIZ) nplot = PLOTNODSIZ-1;
                setpen=lookcolor(nplot);		 /* plot color set on command line*/
                if (setpen>=0) pen = setpen;
                if (pen == NULLVAL) pen = colortab[nplot & 0xf];
		plotpen(pen,nplot);
                break;
       case 'l':
...
/*		   infile = infilnam;     /* ignore "infile" here, */
		   ...
		   srcfile = srcfilnam;
                   ...
                   narg = sscanf(inbuf,"%*s plotlabel %190[ a-zA-Z0-9@#$%^&*()+_=|{};:<>,.?/-]\n",plotlabeltext);
		   // strncpy(plotlabeltext,&inbuf[13],190);
		   plotlabel = plotlabeltext;
		}
                break;
	...
	"#y pl %d siz %lg pen %d %*s %c %*s '%c' %*s %lg %*s %lg %*s %lg\n",
		...
		*/
		...
		plotnod[npl].plotsiz = plotsiz;
                setpen=lookcolor(npl);		 /* plot color set on command line*/
                if (setpen>=0) pen = setpen;
...
#define GRAPHMEMA (PLOTNODSIZ*100000)
 ...
 if (cols>0 && lines>1) {
   ...
   for (j=0; j<totplots; j++) {		/* do one column at at time */
    ...
    plotnod[pl].ppen = colortab[pl & 0xf];
