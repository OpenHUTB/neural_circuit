...
void initsim (void)
{
  ...
  if (debug & NOCOND) nocond = 1;
	...
	fpout = stdout;
      ...
      if (prmap & (PCOND|PCOMP)) { prcomp(); exit(1); }
    }
...

{
  ...
  if (debug & NCRECORD) ncfprintf (stderr,"record node %d %d %d %d mode %d %d ",
   ...
   case WREC: if (!(npnt=findnode(cnod1,cnod2,cnod3,cnod4,
	      ...
	      if ((cpnt=npnt->comptr)) {
	         if (cpnt->compe!=NULL)
			 rval = cpnt->v - cpnt->compe->v; // Vm = comp voltage - vext
		 else
   ...
   case CREC_CNQX: case CREC_GABA: case CREC_BIC:  case CREC_PTX:  case CREC_GLY:
...
//	         }
   ...
   case NRECC9: case NRECD0: case NRECD1: case NRECCA: 
...
/* ncfprintf (stderr,"f0 %d snum %d nf %d\n",spnt->filt1->lfilt[nf],snum,nf); */
    ...
    if (spnt->casens==0) {			/* voltage sensitivity controls release */ 
	...
	transrate = fltvolts;
   						...
   						/* calcium sensitivity controls release */ 
      if (capnt=spnt->comp1->capnt) {
	      double caval;
	caval = capnt->cais[0];		 /* take calcium in first shell */
        transrate = exp(log(caval*dscavg*spnt->casens)*spnt->caegain) * spnt->vgain; /* scale by vgain */
	//fprintf (stderr,"ca-trans %g\n",transrate);
      ...
      transrate /= (vsize * 10.0 * HMICSEC);		/* vesicles released per stiminc step */
...
/* ncfprintf (stderr,"f0 %d snum %d nf %d\n",spnt->filt3->lfilt[nf],snum,nf); */
   ...
   /* Problem here is that 2nd mesg is not saved in synapse structure.
  ...
  } else if (nf==NRECD0) {		/* return mGluR2 modulation */
        double transrate, mod;
	if (spnt->filtm) {
        ...
        return transrate;
  } else if (nf==NRECD1) {		/* return mGluR2 inhibition of channel activation */
        double transrate, mod;
	if (spnt->filtm) {
            nf        = spnt->filtm->nfilt-1;
	    transrate = spnt->filtm->lfilt[nf];  /* neurotrans released, from synaptic filter */
	...
	transrate *= spnt->secmsggain;			/* scale by mglur gain */
...

{
  ...
  if (spnt->casens==0) {
   ...
   if (oldsynapz) transrate = getcurv(spnt,fltvolts);         /* static tranfer func */
	...
	caval = capnt->cais[0];		 /* take calcium in first shell */
	transrate = exp(log(caval*dscavg*spnt->casens)*spnt->caegain) * spnt->vgain; /* scale by vgain */
	// fprintf (stderr,"caval %g dscavg %g casens %g transrate %g\n",caval,dscavg,spnt->casens,transrate);
      }
      else transrate = 0;
  } /* if (spnt->casens) */
    ...
    for (i=0; i<stime; i++) {                   /* one or more steps */
      ...
      if (nsites=spnt->vsites && spnt->vsize) {  /* if noisy release */
        vsize=spnt->vsize * 10.0 * HMICSEC / stiminc;   /* ves rate*timeinc */
							/* need vsize to change inversely with stiminc */
							/*  to make up for larger vesicles with larger stiminc */
...
/* ncfprintf(stderr,"vesrel %g transrate %g p %g size %g nsites %d\n",
           ...
           vsize=  dvsz * 10.0;   /* ves rate*timeinc */
           meanves = transrate / vsize; 
	   vesrel = meanves;
...
{
   int i, npre;
   double chanligand, chanligand2, transrate, nt;
   double synfilt(lpfilt *lpnt, double transrate);
   synap *spnt;
   dyadlst *lpnt;
   gj *gjpnt;
 ...
 for (spnt=synpnt; spnt; spnt= (synap *)spnt->next)
   {
     switch (spnt->ctype)
      {
	   ...
	   for (lpnt=spnt->spre; lpnt; lpnt=lpnt->next) {
             if (lpnt->sdyad) {
               transrate += lpnt->sdyad->transrate * lpnt->sdyad->trconc; /* trans from other presyn */
	       npre++;
	     }
           }
	   transrate /= npre;
	 ...
	 if (stime) {
	    ...
	    if (spnt->filtm)
	      nt = synfilt(spnt->filtm,transrate); /* mglu lowp filt*/
           }
	 }
	 else chanligand = transrate;

	 if (spnt->compe != NULL) {
		 spnt->compe->v = (chanligand * spnt->vfbg) - spnt->vfbo;
         	 // fprintf (stderr,"chanligand %g compe->v %g %g %g\n",chanligand,spnt->compe->v, spnt->vfbg,spnt->vfbo);
	 }
	 else rchan(spnt,stime,chanligand);		/* run the channels */
...
/*  AMPA, ACH, KAINATE, CGMP, GABA, GLY, SYN2, and NMDA channels are run as state machines 
 ...
 rspnt->recnm1,rspnt->recnm2,rspnt->ctype,rspnt->val,rspnt->wavel,rspnt->time,rtime,stop);/**/
   ...
   if ((rspnt->time >= rtime) && (rspnt->time <stop) && (bgend==0 || bgend==rspnt->bgend)) {
     ...
     switch (rspnt->ctype) {
       ...
       case 'a':                /* absolute light intensity */
