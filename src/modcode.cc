...
void makgaborx(double speriod, double sphase, double orient, double xloc, double yloc,
		...
		double xenv, double yenv, int makenv, int ssq, int tsq, double mask, int stimchan,
...
void initray (double xrot, double yrot, double zrot,
        double xcent, double ycent, double zcent,
        double rxcent, double rycent, double rzcent, double scal, int bknd_color);
void set_icons(void);
...
//		break;
    ...
    case TCAI: 	  if (apnt) apnt->tcai = d1.val;
		break;
    case CADIA:  if (apnt) apnt->cadia = d1.val;
		break;
  ...
  switch (param->type)
   {
    ...
    case TIMEC3: 
    case TIMECM: npops=0;
		nfilts = (long long int)*pc++; /* # of tau's actually entered */
  ...
  switch (param->type)
   {
    ...
    case CLOSE: epnt->ntact = param->type; 
		epnt->secmsg = 1;
		break;
    case CA:  	epnt->casens  = d1.val;
		break;
    case V:  	epnt->casens  = 0;
		break;
    ...
    case NFILTM:  epnt->nfiltm = (short int) d1.val; xfilt = 1; break;
    ...
    case TFALL3: epnt->tfall3 = d1.val;
		break;
    case TIMECM:  if (!xfilt) epnt->nfiltm = nfilts;
		  epnt->timecm=makfiltarr(epnt->nfiltm,0,epnt->timecm,0);
	          if (xfilt) {	/* if nfilts was given separately */
		     for (i=0; i<epnt->nfiltm; i++) 
                       epnt->timecm[i]= ttimec[0];
		  }
		  else {	/* if nfilts was given by num of tau's */
		     for (i=0; i<nfilts; i++) 
                       epnt->timecm[i]= ttimec[i];
		  }
		  xfilt = 0;
		break;
    case TFALLM: epnt->tfallm = d1.val;
		break;
...
void conn1l(void)
{
  ...
  narg = (long long int)*pc++;		/* get loc off stack first */
  if (narg > 2) d3 = popm();
...

{
  ...
  switch (param->type) {
    ...
    case SIZE:  narg = (long long int) *pc++;
                if (narg > 2) d3 = popm();
                else d3.val = 0;
                if (narg > 1) d2 = popm();
                else d2.val = 0;
    ...
    case CENTER: narg = (long long int) *pc++;
                if (narg > 2) d3 = popm();
                else d3.val = 0;
                if (narg > 1) d2 = popm();
                else d2.val = 0;
  ...
  switch (param->type)
   {
    ...
    case CHECKERBOARD:
		...
		// yn = d4.val;
 		...
 		checknum(d1 = popm());
		stimdia = d1.val;
		if (d2.val<0) stimydist = stimdia;
		else          stimydist = d2.val;
		stype = param->type; 
		break;
    case RECT:  narg = (long long int)*pc++;
  		if (narg > 1) d2 = popm();
		else d2.val = 1.0;
 		checknum(d1 = popm());
		stimdia = d1.val;
    ...
    */
                stype = param->type; 
                break;
    case LOC:   narg  = (long long int)*pc++;
                if (narg > 0) {
                  if (narg > 1) {
    ...
    case SQ:    checknum(d1 = popm());
                sq = d1.val;
                break;
    case WAVEL: spec = (long long int)*pc++;
		switch (spec) {
		  case 0: 
 		    checknum(d1 = popm());
		    wavel = d1.val;
	...
	if (!arraymade) {
	   ...
	   stimarrsize = (long long int)(max(xrange,yrange) / sscale);
...
*/              break;
        ...
        case CHECKERBOARD:
		if (!setyenv) yenv = xenv;
	    	makcheckerboard(stimarr,stimarrsize, stimdia, stimydist, xenv, yenv, 
...
#undef BARLENGTH
		...
		makgabor(stimarr, stimarrsize, contrast, inten, 
			xloc, yloc, xcent, ycent,
                	xenv, yenv, makenv=0, sq, sq, tfreq, drift, speriod, sphase,
		...
		makgabor(stimarr, stimarrsize, contrast, inten, 
			xloc, yloc, xcent, ycent,
                	xenv, yenv, makenv=1, sq, sq, tfreq, drift, speriod, sphase,
                	sscale, orient, wavel, mask, start, dur);
	  	break;
  ...
  switch (param->type)
   {
	...
	case CCLAMP:
	   ...
	   switch (puffmsg) {
		...
		break;
		case CA:
		 makvstim(start,    rnum1,rnum2,rnum3,rnum4, puffconc,0,"v");
		 makvstim(start+dur,rnum1,rnum2,rnum3,rnum4,-puffconc,1,"v");
		break;
	    }
           break;
	...
	case SINE:			/* stim for possibly all receptors */
	  ...
	  makgaborx(speriod, sphase, orient, xloc, yloc, tfreq, drift,
                inten, contrast, wavel, xenv, yenv, makenv=0, sq, sq,
	...
	case GABOR:			/* stim for possibly all receptors */
	    makgaborx(speriod, sphase, orient, xloc, yloc, tfreq, drift,
                inten, contrast, wavel, xenv, yenv, makenv=1, sq, sq,
		mask, stimchan, action, start, dur);
	  break;
	...
	case CHECKERBOARD:		/* stim for possibly all receptors */
	    if (!setyenv) yenv = xenv;
	    makcheckerboardx(dia, ydist, xenv, yenv, orient, xloc, yloc, 
...
/* Sort the stimulus file after all stimulus events 
   ...
   This routine allows stimuli to be defined out of sequence.
...

{
   ...
   static char stemp[30], *tbuf;
   static char stimaddcmd[] = " -A";  // i.e. run either "nc -A", "nci -A", or "retsim -A"
   static char sortcmd[] = 
      "sort -g -k 11 -k 1,2 -k 2,3 -k 3,4 -k 4,5 -k 5,6 -k 6gr,7 -k 7,8 -k 8,9 -k 9,10 %s | %s %s > %s && mv %s %s";
  ...
  if (makestim) {  
    ...
    s3 = strlen(stimfile);
    s4 = strlen(progname);
    s5 = strlen(stimaddcmd);
    tbuf = emalloc((s1 + 2 * (s2 + s3) + s4 + s5 + 10) * sizeof(char));
    sprintf (tbuf,sortcmd,stimfile,progname,stimaddcmd,stemp,stemp,stimfile);
    system (tbuf);
  }
}
...
void printstim (double time, int node1, int node2, int node3, int node4, double inten, double wavel,
		                        double mask, int stimchan, char action, int seq);

/*    stimadd: add up stimuli at each time to shorten stimfile.  */
...
printf ("# stimfile sorted, added.\n");
  ...
  for (row=0; fgets(inbuf,INBUFSIZ,stdin); ) {
      if (inbuf[0]=='#') ncfprintf (stimout,"%s",inbuf); 
      else {
	  ...
	  if (				// if same time, node, action, etc
	       t != ot  || 
              ...
              se != ose )  
	   {   				// print row, load next
		if (row++ > 0) printstim (ot,on1,on2,on3,on4,oval,ow,om,ost,oa,ose);
		ot  = t; 
		on1 = n1; 
		on2 = n2;
		on3 = n3;
  ...
  printstim (ot,on1,on2,on3,on4,oval,ow,om,ost,oa,ose);       // print the last line
}
...

{
 ...
 if (narg>0) {
   if (((long long int)param>=0 && (long long int)param<MAXPARM) ||
      (((long long int)param<0  || (long long int)param>MAXPARM) && (param->type!=S))) {
 ...
 if ((long long int)param>=0 && (long long int)param < MAXPARM) { /* if Ca (G) parameter */
 
   type=(long long int)param;

   cagparm = (long long int)*pc++;
   switch (type) {
   case G:
	switch(cagparm) {
...

{
   ...
   if (disp_ray) {				     /* move camera, not objects */
     initray(xrot,yrot,zrot,-dxcent,-dycent,-dzcent,
			rxcent,rycent,rzcent,dsize,bknd_color); /* initialize ray-tracer */
   }
 }
}
...
/*	 else  dcomp2 (n1a,n1b,n1c,n1d,n2a,n2b,n2c,n2d,elemtype,exceptype,
   ...
   if ((elemtype==NODE) && (disp & DNODE)) {
        ncdrnod (n1a,n1b,n1c,n1d,elemtype,exceptype,
					na,nb,nc,nd,
					zrange1,zrange2,
					dcolor,vpen,vpenn,dscale,cmap,0);
   }
   ...
   if (elemtype==LABEL && !nolabels) {
        ncdrnod (n1a,n1b,n1c,n1d,elemtype,exceptype,
					na,nb,nc,nd,
					zrange1,zrange2,
					dcolor,vpen,vpenn,dscale,cmap,1);
   }
         ...
         
{
 ...
 /* if (eparam) ncfprintf (stderr,"foreacode efilt name '%s'\n",eparam->name); */
    ...
    narg = (long long int)*pc++;		/* number of snode args to expect */
   ...
   if (narg) {
    ...
    for (i=0; i<NUMDIM; i++) {
       ...
       val[i]  = NULLVAL;
       arg[i]  = (long long int)*pc++;
    }
#ifndef XSTIMM
    execute (savepc+9);			/* evaluate node descriptors */
#endif

   narg2 = (long long int)*(savepc+6);	/* get number of "within" node args */
 ...
 if (efilt) {					/* search all elems */
   for (epnt=elempnt; epnt; epnt=tepnt) {
	 ...
	 if (arg[i]) {			/* If dimension was given blank var, */
       ...
       if (!forexec(savepc+(long long int)*(savepc+7))) break;	
...
#endif
  if (!returning) {
	pc = savepc+(long long int)*(savepc+8);	/* next stmt */
  }
}
...

{
   ...
   int found;
    ...
    if (epnt->node1a != NULLVAL) {
      ...
      if (! found) {				/* if not, then make it */
 	    setnodlst(newnod=maknod(epnt->node1a,epnt->node1b,epnt->node1c,epnt->node1d),epnt);
	     if (epnt->region!=NULLVAL && newnod->region==NULLVAL) newnod->region = epnt->region;
	    epnt->nodp1 = newnod;		/* set node pointer */
      }
    }	/* if (epnt->node1a != NULLVAL) */
    ...
    if (! found) {				/* if not, then make it */
 	    setnodlst(newnod=maknod(epnt->node2a,epnt->node2b,epnt->node2c,epnt->node2d),epnt);
	    if (epnt->region!=NULLVAL && newnod->region==NULLVAL) newnod->region = epnt->region;
	    epnt->nodp2 = newnod;		/* set node pointer */
    }
