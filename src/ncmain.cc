...
int	stimaddf = 0;		/* =1 -> run stimadd on stimfile */
 ...
 do					/* if there are any switches */
  {
   ...
   if (argc)
	...
	case 'A': stimaddf = !stimaddf;		/* run stimadd() on stimfile */
		  silent = 1;			/* don't print help */
		  break;
...
/* if(fin== &cin) system ("stty erase \n"); 		/* */
  ...
  run_interp();			/* in "ncm.c"    */
  if (makestim) {
     if (stimout!=stdout) ncfprintf(stimout,"## %s version %s\n",progname,print_version(ncversion));
...
#else
double elap_time(void) {return 0;}
