...
extern datum	ncpow(datum &x, datum &y), ncgamma(datum &x), ncabs(double &x);
extern datum    elap_timee(void), system_speedd(void), simwait(datum &d1);
extern datum	setvarval(void), xisnum(datum &d), xisalpha(datum &d); 
extern datum	xstrlen(datum &d), set_q10s(void), ncfactorial(datum &x);
extern datum	ncpvalue(datum &d1, datum &d2);
extern datum	ncttest(datum&d1), ncttest2(datum &d1);
...
extern datum	xstrchr(datum &d1,datum &d2);
extern datum	xstrrchr(datum &d1,datum &d2);
...
extern datum	(setchan_rateo)(datum &sc,datum &ss,datum &state, datum &trans, 
					datum &val);
extern datum	(call_set2ndmsg) (datum &ctype, datum &stype, datum &prect, datum &postct, 
					datum &maxdist, datum &gain, datum &mtype);
...
static builtin builtins[] = {
	...
	"gamma",(Fpa )ncgamma,	/* checks range */
	"factorial",(Fpa )ncfactorial,	/* checks range */
	"pvalue",(Fpa )ncpvalue, /* checks range */
	"ttest",(Fpa )ncttest, /* checks range */
	"ttest2",(Fpa )ncttest2, /* checks range */
	...
	"simwait", (Fpa )simwait,
	"isnum",  (Fpa )xisnum,
	"isalpha", (Fpa )xisalpha,
	"strlen", (Fpa )xstrlen,
	"strtok", (Fpa )xstrtok,
	"strcmp", (Fpa )xstrcmp,
	"strstr", (Fpa )xstrstr,
	"strchr", (Fpa )xstrchr,
	"strrchr", (Fpa )xstrrchr,
	"substr", (Fpa )xsubstr,
	"ccstr", (Fpa )xccstr,
	"atof",  (Fpa )xatof,
...
void varcopy(void)
{
  ...
  dcacomplen = getval ("dcacomplen");
  dcashd  = getval ("dcashd");
  ddca    = getval ("ddca");
  ddcao   = getval ("ddcao");
  ...
  rncolor   = (int)getval ("rncolor");
  cacolor   = (int)getval ("cacolor");
  naacolor   = (int)getval ("naacolor");
  naicolor   = (int)getval ("naicolor");
  sgcolor   = (int)getval ("sgcolor");
  srcolor   = (int)getval ("srcolor");
  bknd_color= (int)getval ("bknd_color");
  node_color= (int)getval ("node_color");
...
#define NR NUMBER
 ...
 setval (lookup("dftc"),2,NR);		/* synaptic time const (msec) */
 setval (lookup("dftm"),10.0,NR);	/* time const (msec) for mglur mod of channel */
 setval (lookup("dsgm"),5.0,NR);	/* gain for mglur mod of channel */
 ...
 setval (lookup("dmaxsyn"),1e-9,NR);	/* maximum cond for synapse (S) */
 ...
 setval (lookup("dvfbg"),1.0,NR);	/* gain for voltage feedback from transrate */
 setval (lookup("dvfbo"),0.03,NR);	/* offset for voltage feedback from transrate */
 ...
 setval (lookup("dsms"),100,NR);	/* max sustained release rate (/sec) */
 ...
 setval (lookup("dcacomplen"),2.5,NR);  /* default Ca compartment length (um) */
 setval (lookup("dcashd"),0.1,NR);      /* default Ca shell depth (um) */
 ...
 setval (lookup("dachu"),80e-12,NR);	/* unitary cond of ACh chan (S) */
 ...
 setval (lookup("rncolor"),RNCOLOR,NR);	/* Use color from cell region number */
 setval (lookup("cacolor"),CACOLOR,NR);	/* Use color from [Ca]i */
 setval (lookup("naacolor"),NAICOLOR,NR);/* Use color from Na chan activation */
 setval (lookup("naicolor"),NAICOLOR,NR);/* Use color from Na chan inactivation */
 setval (lookup("sgcolor"),SGCOLOR,NR); /* Use color from syn cond */
 setval (lookup("srcolor"),SRCOLOR,NR); /* Use color from syn rate */
 setval (lookup("bknd_color"),7,NR);    /* Background color for display of model */
 setval (lookup("node_color"),0,NR);    /* Node number color for display of model */
 setval (lookup("unmaskthr"),-1e20,NR); /* neg threshold for unmasking light */
 setval (lookup("unmaskthr2"),1e20,NR); /* pos threshold for unmasking light (i.e. Vm) */
...
#define VLINSIZ 40
    ...
    if (strcmp(progname,"ndv")==0) { 
          FILE *ftemp;
    	disp |= DISP; 
        setval (lookup("vidmode"), 1.0,NR);	/* set vid mode for output */
	vidmode=1; 
        setval (lookup("disp"), 1.0,NR);	/* set disp mode for output */
...

{
  ...
  setval (lookup("stdin"), (double)(long long int)stdin,NR);   /* standard input */
  setval (lookup("stdout"),(double)(long long int)stdout,NR);  /* standard output */
  setval (lookup("stderr"),(double)(long long int)stderr,NR);  /* standard err out */
}
...
#define NR NUMBER
 ...
 setval (lookup("dftc"),&dftc,NR);	/* synaptic time const (msec) */
 setval (lookup("dftm"),&dftm,NR);	/* channel time const (msec) for mglu */
 setval (lookup("dsgm"),&dsgm,NR);	/* gain for mglu */
 ...
 setval (lookup("dscaeg"),&dscaeg,NR);	/* synaptic ca vesicle release expon gain */
 ...
 setval (lookup("dvfbg"),&dvfbg,NR);	/* gain for voltage feedback */
 setval (lookup("dvfbo"),&dvfbo,NR);	/* gain for voltage offset */
 ...
 setval (lookup("dcacomplen"),&dcacomplen,NR); /* default Ca compartment length (um) */
 setval (lookup("dcashd"),&dcashd,NR);  /* default Ca shell depth (um) */
 ...
 setval (lookup("dachu"),&dachu,NR);	/* unitary cond of ACh chan (S) */
 ...
 setval (lookup("rncolor"),&rncolor,NR);/* Use color from cell region number */
 setval (lookup("cacolor"),&cacolor,NR);/* Use color from [Ca]i */
 setval (lookup("naacolor"),&naacolor,NR); /* Use color from Na chan activation */
 setval (lookup("naicolor"),&naicolor,NR); /* Use color from Na chan inactivation */
 setval (lookup("sgcolor"),&sgcolor,NR); /* Use color from syn cond */
 setval (lookup("srcolor"),&srcolor,NR); /* Use color from syn rate */
 setval (lookup("bknd_color"),&bknd_color,NR); /* Background color for display of model */
 setval (lookup("node_color"),&node_color,NR); /* Node number color for display of model */
 setval (lookup("numstimchan"),&numstimchan,NR); /* num of indep stimulus channels */
 setval (lookup("unmaskthr"),&unmaskthr,NR);   /* neg threshold for unmasking light */
 setval (lookup("unmaskthr2"),&unmaskthr2,NR); /* pos threshold for unmasking light */
