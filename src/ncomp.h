...
//#define RNDSIZ  (5*sizeof(uint64_t))    /* size of random number state table for rnd_taus258 */
//#define RNDSIZ  (8*sizeof(uint32_t))    /* size of random number state table for rnd_tinymt64 */
...
struct comp: id {			/* structure for compartment */
	...
	comp *compe;			/* pointer to outside comp (vext) */
...
struct chan: conn {            		/* basis stuct for membrane channel */
	...
	short int secmsgtype;		/* type of mGlu mod func */
	double secmsggain;		/* gain for mGluR2 synaptic modulation */
	synap *secmsgmod;		/* mGluR2 modulation by synapse */
...
struct synap: conn {                  	/* structure for synaptic connection */
	...
	double npow;			/* Hill coeff for nt binding */
        ...
        double casens;    	   	/* presyn sensitivity, 0 => V (default), >0 => Ca */
        double caegain;                 /* presyn ca expon sensitivity: 1 => linear, >1 => expon Ca */
        ...
        double vfbg;             	/* voltage feedback gain */
        double vfbo;             	/* voltage feedback offset */
	...
	double secmsggain;		/* gain for mglur2 modulation of Ca chans */
	double secmsgthresh;		/* threshold for mglur2 modulation of Ca chans */
        short int setvrev;             	/* =1 -> vrev set by user */
	...
	lpfilt *filtm;			/* low pass filter for 2ndmsg (mglu) delay */
        };
