...
{
   if (vidmode) _move (x,y); 			/* in libP.a */
   else ext_gmove(x, y);
}

void gdraw (double x, double y)
{
   if (vidmode) _draw (x,y);			/* in libP.a */
   else ext_gdraw(x, y);
}
...
{
 rmframe ((char *)s);
 if (!vidmode) ext_grmframe((char *)s);
}

void gsize (double x)
{
 scale (x,x);
 if (!vidmode) ext_gsize(x);
}

void gwindow (double x1, double y1, double x2, double y2)
{
 window (x1,y1,x2,y2);
 if (!vidmode) ext_gwindow(x1, y1, x2, y2);
}
