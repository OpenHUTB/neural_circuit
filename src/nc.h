...
typedef struct SYMTYPE {
	const char *name;
	int type;			/* type set in variables */
	int vtype;			/* type set in varval */
	char   *cptr;			/* command line string */
	union {
	    double val;
	    int    *iptr;
	    char   **sptr;
...
void conn1m(void), dofft(void), do_lmfit(void), do_lmfit2d(void);
void gettype(void), getntype(void); 
