...

{
 ...
 case SYNAPSE:	
	...
	((synapse *)epnt)->nfiltm = NULLVAL;
	((synapse *)epnt)->tfallm = NULLVAL;
	((synapse *)epnt)->timecm = (double *)NULL;
	...
	((synapse *)epnt)->casens = NULLVAL;	/* 0 => Voltage sens, >0 => Ca sens */
...

{
	...
	double *filt;
     ...
     case SYNAPSE: if (filt=((synapse*)epnt)->timec1) efree (filt);
     		   ...
     		   if (filt=((synapse*)epnt)->timecm) efree (filt);
