...
void label(synapse *spnt, int color, const char *text)
{
  if (spnt!=NULL) {
      if (spnt->nodp1!=NULL) {
	  spnt->nodp1->label = color;			// set to zero to set label on
...
photorec *make_transducer (node *node1, int stimchan)

{
   return make_photorec(VTRANSDUCER,node1,stimchan);
}
...
photorec *make_transducer (node *node1, double xpos, double ypos, int stimchan)

{
   return make_photorec(VTRANSDUCER,node1,xpos,ypos,stimchan);
}
...
elem *make_conn (node *node1, node *node2, int elemtype )
{
   elem *epnt;
...
elem *make_conn (int node1a, int node1b, int node1c, int node1d,
...
elem *make_conn (int node1, int node2, int elemtype)

{
  return make_conn(node1, NULLVAL, NULLVAL, NULLVAL,
		   node2, NULLVAL, NULLVAL, NULLVAL, elemtype);
}

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

elem *make_conn (int node1a, int node1b, int node2a, int node2b, int elemtype)

{
  return make_conn(node1a, node1b, NULLVAL, NULLVAL,
		   node2a, node2b, NULLVAL, NULLVAL, elemtype);
}

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

elem *make_conn (int node1a, int node1b, int node1c, 
		 int node2a, int node2b, int node2c, int elemtype )

{
  return make_conn(node1a, node1b, node1c, NULLVAL,
...

{
  ...
  epnt = make_conn (node1, node2, CABLE);
  return (cable *)epnt;
}
...

{
   ...
   s = (electrode *)make_conn (node1, node2, ELECTRODE);
   return s;
}
...
double v(int elemnum, double fracdist)
{
   ...
   if (!makestim) rval = reccable(elemnum, fracdist);
   else rval = 0;
   return rval;
}
...
/* nt = GLU, AMPA, KAINATE, ACH, NMDA, CNQX, GABA, BIC, PTX, GLY
        STRY, CAMP, CGMP
*/
...
//
//      if (!ca_err_printed) {
//          ncfprintf (stderr,"# nc: error, node %s, region %d  does not have a Ca compartment.\n",
//                 prnode(npnt), npnt->region);
//          ca_err_printed = 1;
//      }
    }
  }
...
void display (int disptype, node *nd1, int dcolor, int cmap, double dscale)

{
    int elemtype, narg2, exceptype, hide, excl;
    double vmax, vmin, stime;
    Symbol *vpen;
    double (*vpenn)(int,int);
    node *nd2, *nexc;
...
void display (int elemtype, int disptype, node *nd1, int dcolor, int cmap, double dscale)

{
    int narg2, exceptype, hide, excl;
    double vmax, vmin, stime;
    Symbol *vpen;
    double (*vpenn)(int,int);
    node *nd2, *nexc;
...
void display (int elemtype, int disptype, node *nd1, node *nd2, int dcolor, int cmap, double dscale)
...
nattrib *make_chsyn_noise (elem *epnt)
...

{
  ...
  spnt = (synapse *)make_conn (node1, node2, SYNAPSE);
  return spnt;
}
...

{
  ...
  g = (gapjunc *)make_conn (node1, node2, GJ);
  g->gmax = maxcond;
  return (gapjunc *)g;
}
...

{
  ...
  g = (gapjunc *)make_conn (node1, node2, PNX);
  g->gmax = maxcond;
  return (gapjunc *)g;
}
...

{
  ...
  b = (vbuf *)make_conn (node1, node2, BUF);
  b->offset = offset;
  b->gain = gain;
  b->delay = delay;
  b->tau = tau;
...
nbuf *make_nbuf (node *node1, node *node2, double offset, double gain, double ntoffset, int ntrans)
{
  ...
  n = (nbuf *)make_conn (node1, node2, NBUF);
  n->offset = offset;
  n->ntoffset = ntoffset;
  n->gain = gain;
  n->ntrans = ntrans;
...
/* If elem is cable, return 2nd matching node, to allow finding dendritic/axon tips */

{
   elem *epnt=ept;
   int etype = foreachn_etype;
   int na = foreachn_a;
   int nb = foreachn_b;
  ...
  for (; epnt; epnt=epnt->hnnext) { 		/* look for elem type */
    ...
    if (epnt->ctype==SPHERE)  *foreachn_pn = epnt->nodp1; /* return 1st matching node */
    else                      *foreachn_pn = epnt->nodp2; /* return 2nd matching node */
    break;
  }
  return epnt;
}
...
{
    elem *epnt=ept;
    int etype = foreachn3_etype;
    int na=foreachn3_a;
    int nb=foreachn3_b;
    node *npntw = foreachn3_npntw;
    double radius = foreachn3_radius;
    double r1;
    double r2 = 1e10;
  ...
  for (; epnt; epnt=epnt->hnnext) { 				/* look for elem type */
    ...
    r1 = foreachn3_distfunc(epnt->nodp1,npntw);
    if (epnt->nodp2) r2 = foreachn3_distfunc(epnt->nodp2,npntw);
    if (r1 <= r2) *foreachn3_pn = epnt->nodp1;
    else          *foreachn3_pn = epnt->nodp2;
    break;
  }
  return epnt;
}
...
int set2ndmsg (int ctype, int stype, int prect, int postct, int ntype, double maxdist, 
				double gain, double tau, double thresh)

/* For every channel of type "ctype,stype" in postct (e.g. SBACs), find nearby synapse and save. */
/* If cype or stype < 0, any type or subtype matches */
/*  Must match ntype (GLU, AMPA, KAINATE, GABA, etc) */
/* Run this once after first step(), to define runtime synap's. */

{
    int j, n, nnodes, num1, num2, numcomp, nchans, postcn;
    double xloc, yloc, zloc;
    double avgx,avgy,avgz,ltau;
    double xdist, ydist, zdist, dist, mindist;
    double x1,y1,z1;
    double x2,y2,z2;
    comp *comp1,*compn,*pnt,*pnt1,*pnt2;
    elem *epnt;
    synapse *esav;
    node *npnt;
    chan *chpnt;
    ...
    // find location of channel on postct from average node loc
    //  and remember the cn of the cell (as postcn)

  for (nchans=0,chpnt=chanpnt; chpnt; chpnt=(chan*)chpnt->next) {   /* channels */
    postcn = -1;
    if (chpnt->ctype==ctype && (stype<0 ? 1 : chpnt->stype==stype) ) {
	if ((pnt=chpnt->comp1)!=NULL) {
	     avgx=avgy=avgz = 0;
	     for (nnodes=0,lpnt=pnt->nodlst; lpnt; lpnt=lpnt->next) {
	          if (npnt=(node*)lpnt->conpnt) {
			  if (npnt->nodenm1!=postct) break;
			  postcn = npnt->nodenm2;
			  avgx += npnt->xloc;
			  avgy += npnt->yloc;
			  avgz += npnt->zloc;
	    ...
	    if (nnodes > 0) {
                    xloc = avgx / nnodes;	/* average loc of comp */
                    yloc = avgy / nnodes;
                    zloc = avgz / nnodes;
	    }

	    else {		/* If no nodes at original comp, */
		    		/*   find first and last node in cable. */
				/* See dcomp() in prcomp.cc */
             ...
             for (j=0,lpnt=pnt->clst; lpnt; lpnt=lpnt->next,j++)  { /* find first node */
               ...
               if (j==0) {		 /* get avg loc of first end compartment */
                  num1 = n;
                  avgx=avgy=avgz=0.0;
                  for (nnodes=0,nlpnt=pnt1->nodlst; nlpnt; nlpnt=nlpnt->next) {
  
                    if (nlpnt->conpnt) {
                        npnt = (node *)(nlpnt->conpnt);
			if (npnt->nodenm1!=postct) break;	/* stop if not postct */
			postcn = npnt->nodenm2;
			...
			nnodes++;
                  ...
                  if (nnodes) {
                    x1 = avgx / nnodes;			/* average loc of first compartment */
                    y1 = avgy / nnodes;
                    z1 = avgz / nnodes;
                  }
               }  /* if (j==0) */
		      ...
		      nnodes++;
                  ...
                  if (nnodes) {
                    x2 = y2 = z2 = 0.0;
                    if (avgx) x2 = avgx / nnodes;	/* average loc of second end comp */
                    if (avgy) y2 = avgy / nnodes;
                    if (avgz) z2 = avgz / nnodes;
                  }
                 }   /* if (j==1) */
             }    /* for (j=0;;) */

	     numcomp = num1 + num2 + 2;		/* number of comps */
	     j = num1;
	    ...
	    if (info >= 3)
	      fprintf (stderr,"# chan loc %-4.3g %-4.3g %-4.3g\n",xloc,yloc,zloc);

    // find nearest synapse located on same cell (i.e. postct, postcn) as channel 
    
  	    if (nnodes > 0) { 
	      mindist = 1e6; 
	      esav = NULL;
              for(epnt=elempnt; epnt = foreach (epnt, SYNAPSE, prect, -1, postct, postcn, 0); epnt=epnt->next) {
                // fprintf (stderr,"# dbp1 %d sbac %d\n", epnt->node1b, epnt->node2b);

	          if (((synapse*)epnt)->attpnt != NULL) { 		// if post-syn channel has been specified
		        // fprintf (stderr, "ctype %d ntype %d\n",((synapse*)epnt)->attpnt->ctype, ntype);
		        if (((synapse*)epnt)->attpnt->ctype != ntype)  continue; 
		  }

		  xdist = xloc - epnt->nodp2->xloc;
		  ydist = yloc - epnt->nodp2->yloc;
		  zdist = zloc - epnt->nodp2->zloc;
		 ...
		 if (dist < mindist) {
	      ...
	      if (esav != NULL && mindist < maxdist) {

                if ((spnt=(synap*)esav->lptr)!= NULL) {		// get pointer to run-time synap

		     // make temporal filter for 2ndmsg (mglur2) in synapse if it hasn't already been made
		     
		     // fprintf (stderr,"set2ndmsg 1 filtm %x %g %g\n",spnt->filtm,tau,dftm);	
		     if (spnt->filtm==NULL) {
			 ltau = tau;
			 if (tau <= 0) ltau = dftm;
			 spnt->filtm = maksynfilt (LP, 2, 0, 0, 0, makfiltarr(2,0,NULL,ltau), stiminc/MSEC);
		         // fprintf (stderr,"set2ndmsg 2 filtm %x %g %g\n",spnt->filtm,ltau, dftm);	
		     }

                     chpnt->secmsgmod = spnt;
                     chpnt->secmsgtype = ntype;
		     if (gain < 0) gain = dsgm;		/* default gain for mglur2 */
                     chpnt->secmsggain = gain;
		     spnt->secmsggain = gain;		/* gain for plot of synaptic mglu mod */
		     spnt->secmsgthresh = thresh;	/* thresh for plot of synaptic mglu mod */
		     nchans++;
		     if (info >= 3) fprintf (stderr,"# synap    %-4.3g %-4.3g %-4.3g   %-4.3g\n#\n", 
				     esav->nodp2->xloc,esav->nodp2->yloc,esav->nodp2->zloc,mindist);
...
int set2ndmsg (int ctype, int stype, int prect, int postct, int mtype, double maxdist, double gain, double tau)

/* same as above but call to use default 2nd msg tau: dftm */

{
   return set2ndmsg (ctype, stype, prect, postct, mtype, maxdist, gain, tau, 0.0);
}

/*-----------------------------------------------------------*/

int set2ndmsg (int ctype, int stype, int prect, int postct, int mtype, double maxdist, double gain)

/* same as above but call to use default 2nd msg tau: dftm */

{
   return set2ndmsg (ctype, stype, prect, postct, mtype, maxdist, gain, -1, 0.0);
}

/*-----------------------------------------------------------*/

int set2ndmsg (int ctype, int stype, int prect, int postct, int mtype, double maxdist)

/* same as above but call to use default 2nd msg tau: dftm */

{
   return set2ndmsg (ctype, stype, prect, postct, mtype, maxdist, -1, -1, 0.0);
}
