...
{       
  ...
  if (stimchan >= numstimchan) numstimchan = stimchan+1;
  return stimchan;
}
...
{
     ncfprintf(stimout,"%-.4g %-d %-d %-d %-d %-12.6g %-g %-g %-d %c %d\n",
			time, node1,node2,node3,node4,inten,wavel,
...

{
  ...
  if (node1 == NULLVAL) node1 = NULND;
   ...
   printstim (time,node1,node2,node3,node4,inten,npt->wavel,mask,stimchan,*action,seq);
}
...

{
 ...
 if (ncabs(delta) > 1e-3) {                       /* save and output new val */
...

{
  ...
  ncinitstate (chseed,chpnt,RNDSIZ);
...
void makgabor(int arr, int arrsize, double contrast, double inten_add, double inten_mult,
		...
		int makenv, int ssq, int tsq, double tfreq, double drift, 
  ...
  for (stime=start; stime<stimend; stime += tincr) {
    ...
    if (tmod) {
      ...
      if (drift) {		     /* drifting grating */
        ...
        sinttheta = sinsq2(2*MPI * toffset,tsq); /* temporal shift, indep sine func */
      }
    }
    xradsq=xenv*xenv;
    yradsq=yenv*yenv;
	...
	//		  sinttheta, stoff);
	 ...
	 sinstheta = sinsq2(2*MPI * stheta,ssq) * exp(-xr*xr/xradsq) * exp(-yr*yr/yradsq);
		...
		double mask, double start, double dur)
{
   ...
   makgabor(arr, arrsize, contrast, inten_add=0, inten_mult, xloc, yloc, xcent, ycent, 
		     xenv, yenv, makenv, ssq, tsq, tfreq, drift, speriod, sphase, 
...

{
  ...
  stimlist(1.0, start,mask,stimchan);
  stimlist(-1.0, start+dur,mask,stimchan);
  recback(arr,0.0,1.0);     /* zero photrec stim inten */
}
