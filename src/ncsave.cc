...

{
 ...
 if (cumsynap > 0 || cumgj > 0 || cumdbuf) {
  fprintf (sfil,"# synapses num conduct vrev transrate vtint vtime vflag rrpool oldc1 oldc2 1 1h 2 3 m\n");
  for (spnt=synpnt; spnt; spnt=(synap *)spnt->next) {      /* save synapses */

   switch (spnt->ctype) {

    case SYNAPSE:
    fprintf (sfil,"s    %4d %.16g %.16g %.16g %.8g %.8g %d %.8g %.16g %.16g ",
			spnt->num,spnt->conduct,spnt->vrev,spnt->transrate,
			spnt->vtint,spnt->vtime,spnt->vflag,
			spnt->rrpool,spnt->oldc1,spnt->oldc2);
    ...
    case BUF:
     ...
     if (delay > 0) {
       fprintf (sfil," %lld", (long long int)(((dbuf *)spnt)->delpnt));
       for (i=0; i<delay; i++) {
...

{
   ...
   double offset, v; 
   double cais,cab;
   double val, scond, transrate, gjfrac,rrpool,oldc1,oldc2;
   double vtint,vtime;
  ...
  for (npl=0; fgets(inbuf,inbufsiz,sfil)!=NULL; npl++) {		/* scan file */
   ...
   if (inbuf[0]=='#')  {				/* comps, synapses, photrecs */
     switch (inbuf[2])  {
      ...
      case 'c':						/* found "# compartments */
       ...
       for (; fgets(inbuf,inbufsiz,sfil)!=NULL; npl++) {
      ...
      case 'k':						/* found "# kchannels" */
       ...
       for (; fgets(inbuf,inbufsiz,sfil)!=NULL; npl++) {
	    ...
	    if (n != 7) {
    		sprintf (ebuf,"restoremodel: can't read channel %d.\n",chnum);
     	        execerror ("warning,",ebuf);
		return;
	    }
	    if (chpnt->num!=chnum) {
	  ...
	  break;
       ...
       for (; fgets(inbuf,inbufsiz,sfil)!=NULL; npl++) {
		...
		/* look for delay buf */
	      ...
	      ((dbuf *)spnt)->delpnt  =  (double *)(long long)bpnt;
	    ...
	    break;
         ...
         case 's':					/* synapse */
           switch (inbuf[1])  {
            ...
            sscanf(inbuf,"s %d %lg %lg %lg %lg %lg %d %lg %lg %lg",
			    &snum,&scond,&vrev,&transrate,&vtint,&vtime,&vflag,&rrpool,&oldc1,&oldc2);
	    ...
	    spnt->oldc2   = oldc2;
	    tsyns++;
	    cp = scantoken (inbuf,12);			/* find 12th token */
