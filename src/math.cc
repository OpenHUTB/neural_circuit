...
double pvalue(double t, int df);
double ttest(datum &data);
double ttest2(datum &data);
...
double dogamma(double x)
{
	x=errcheck(gamma(x), "gamma");
	return x;
}
datum ncgamma(datum &x)
{
        return domonofunc(x,dogamma);
}
...
{
	x=errcheck(factorial(int(x)), "factorial");
	return x;
}
datum ncfactorial(datum &x)
{
        return domonofunc(x,dofactorial);
}

/* - - - - - - - - - - - - - - - - - */

double dopvalue(double x, double df)
{
	x=errcheck(pvalue(x,df), "pvalue");
	return x;
}
datum ncpvalue(datum &x, datum &df)
{
        return doduofunc(x,df,dopvalue);
}

/* - - - - - - - - - - - - - - - - - */

datum ncttest(datum &data)
{
	datum y;
	y.val=errcheck(ttest(data), "ttest");
	y.vtype = NUMBER;
        return y;
}
...
datum ncttest2(datum &data)
{
	datum y;
	y.val=errcheck(ttest2(data), "ttest2");
	y.vtype = NUMBER;
        return y;
}
...
/* algorithm for real values of z */
...
double gamma(double z)
{
    ...
    if (z < 0.5) {
        y = MPI / (sin(MPI * z) * gamma(1 - z));  // Reflection formula
    } else {
        z -= 1;
        x = p[0];
        for (i=1; i<n; i++)
            x += p[i] / (z + i);
        t = z + g + 0.5;
        y = sqrt(2 * MPI) * pow(t, (z + 0.5)) * exp(-t) * x;
    };
    return (y);
};
...
datum xisalpha(datum &d)
{
	switch (d.vtype) {
	  ...
	  case NUMBER:
		...
		break;
	  case LITCHAR:
		if (isalpha(int(d.val))) d.val = 1; else d.val = 0;
		break;
	  case STRING:
		if (isalpha(d.str[0])) d.val = 1 ;else d.val = 0;
		break;
	}
	d.vtype = NUMBER;
	return d;
...
datum xstrchr(datum &d1, datum &d2)
{
    const char *p;
    char ch;
...
datum xstrrchr(datum &d1, datum &d2)
{
    const char *p;
    char ch;
...

{
  ...
  if      (n<=0) retval = 1;
  else if (n==1) retval = 1;
  else {
     for (retval=i=1; i<=n; i++) {
	 retval *= i;
     } 
  }
  return retval;
...
/* rising factorial, or Pochhammer function */ 
...
/* 2F1 (a,b;c;z); From https://en.wikipedia.org/wiki/Hypergeometric_function */

{
   ...
   sval = 90;
   for (n=1; n<sval; n++) {
     retval +=  ( poch(a,n) * poch(b,n) ) / poch(c,n) * pow(z,n) / factorial(n);
   printf ("hyperg %g\n",retval);
   } 
   return retval;
}
...
//  Student's pvalue function (student cumulative dist)
//  From: https://en.wikipedia.org/wiki/Student%27s_t-distribution
//   valid for t*t < df

double pvalueg (double t, int df)
{
     double retval;

   retval = 1 - ( 0.5 + t * gamma (0.5*(df+1)) / ( sqrt(MPI*df)*gamma(df*0.5) ) * 
		   hypergeom(0.5, 0.5*(df+1), 1.5, -t*t/df) );
   return retval;
}
...
double gsl_cdf_tdist_Px (double x, double nu);
double gsl_cdf_tdist_Qx (double x, double nu);

double pvalue (double t, int df)

/* t is t-score, df is n-1 */
/* From: gsl-2.7 */

{
     double retval;
     static double t2, df2;
  ...
  if     (t2==0) retval = 0.5;
  else if (t>0)  retval = gsl_cdf_tdist_Qx (t2, df2);
  else           retval = gsl_cdf_tdist_Px (t2, df2);
  return retval; 
}

/* - - - - - - - - - - - - - - - - - - - - - */

double ttest2(double *arr, int nrows)

/* Do a paired ttest on an array with ncols=2 and nrows */
/*  This is equivalent to doing a one-sample test on the diffeences */
...
{  
 ...
 nrows = data.arrp->dim[0];
 if (ncols=data.arrp->dim[1] != 2) {
     fprintf (stderr,"ttest: error, ncols != 2\n");
     return 0;
 }
 arr = data.arrp->arr;
 return ttest2(arr, nrows);
}

/* - - - - - - - - - - - - - - - - - - - - - */

double ttest(double *arr, int nrows)

/* Do an independent ttest on an array with ncols=2 and nrows */
/* This is also called a "two-sample" test. */
