...
void makgabor(int arr, int arrsize, double contrast, double inten_add, double inten_mult,
		double xloc, double yloc, double xcent, double ycent,
		double xenv, double yenv, int makenv, int ssq, int tsq,
...

{
 if (dur > 0 && !makestim) {
  switch (puffmsg) {
   ...
   break;
   case CA:
      makvstim(start,     npnt->nodenm1,npnt->nodenm2,npnt->nodenm3,npnt->nodenm4, puffconc,0,"v");
      makvstim(start+dur, npnt->nodenm1,npnt->nodenm2,npnt->nodenm3,npnt->nodenm4,-puffconc,1,"v");
   break;
   }
  }
}
...
#define  STFILNAM 100
  ...
  if (makestim) {			/* "stim" running, make stim file */
     ...
     *stimfile = 0;
     strncpy (stimfile,filename,100);	/* copy filename */
     ...
     if (info >= 1) fprintf (stderr,"# Creating stim file '%s'\n",stimfile);
     stcomment();			/* print column label comment at beginning of stim file */
  }
  else	/* nc running */
	   ...
	   If it doesn't exist, look for .gz, .bz2, or .xz compressed versions.  */
  {
    ...
    if ((ftemp=fopen(stimfile,"r"))==NULL) { /* open file */
      ...
      if ((ftemp=fopen(stimfilz,"r"))==NULL){ /* look for .gz file */
         ...
         if ((ftemp=fopen(stimfilz,"r"))==NULL){ /* look for .bz2 file */
            ...
            if ((ftemp=fopen(stimfilz,"r"))==NULL){ /* look for .xz file */
               ncfprintf (stderr,"%s: can't find stim file '%s'.\n",progname,filename);
      ...
      switch (compress_type) {
		case 2: strcpy(stimfilz,"gzip -dc ");		/* check for .gz file */
			break;
		case 3: strcpy(stimfilz,"bzip2 -dc ");		/* check for .bz2 file */
			break;
		case 4: strcpy(stimfilz,"xz -dc ");		/* check for .xz file */
			break;
      }
        strncat(stimfilz,stimfile,STFILNAM-8);
	switch (compress_type) {
		case 2: strncat(stimfilz,".gz",STFILNAM-8);	/* .gz file */
			break;
		case 3: strncat(stimfilz,".bz2",STFILNAM-8);	/* .bz2 file */
			break;
		case 4: strncat(stimfilz,".xz",STFILNAM-8);	/* .xz file */
			break;
	}
    ...
    if (info>=1)
       switch (compress_type) { 
	    case 1: fprintf (stderr,"# Using stim file '%s'\n",stimfile);
		    break;
	    case 2: 
	    case 3: 
	    case 4: 
		    fprintf (stderr,"# Decompressing stim file with '%s'\n",stimfilz);
		    break;
       }
    stimin = ftemp;
    stimfflg = 1;
...
{
  stim_backgr(backgr, wavel, mask, 0, start);
}

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

void stim_backgr (double backgr, double mask, int stimchan, double start)

{
  stim_backgr(backgr,1, mask, stimchan, start);
}

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

void stim_backgr (double backgr, double mask, double start)

{
  stim_backgr(backgr,1, mask, 0, start);
}
...
void stim_blur (double blurdia, double blur_ampl, double scatter_ampl, double scatter_rad, 
...

{
   ...
   if (! makestim || blur_ampl == 0) return;

   blursize=makblur(blurdia/2.0,blur_ampl,sscale,  /* make a blur function */
...
//      }
      ...
      if (blurdia>0) stimarrsize += blursize*2;
...
void stim_blur (double blurdia, double scatter_ampl, double scatter_rad, 
			double scatter_pow, double sscale)
{
     double blur_ampl;

    stim_blur (blurdia, blur_ampl=1.0, scatter_ampl, scatter_rad, scatter_pow, sscale);
}

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

void stim_blur (double blurdia, double blur_ampl, double sscale)
{
     double scatter_ampl, scatter_rad, scatter_pow;
     
    stim_blur (blurdia, blur_ampl, scatter_ampl=0, scatter_rad=0, scatter_pow=0, sscale);
}

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

void stim_blur (double blurdia, double blur_ampl)
{
     double scatter_ampl, scatter_rad, scatter_pow, sscale;
     
    stim_blur (blurdia, blur_ampl, scatter_ampl=0, scatter_rad=0, scatter_pow=0, sscale=1);
}
...
void stim_spot (double dia, double xloc, double yloc, double inten, 
		double start, double dur, double mask, int stimchan, int invert)
{
   int arr;
   double scale = 1.0;
   double wavel = 1.0;
		...
		double scale, double inten, double start, double dur, double wavel, double mask)
{
   ...
   stim_spot(dia, xloc, yloc, xcent, ycent, scale, inten, start, dur, wavel, mask, stimchan=0);
}
...
void stim_spot (double dia, double xloc, double yloc, double scale, double inten, double start, double dur)
{
   double xcent, ycent, wavel, mask;
   int stimchan;

   stim_spot(dia, xloc, yloc, xcent=0, ycent=0, scale, inten, start, dur, wavel=1, mask=0, stimchan=0);
}
...
void stim_spotc (double dia, double xloc, double yloc, double inten, double start, double dur, int stimchan)
{
   double xcent, ycent, scale, wavel, mask;
   stim_spot(dia, xloc, yloc, xcent=0, ycent=0, scale=1.0, inten, start, dur, wavel=1, mask=0, stimchan);
}
...
void makgaborx(double speriod, double sphase, double orient,
		...
		int makenv, int ssq, int tsq, double mask, int stimchan,
		const char *action,
		double start, double dur)
...

{
  ...
  if (drift || tfreq) {			/* drifting or counterphase grating */
     ...
     tincr = ncabs(tincr);		/* allow negative temporal frequency */
  ...
  for (stime=start; stime<stimend; stime += tincr) {
   ...
   if (tmod) {
     ...
     if (drift) {                      /* drifting grating */
       ...
       sinttheta = sinsq(2*MPI * toffset,tsq); /* temporal shift is indep sine func */
     }
   }
   if (runyet) {
    for (rpnt=recpnt; rpnt; rpnt=(photrec*)rpnt->next) {
	 ...
	 y = rpnt->yloc;
          ...
          if (inrect(x,y,xedgemax,xedgemin,yedgemax,yedgemin,orient)) {
			...
			ssq, sino, coso, sphasenorm, speriod, sinttheta, stoff);
	  } 
...

{
   ...
   makgaborx(speriod, sphase, orient, xloc, yloc, tfreq, drift, inten_add=0, inten_mult, contrast, 
		   wavel, xenv, yenv, makenv, ssq, tsq, mask, stimchan, action, start, dur);
}
...
void stim_sine(double speriod, double sphase, double orient, 
		...
		double inten_add, double inten_mult, double contrast, 
		int ssq, int tsq, double start, double dur, double wavel, double mask, int stimchan)
{
    const char *action;
    int arr,makenv;
    double xenv, yenv;
		...
		int ssq, int tsq, double start, double dur, double wavel, double mask, int stimchan)
{
  ...
  stim_sine(speriod, sphase, orient, xloc, yloc, xcent, ycent, tfreq, drift, scale, 
		   inten_add=0, inten_mult, contrast, ssq, tsq, start, dur, wavel, mask, stimchan); 
}
/*---------------------------------------------------------------------*/
// 13
...
void stim_sine(double speriod, double sphase, double orient, 
		...
		double inten_add, double inten_mult, double contrast, 
		int ssq, int tsq, double start, double dur)

{
   ...
   stim_sine(speriod, sphase, orient, xloc, yloc, xcent=0, ycent=0, tfreq, drift, 
		   scale=1, inten_add, inten_mult, contrast, ssq, tsq, start, dur, wavel=1, mask=0, stimchan=0);
}
...

{
   ...
   stim_sine(speriod, sphase, orient, xloc, yloc, xcent=0, ycent=0, tfreq, drift, 
		   scale=1, inten_add=0, inten_mult, contrast, ssq, tsq, start, dur, wavel=1, mask=0,stimchan=0);
}
...
void stim_sine(double speriod, double sphase, double orient, 
		double xloc, double yloc, double tfreq, int drift, 
		double inten_mult, double contrast, 
		int ssq, double start, double dur)

{
   ...
   stim_sine(speriod, sphase, orient, xloc, yloc, xcent=0, ycent=0, tfreq, drift, 
		   scale=1, inten_add=0, inten_mult, contrast, ssq, ssq, start, dur, wavel=1, mask=0,stimchan=0);
}
		...
		int ssq, int tsq, double start, double dur, double wavel, double mask, int stimchan)
{
  ...
  if (makestim) 
	makgabor(arr=STIMARR, stimarrsize, contrast, inten_add, inten_mult,
		...
		xenv, yenv, makenv=1, ssq, tsq,
		tfreq, drift, speriod, sphase,
		scale, orient, wavel,
		mask, stimchan, start, dur);
  else
  	makgaborx(speriod, sphase, orient, xloc, yloc, tfreq, drift,
                inten_add, inten_mult, contrast, wavel=1, xenv, yenv, makenv=1, ssq, tsq,
		mask=0, stimchan, action="d", start, dur);
		...
		int ssq, int tsq, double start, double dur, double wavel, double mask, int stimchan)
{
   ...
   stim_gabor(speriod, sphase, orient, xloc,  yloc,  xcent,  ycent,  tfreq,  drift,  scale,  inten_add=0,  inten_mult,  contrast,  xenv,  yenv,  ssq,  tsq, start,  dur,  wavel,  mask, stimchan);
...
void stim_gabor(double speriod, double sphase, double orient, 
		...
		double tfreq, double drift, 
		double inten_add, double inten_mult, double contrast, 
		double xenv, double yenv, int ssq, int tsq, double start, double dur)
{
   double xcent, ycent, scale;
   double wavel, mask;
   int stimchan;
  ...
  stim_gabor(speriod, sphase, orient, xloc, yloc, xcent=0, ycent=0, tfreq, drift, scale=1,
		inten_add, inten_mult, contrast, xenv, yenv, ssq, tsq, start, dur, wavel=1, mask=0, stimchan=0);
}
		...
		double xenv, double yenv, int ssq, int tsq, double start, double dur)
{
  ...
  stim_gabor(speriod, sphase, orient, xloc, yloc, xcent=0, ycent=0, tfreq, drift, scale=1,
		inten_add=0, inten_mult, contrast, xenv, yenv, ssq, tsq, start, dur, wavel=1, mask=0, stimchan=0);
}
		  ...
		  double contrast, double renv, int sq, double start, double dur)
{
   ...
   stim_sineann(speriod, sphase, xloc, yloc, stim_xcent, stim_ycent, tfreq, drift, scale=1,
		  inten_add, inten_mult, contrast, renv, makenv=0, sq,  start, dur, wavel=1, mask=0, stimchan=0);
}
		  ...
		  double contrast, double renv, double start, double dur)
{
   double xcent, ycent, scale;
   double wavel, mask;
   int makenv, sq, stimchan;

   stim_sineann(speriod, sphase, xloc, yloc, xcent=0, ycent=0, tfreq, drift, scale=1,
		  inten_add, inten_mult, contrast, renv, makenv=0, sq=0, start, dur, wavel=1, mask=0, stimchan=0);
}
		  ...
		  double contrast, double renv, double start, double dur)
{
   ...
   stim_sineann(speriod, sphase, xloc, yloc, stim_xcent, stim_ycent, tfreq, drift, scale=1,
		  inten_add=0, inten_mult, contrast, renv, makenv=0, sq=0,  start, dur, wavel=1, mask=0);
}
			...
			double renv, double start, double dur)
{
   ...
   stim_windmill(speriod, sphase, xloc, yloc, xcent=0, ycent=0, tfreq, drift, scale=1, 
		     inten_add=0, inten_mult, contrast, renv, makenv=0, sq=0, start, dur, wavel=1, mask=0, stimchan=0);
}
...
void stim_grating (int type, double speriod, double sphase, double orient,
		...
		double xenv, double yenv, int ssq, int tsq, double mask, int stimchan,
		double start, double dur)
{
     double xcent, ycent;
     double scale;
  ...
  switch (type) {
	case SINE:			/* stim for possibly all photrecs */
	  stim_sine(speriod, sphase, orient, xloc, yloc, xcent=0, ycent=0,
		tfreq, drift, scale=1, inten, contrast, ssq, tsq, start, dur,
	       	wavel, mask, stimchan);
	  break;

	case GABOR:			/* stim for possibly all photrecs */
	    stim_gabor(speriod, sphase, orient, xloc, yloc, xcent=0, ycent=0,
		tfreq, drift, scale=1, inten, contrast, xenv, yenv, ssq, tsq,
		start, dur, wavel, mask, stimchan);
	  break;
	...
	case SINEANN:			/* stim for possibly all photrecs */
	  ...
	  stim_sineann(speriod, sphase, xloc, yloc, xcent=0, ycent=0, 
		tfreq, drift, scale=1, inten, contrast, xenv, ssq,
		start, dur, wavel, mask, stimchan);
	  break;
