...

{
   ...
   for (n=0,lpnt=pnt->clst; lpnt; lpnt=lpnt->next) {  /* find connections */
	...
	case GLY:
		...
		if (nchan=int(((chan *)cpnt)->nchan))
                     ncfprintf (stdout,
			"%s(%d) %-.3g n=%d",
		  findsym(cpnt->ctype),((chan *)(long long)cpnt->stype),cond,nchan);
                else ncfprintf (stdout,
			"%s(%d) %-.3g",
			findsym(cpnt->ctype),((chan *)(long long)cpnt->stype),cond);
                if (((chan *)cpnt)->compe!=NULL) 
			ncfprintf (stdout," compe %-d",((chan *)cpnt)->compe->num);
                if (((chan *)cpnt)->compp!=NULL) 
			ncfprintf (stdout," compe %-d",((chan *)cpnt)->compp->num);
                break;
        case SYNAPSE:
  ...
  if (pnt->capnt) {
		...
		ncfprintf (stdout,
			"Cai comp area %-4.3g, %-d shell%s, dr %g", pnt->capnt->area, nshell,sc,pnt->capnt->dr);
                nprint = testlin(nprint);
