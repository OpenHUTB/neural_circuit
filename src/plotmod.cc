...
int plotnum=-1;
int pcolor=-1;			/* plot color set on command line */
...
extern int plotcolors[PLOTNODSIZ];	/* plot color from command line */
...
extern "C" {
  ...
  // void exit (int __status) __THROW __attribute ((__noreturn__));
  void exit (int status);
#ifdef __cplusplus
}
#endif

int find_color (char *colorstr); 
void plotrun(void);
void initpl(int);
   ...
   if (argc)
    {
     if (*cptr == '-')
	  ...
	  case 'm': 
 	  ...
 	  case 'o': 				/* specify plot colors */
		argv++; argc--;			/* either char or numeric */
		if (isdigit(**argv)) pcolor = atof(*argv);
		else pcolor = find_color(*argv);
		break;
 	  ...
 	  case 'p': 				/* specify indiv. plots */
		...
		if (*strp=='-') {		/*  or specify range: a-b */
		  ...
		  if (pstop < pstart) {		/*  if they're backwards */
		     for (i=pstart; i>=pstop; i--) {
			if (pcolor>=0) plotcolors[i] = pcolor;
			...
			if (pcolor>=0) plotcolors[pstart] = pcolor;
			listplots[numpl++]  = pstart;
		}
		pcolor = -1;		/* revert back to original color */
		break;
...

{
  ...
  ncfprintf (stdout,"        -l x  set x as label, non numeric string or inside \"\".\n"); 
  ncfprintf (stdout,"        -M n  override Ymax.\n"); 
  ncfprintf (stdout,"        -m n  override Ymin.\n"); 
  ncfprintf (stdout,"        -n    no node numbers on plot.\n");
  ncfprintf (stdout,"        -o n  set plot color, either alpha or numeric.\n");
...
const char *color_names[NCOLORS]={"black", "blue", "green", "cyan", "red", "magenta", "brown", "white",
			  "gray", "ltblue", "ltgreen", "ltcyan", "ltred", "ltmag", "yellow", "brtwht", "nocolor"};
...
int find_color (char *colorstr) {
    ...
    for (i=0; i<NCOLORS; i++) {
       if (strncmp(color_names[i],colorstr,NCOLORS)==0) {
	  if (i==16) i=NOCOLOR;
           return i;
       }
    } 
...
void rprintf (char*, double, double, double, double, double, int) {}
