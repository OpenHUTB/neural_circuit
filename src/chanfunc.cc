...
datum call_set2ndmsg (datum &ctype, datum &stype, datum &prect, datum &postct, datum &maxdist, datum &gain, datum &mtype)

/* For every Ca++ channel in postct (SBACs), find nearby synapse and save. */
	/* If cype or stype < 0, any type or subtype matches */
	/* Run this once after first step(). */

{  datum nchans;

  // fprintf (stderr,"ctype %g stype %g prect %g postct %g maxdist %g\n",
  //  		ctype.val, stype.val, prect.val, postct.val, maxdist.val);
  nchans.val = set2ndmsg (ctype.val, stype.val, prect.val, postct.val, maxdist.val, gain.val, mtype.val);
  nchans.vtype = NUMBER;
  return nchans;
}
...
#define MAXPERMITER 50
  ...
  for (ovrev=-1,dvrev=0.01,j=0; j<MAXPERMITER && ncabs(dvrev) > 1e-8; j++) {
...
void set_perms (chantype *chtyp, iontab *ions, double svrev)
 ...
 for (t=1; t<=nfval; t++) {  /* calc tables for alpha, beta rate funcs */ 
  ...
  for (vn=0; vn<=RATESIZ; vn++) {   /* make tables from -250 to 250 mV, */
    ...
    *(npt + vn) = q * (*chanf)(v,t);
      ...
      // ncfprintf (stderr,"func %d V %g q %g rate %g %g\n", t, v, q,*(npt+vn), (*chanf)(v,t),calcca6m(v,t));/* */
...
#endif
//  fprintf (stderr,"chanrate %g\n",cpnt->comp1->v - cpnt->comp1->vph - vext(cpnt) - ccavoff(cpnt) -
