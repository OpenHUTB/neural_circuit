...

{
  ...
  double *caise;			/* calcium internal shell estimates */
  double *caose;			/* calcium external shell estimates */
  ...
  if (pnt->compe!=NULL) {
      ph = getnt(pnt->compe, PH);		   /* get pH from external compartment */
      if (ph==0) ph = 7.4;
...
/* ncfprintf (stderr,"doca: chan %d %d\n",chpnt->ctype,chpnt->stype); /* */
  ...
  // if (pnt->num==2)  ncfprintf (stderr,"ica %g\n",ica);   	/* */
...
void run2ndmsg(chan *chpnt)

/* Modulate channel conductance from 2nd messenger. */
/*   for Glu, activated inversely by glutamate released by synapse */
/*  Pointer to synapse "secmsgmod" must already be set. */

{
	int nf;
	double mod, transrate;
	synap *spnt;

  if ((spnt=chpnt->secmsgmod)==NULL) return;
  if (spnt->filtm) {
        nf = spnt->filtm->nfilt-1;
        transrate = spnt->filtm->lfilt[nf];  /* neurotrans released, from synaptic filter */
  }
  else  transrate = spnt->transrate * spnt->trconc;

  switch (chpnt->secmsgtype) {
      case AMPA:			/* mglur2: modulate chan cond inversely by glutamate released by synapse */
      case GLU: 
      case KAINATE: 
      case NMDA:
        switch (chpnt->ctype) {
	  case CA:
	   transrate -= spnt->secmsgthresh * spnt->trconc;
	   if (transrate < 0) transrate = 0;
	   transrate *= chpnt->secmsggain;
           mod = 1.0 - transrate / (transrate + spnt->trconc);
//           mod = spnt->trconc / transrate;
//           mod = 1.0 - transrate / (transrate + spnt->trconc);
//           mod = 1.0 - pow(transrate / spnt->trconc,3);
	   if (mod < 0) mod = 0;
 	   break;
     ...
     case GABA:		/* GABA-b modulates channel conductance by GABA released by synapse */
       switch (chpnt->ctype) {

 	 case CA:		/* GABA-b deactivates Ca channels */ 
	   transrate *= chpnt->secmsggain;	
           mod = 1.0 - transrate / (transrate + spnt->trconc);
	   break;
           	
	 case K:		/* GABA-b activates K channels */ 
           mod = transrate / (transrate + spnt->trconc) * chpnt->secmsggain;	
	   break;
 ...
 //  fprintf (stderr,"chan %d %d secmsg %d mod %g %g %g %x\n",
 //  chpnt->ctype,chpnt->stype,chpnt->secmsgtype,mod,transrate,spnt->trconc,spnt);
...
void doverr(comp *pnt, int noskip, double critxx);
...
// implfk = .5;
 ...
 for (pnt=compnt; pnt; pnt=pnt->next) {
   ...
   pnt->extv_old = 0;	        /* save previous voltage clamp voltage */
 }
...
/* */
 ...
 for (pnt=compnt; pnt; pnt=pnt->next)                   /* First estimate */
  {                                                     /* for all comps */
   ...
   if (pnt->compe!=NULL) pnt->vext = pnt->compe->v; /* set external voltage for cacomp */
   else                  pnt->vext = 0;
     ...
     if (!ocpntf)
      ...
      {                         /* add connection conductances that change */
       ...
       case SYNAPSE:					 /* cur thru syn chan */
		if (!(((synap*)conpnt)->resp1 || ((synap*)conpnt)->resp2)) { /* regular synapse, no postsyn chan */
		  if (pnt==((synap *)conpnt)->comp2) {
                    nodcr += ((synap *)conpnt)->conduct * (((synap *)conpnt)->vrev-compcavoff(pnt));
          	    nodcur -= ((synap *)conpnt)->conduct * (pnt->v - vext(conpnt,0));  /* current due to Vm */
      ...
      case CONE:                                /* find photrec instantan */
          ...
          nodcur -= ((photrec *)conpnt)->conduct * (pnt->v - vext(conpnt,0)); 	/* current due to Vm */
...
/*   ncfprintf (stderr,"vrev %10.3g conduct %10.3g\n",
      ...
      case SYN2:
		...
		chpnt->conduct = conduct;
                ...
                nodcur -= conduct * (pnt->v - vext(conpnt,0)); 		/* current due to Vm */
                tcondn += conduct;    			/* total cond */
		break;
      case CHRC:
		chpnt = (chan *)conpnt;
      ...
      case K:   
		...
		default:
	 	chpnt = (chan *)conpnt;
  		...
  		conduct *= chpnt->maxcond * get_gfrac(chpnt,pnt->v);
		chpnt->conduct = conduct;
		if (chpnt->secmsgmod!=NULL) run2ndmsg(chpnt);		/* run mGluR2 mod */
                nodcr += conduct * (get_gvrev(chpnt,pnt->v) - compcavoff(pnt));
                nodcur -= conduct * (pnt->v - vext(conpnt,0));  			/* current due to Vm */
                tcondn += conduct;    		/* total cond */
		break;
      ...
      case KCa:   
		...
		if (pnt->capnt) {  /* calcium compartment exists */
		  if (!kcapnt->initfl){	/* Calcium not set up before K chan. */
			...
			double alph,bet;
		    ca = pnt->capnt->cais[0];
		    alph = akcacalc((pnt->v-vext(conpnt,0)-ccavoff(kcapnt)-kcapnt->voffsm),ca,
			kcapnt->arate,kcapnt->d1,kcapnt->k1);
		    bet = bkcacalc((pnt->v-vext(conpnt,0)-ccavoff(kcapnt)-kcapnt->voffsm),ca,
			kcapnt->arate,kcapnt->d2,kcapnt->k2);
		    kcapnt->m = alph / (alph + bet);
		    kcapnt->initfl = 1;
		    ocais = ca;
		  }
 		  kcatab((pnt->v-vext(conpnt,0)-ccavoff(kcapnt)-kcapnt->voffsm),    /* set K rate chtyp */ 
		     (ocais+pnt->capnt->cais[0])*.5,stype,kcapnt->arate,
		     kcapnt->d1,kcapnt->d2,kcapnt->k1,kcapnt->k2,chp);
		}
		else {		/* Can't find ca comp for kca. */
				/* Use default cai */
     		  kcatab((pnt->v-vext(kcapnt,0)-ccavoff(kcapnt)-kcapnt->voffsm),    /* set K rate chtyp */ 
...
/* ncfprintf (stderr,"conduct %g ipump %g\n",conduct,capnt->ipump); /* */
   ...
   //
   // if (pnt->compe!=NULL) nodcr += (pnt->vext - pnt->oldvext) * pnt->cap / timinc; /* capacitive current from vext */
