...
void stsrchfit (double (*user_func) (double user_x_point, double *coeff, int model_num),
                int m_dat, double *xydata, int nparms, double *coeff, double *coeffc);
...
void run_model (double *coeff, int nparms, int n) 
...

{
  ...
  //  fprintf (stderr,"x %18.18g c1 %18.18g\n",x, coeff[0]);
      ...
      if (waitsecs>0) usleep(int(waitsecs*1e6));				// if run multi, wait
}

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

#define DELWAIT 900   // 30 minutes with 2 sec wait
#define AVG_MULT 50
static  int delwait = DELWAIT;

int calc_delwait(int npoll)

/* The problem here is that sometimes the retsim process stops 
     unexpectedly (because of some problem with mosrun).  
     If we have to wait several hours to find this out, it isn't 
     practical. But if we set an arbitrary shorter value for 
     the number of polls before stopping, it may be too short 
     for a model that just happens to take longer,
     i.e. because the model has many more compartments.

   So we make an average value and stop after a proportionately
     longer number.
*/

{
     static double delwait_sum = 0;
     static int deln = 0;
     int avg_delwait;

  deln++;
  delwait_sum += npoll;
  avg_delwait  = delwait_sum / deln * AVG_MULT; 
  return (avg_delwait); 
}
...

{
   ...
   for (i=0,retval=0;i<delwait && retval==0;i++) {		// poll for all models done
      if (strstr(xsystem(tailcmd),"done")!=NULL) retval = 1;
      if (retval==0) simwait (2);	// if not ready, sleep for 2 seconds before polling again
   }
   delwait = calc_delwait(i);
   if (retval==0) fprintf (stdout,"dw %d\n",delwait);
   return retval;
}
...
{
      int i, w, xval, nrows, ncols, addr_val;
  ...
  w = 1;
  if (n<0) {
      run_model(coeff,nparms,-n);		// call with negative n starts the models running
      return 0;
  }
  if (xval==0) {
     if (n==0) {
       run_model(coeff,nparms,n);		// call with zero n starts model running, don't wait
     }
     else  w = wait_for_model_data(n);  // n > 0
     if (w==0) {
         if (model_data_arr !=NULL) {
	     efree (model_data_arr); // remove data array so this one is skipped
	     model_data_arr = NULL;
	 }
         return 0;
     }
...
int main (int argc, char **argv) 
{
  ...
  */
  ...
  if (notinit (fit_type))           fit_type = 1;
  ...
  if (notinit(waitsecs)) waitsecs = 0.2;
  ...
  sprintf (data_split_string,data_split,model_output_file, plotn, datasize, model_output_file);
...
#undef UC
  ...
  if (fit_type==1) lmfit (get_model_data, datasize, xydata, nparms, coeff, coeffc);
  else         stsrchfit (get_model_data, datasize, xydata, nparms, coeff, coeffc);
}
