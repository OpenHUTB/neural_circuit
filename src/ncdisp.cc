...
double *rcolors=NULL;			/* pointer to density file row for colors */
...
void callfunc10(Symbol *funcp, int npar, double par1, double par2, 
			double par3, double par4, double par5, 
			double par6, double par7, double par8,
			double par9, double par10);
...
int get_ecolor(elem *epnt, int ecolor, double vmax, double vmin, int cmap);
...
Symbol *rayphot_drpnt = NULL;
void (*rphd)(int,double,double,double,double,double,double,double,int) = NULL;
Symbol *raysyn_drpnt = NULL;
void (*rsyd)(synapse*,double,double,double,double,double,double,double,int) = NULL;
Symbol *raygapj_drpnt = NULL;
void (*rgjd)(double,double,double,double,double,double,double,int) = NULL;

void rayphot_draw (int ctype, double dia, double xr, double yr, double zr,
		                double tx1, double ty1, double tz1, int color);
void rayphot_dr   (int type, double dia, double xr, double yr, double zr, 
		                        double tx1, double ty1, double tz1,  int color);
void raysyn_draw (synapse *spnt, double dia, double tx1, double ty1, double tz1, 
			                     double tx2, double ty2, double tz2, int color);
void raysyn_dr (synapse *spnt, double dia, double tx1, double ty1, double tz1, 
			                   double tx2, double ty2, double tz2,  int color);
void raygapj_draw(double dscale, double tx1, double ty1, double tz1,
	                         double tx2, double ty2, double tz2, int color);
void raygapj_dr  (double dscale, double tx1, double ty1, double tz1,  
			         double tx2, double ty2, double tz2, int color);
...
void ncdrnod (int n1a, int n1b, int n1c, int n1d, 
	...
	double zrange1, double zrange2,
	int dcolor, Symbol *vpen, double(*vpenn)(int elnum, int color),
	double dscale, int cmap, int drlabel)
...
/* -  -  -  -  -  -  -  -  -  -  -  -  - */
 ...
 if (n1a >= 0 && 
  ...
  if ((debug & NCDISP) && (debugz & 2))
    ...
    if (checknodez(npnt,zrange1,zrange2)) continue;
    if (drlabel) {
	    if (npnt->label==NULLVAL) continue;		// skip if not label 
            if (dcolor==RCOLOR) color = int(rcolors[npnt->region]);
	    else color = dcolor;
	    if (color==VCOLOR)  color = npnt->label;
	    if (color==BLACK) color = GCOL1;
    } 
    else color = dcolor;
...
CCOL11, CCOL12, CCOL13, CCOL14, CCOL15, CCOL16, CCOL17, CCOL18, CCOL19, CCOL20, 
CCOL21, CCOL22, CCOL23, CCOL24, CCOL25, CCOL26, CCOL27, CCOL28, CCOL29, CCOL30, 
CCOL31, CCOL32, CCOL33, CCOL34, CCOL35, CCOL36, CCOL37, CCOL38, CCOL39, CCOL40, 
CCOL41, CCOL42, CCOL43, CCOL44, CCOL45, CCOL46, CCOL47, CCOL48, CCOL49, CCOL50, 
CCOL51, CCOL52, CCOL53, CCOL54, CCOL55, CCOL56, CCOL57, CCOL58, CCOL59, CCOL60, 
CCOL61, CCOL62, CCOL63, CCOL64, CCOL65, CCOL66, CCOL67, CCOL68, CCOL69, CCOL70, 
CCOL71, CCOL72, CCOL73, CCOL74, CCOL75, CCOL76, CCOL77, CCOL78, CCOL79, CCOL80, 
CCOL81, CCOL82, CCOL83, CCOL84, CCOL85, CCOL86, CCOL87, CCOL88, CCOL89, CCOL90, 
CCOL91, CCOL92, CCOL93, CCOL94, CCOL95, CCOL96, CCOL97, CCOL98, CCOL99, CCOL100,
CCOL101,CCOL102,CCOL103,CCOL104,CCOL105,CCOL106,CCOL107,CCOL108,CCOL109, CCOL110,
CCOL111,CCOL112,CCOL113,CCOL114,CCOL115,CCOL116,CCOL117};
...

{
   ...
   colormaps[numcmap].ncolors=cmapsize;
   colormaps[numcmap].lut=new_cmap;
   numcmap++;
   return numcmap;
}
   ...
   if ((rspnt->time >= start) && (rspnt->time <stop)) {
      ...
      switch (rspnt->ctype) {
       ...
       case 'a':                /* absolute additive */
	     ...
	     rpnt->chanw[rspnt->stimchan] = 1.0;

	  }
#ifdef DEBUG
          if (debug & NCDISP && debugz & 1)  ncfprintf (stderr,"abs stim %d %g %g %d %g\n",
                                        rspnt->recnm1,rspnt->val,rspnt->wavel,rspnt->stimchan,rspnt->time);
#endif
         break;
     ...
     case 'b':                /* delta masking */
	    ...
	    rpnt->chanw[rspnt->stimchan] = 1.0;
          ...
          if (debug & NCDISP && debugz & 1)  ncfprintf (stderr,"del mask stim %d %g %g %g %g\n",
					...
					rpnt->mflux[rspnt->stimchan],rspnt->mask);
#endif
         break;
...
#ifdef DEBUG
          if (debug & NCDISP && debugz & 1)  ncfprintf (stderr,"del stim %d %d %g %g %d %g %g\n",
                                        rspnt->recnm1,rspnt->recnm2,rspnt->val,rspnt->wavel,rspnt->stimchan,
								rpnt->aflux[rspnt->stimchan],rspnt->time);
#endif
         break;
      } /* switch */	
 ...
 if (max_flux == -LARGENODE) { /* If max_flux is not specified, */
   ...
   for (n=0,flux_avg=0,max_flux=-LARGENODE,min_flux=LARGENODE,rpnt=recpnt; 
	...
	for (i=0; i<numstimchan; i++) {		/* add up all disp channels */
	   ...
	   if (mask>1) mask = 1;
	   if (mask<0) mask = 0;
   	   if (rpnt->mflux[i] <= unmaskthr)  mask = 0;       /* neg intensity masking */
   	   if (rpnt->mflux[i] >= unmaskthr2) mask = 0;       /* pos intensity masking (i.e. Vm) */
   	   flux += rpnt->chanw[i] * rpnt->aflux[i]*(1-mask) + rpnt->mflux[i]*mask; /* masking */
	}
   	flux_val = flux;
  ...
  cmap--;
  if (cmap<0) cmap = 0;
  if (cmap>=numcmap) cmap = numcmap-1;
  if (cmap==0) cmap=7;
...

{
  ...
  if (dcolor==NOCOLOR) color = dcolor;
  else {
...
/* find the color of an element from its cell region in the density file. */

{
  ...
  if (rcolors != NULL) {
    ...
    color = get_ecolor(epnt,color,rmax,rmin,cmap);
  }
  cmap--;
  if (cmap<0) cmap = 0;
  if (cmap>=numcmap) cmap = numcmap-1;
  ...
  if (color!=NOCOLOR) color = color_lut[color];
  return color;
}

/*------------------------------------*/

int disprncolor (elem *epnt, int dcolor, double rmax, double rmin, int cmap)

/* find the color of an element from the number of its cell region. */
...
int get_ecolor(elem *epnt, int ecolor, double vmax, double vmin, int cmap)
{
  ...
  switch (ecolor) {
	 case   VCOLOR: dcolor = dispvcolor  (epnt,ecolor,vmax,vmin,cmap); break;
         ...
         case  RNCOLOR: dcolor = disprncolor (epnt,ecolor,vmax,vmin,cmap); break;
	 case  SRCOLOR:
	 case  SGCOLOR:
	 case NAICOLOR:
	 case NAACOLOR: dcolor = dispccolor  (epnt,ecolor,vmax,vmin,cmap); break;
	 case NOCOLOR:  dcolor = ecolor; break;
...

{
  ...
  dcolor = get_ecolor(epnt, ecolor, vmax, vmin, cmap);   /* set color from voltage or other parameters */

  if (vpen) {			     /* set color from vpen() interpreter function */
     dcolor = int(callfunc (vpen,2,(double)epnt->elnum,(double)ecolor,0.0,0.0));
  } 
  if (vpenn) {			    /* set color from vpen compiled function */
		...
		double length, double foreshorten, int hide)
{
  ...
  if (length > 1e-3) {
     ...
     // if (dia > 0.001) gcirc (dia/2.0,fill);		// draw circle
     // else             gcirc (0.001,fill);
     if (dia > 0.001) gtri (-tsiz+len, tsiz,tsiz+len, 0, -tsiz+len,-tsiz, fill); // draw triangle
     else             gcirc (0.001,fill);
     gmove (0,0);
     gdraw (length,0);
  }
...
void drcalib(double x, double y, double len, double size, int color)
{
  ...
  if (disp_ray) {
	...
	if      (color==WHITE) color = BLACK;
     ...
     raycalib (x, y-.03, len, size, color);
  }
  else {
   dist = len * xyscal;
   if (color!=NULLVAL) gpen(color);
...

{
  ...
  if (rphd==NULL) {
    if ((f=lookup("rayphot_dr"))==NULL) {
	rphd = rayphot_draw;
    }
    else {
	rayphot_drpnt = f;	/* set pointer to user function */
	rphd = rayphot_dr;      /* set pointer to call user function */
    }
  }
  if (rsyd==NULL) {
    if ((f=lookup("raysynapse_dr"))==NULL) 
	rsyd = raysyn_draw;
    else {
	raysyn_drpnt = f;	/* set pointer to user function */
	rsyd = raysyn_dr;      /* set pointer to call user function */
    }
  }
  if (rgjd==NULL) {
    if ((f=lookup("raygapj_dr"))==NULL) 
	rgjd = raygapj_draw;
    else {
	raygapj_drpnt = f;	/* set pointer to user function */
	rgjd = raygapj_dr;      /* set pointer to call user function */
    }
  }
...

{
 ...
 callfunc8
(elec_drpnt,7,(double)color,dscale,dia,length,foreshorten,(double)hide,(double)elnum,x);
}
...
void set_rayphot_dr (void (*rayphot_drpnt)(int,double,double,double,double,double,double,double,int))
{
  if (rayphot_drpnt) rphd = rayphot_drpnt;
}
...
/* Call a user-defined subroutine for drawing a povray gap junction icon. */

{
   double x=0;

   callfunc10 (rayphot_drpnt,9,(double)type, dia, xr, yr, zr, 
		   	       tx1, ty1, tz1, (double)color, x);
}

/*--------------------------------------*/

void set_raysyn_dr (void (*raysyn_drpnt)(synapse*,double,double,double,double,double,double,double,int))
{
  if (raysyn_drpnt) rsyd = raysyn_drpnt;
}
...
{
   callfunc8 (raysyn_drpnt,8,dia, tx1, ty1, tz1, tx2, ty2, tz2, (double)color);
}

/*--------------------------------------*/

void set_raygapj_dr (void (*raygapj_drpnt)(double,double,double,double, double,double,double,int))
{
  if (raygapj_drpnt) rgjd = raygapj_drpnt;
}
...
{
   callfunc8 (raygapj_drpnt,8, dscale, tx1, ty1, tz1, tx2, ty2, tz2, (double)color);
}
