...

{
  ...
  chparms = fread(chparmfil, &nrows, &ncols);  /* read chanparams file into temp arr */
...
void celseg_init(void)
{
	...
	for (n=0; n<nceltypes; n++) {	     /* set up ndens and cellnums to be indexed by [ct][cn] */
	  if (getn(n,MAKE)>0) {
		...
		celnode[n] = (int *)emalloc(ncells*sizeof(int)); 
	  } else {
		ndens[n] = NULL;
		cellnums[n] = NULL;
		celnode[n] = NULL;
	...
	_K8  0e-3	0e-3	0e-3	0e-3	0e-3	0e-3	0e-3    0e-3   0e-3   # KIh
	_K9  0e-3	0e-3	0e-3	0e-3	0e-3	0e-3	0e-3    0e-3   0e-3   # KIh
	_K10  0e-3	0e-3	0e-3	0e-3	0e-3	0e-3	0e-3    0e-3   0e-3   # KIh
	_K11  0e-3	0e-3	0e-3	0e-3	0e-3	0e-3	0e-3    0e-3   0e-3   # Kv3.1b
...
*/
	...
	if (notinit(dendm_dia_factor))
	  dendm_dia_factor = 1; 	/* dia factor for medial dends */
...

{
 ...
 /* possibly block channels */
  ...
  for (region=0; region<R_NREGIONS; region ++) {
    ...
    celdens[ct][dn][C_K11][region] *= (1 - tea);

    celdens[ct][dn][C_K0][region] *= (1 - cesium);
    celdens[ct][dn][C_K1][region] *= (1 - cesium);
    celdens[ct][dn][C_K2][region] *= (1 - cesium);
    celdens[ct][dn][C_K3][region] *= (1 - cesium);			// KA
    celdens[ct][dn][C_K6][region] *= (1 - cesium);
    celdens[ct][dn][C_K7][region] *= (1 - cesium);
    celdens[ct][dn][C_K11][region] *= (1 - cesium);
...
void addcadens (int ct, int cn, int region, cattrib *ca, int nshells)
{
      double pkm, bmax, bkd, btot, btoti, cashell, cadia;
   ...
   if ((cashell = celdens[ct][ndens[ct][cn]][C_CASHELL][region]) <=0) cashell = nshells;
   ca->cashell = cashell;

   if ((cadia = celdens[ct][ndens[ct][cn]][C_CADIA][region]) >0) ca->cadia = cadia;
	...
	if (ndensity > 0 || e->attpnt==NULL) {
	   ...
	   if (makeCattribs && (chanType == CA)) {				// don't use, makeCattribs = F
		...
		cattrib* ca = (cattrib*) a;
                addcadens (ct,cn,region,ca,nshell);
        }
     }
...
void addChannelsToSphere(int ct, int cn, int nod1, int region, double somadist)
{
	...
	e = at(npnt, CHAN);			/* Ih type 8*/
	addChannelToElem(ct, cn, region, e, K, 8, C_K8, k8nois, somadist, true);	
	e = at(npnt, CHAN);			/* IH type 9*/
	addChannelToElem(ct, cn, region, e, K, 9, C_K9, k9nois, somadist, true);	
	e = at(npnt, CHAN);			/* Ih type 10*/
	addChannelToElem(ct, cn, region, e, K, 10, C_K10, k10nois, somadist, true);	
	e = at(npnt, CHAN);			/* Kv3.1b, K type 11 */
	addChannelToElem(ct, cn, region, e, K, 11, C_K11, k11nois, somadist, true);	
	e = at(npnt, CHAN);			/* KCa */
	...
	// always make Cacomp if the density file sets it
	
	  if (celdens[ct][ndens[ct][cn]][C_CAPUMP][region] > 0 ||
	      celdens[ct][ndens[ct][cn]][C_CABTOT][region] > 0 ||
	      celdens[ct][ndens[ct][cn]][C_CAEXCH][region] > 0)
	  {
			  
	  e = at(npnt, CACOMP);		
	  cattrib* ca = (cattrib *) make_chan(e, CACOMP, 0);
          addcadens (ct,cn,region,ca,nshell_soma);
      }
}
...
void addChannelsToCable(int ct, int cn, elem* e, int region, double somadist)
{
	...
	addChannelToElem(ct, cn, region, e, K,  8, C_K8, k8nois, somadist, false);
	addChannelToElem(ct, cn, region, e, K,  9, C_K9, k9nois, somadist, false);
	addChannelToElem(ct, cn, region, e, K,  10, C_K10, k10nois, somadist, false);
	addChannelToElem(ct, cn, region, e, K,  11, C_K11, k11nois, somadist, false);
		 ...
		 int nod2, double cd1, double cd2, int region, double cplam)
{
  ...
  if (ninfo>=5) fprintf(stderr,"# Make_celseg: nod1 %d nod2 %d reg %d cd1 %g cd2 %g cplam %g\n", 
		   nod1, nod2, region, cd1, cd2, cplam);
...
*/
  ...
  else if (getn(ct,BIOPHYS)>0) {
	  local_cplam = celdens[ct][ndens[ct][cn]][C_CPLAM][region];		/* look in dens_xxx.n for cplam */
  }
  else local_cplam = getn(ct,COMPLAM); 						/* otherwise, use nval_xxx.n */ 
  ...
  if (region==DEND || region==DEND_PROX || region==DEND_MED || region==DEND_DIST) {
     local_dia *= dend_dia_factor;
  }

  if (region==DEND_MED)  {
     local_dia *= dendm_dia_factor;
  }
  ...
  if (ninfo >=5) fprintf(stderr, "# Make_celseg cd1 %g cd2 %g local_dia %g\n", cd1, cd2, local_dia);
...
/*  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  */
  ...
  if (getn(ct,BIOPHYS)==0) {		/* no channels, passive membrane */
    ...
    if (nod1==nod2){
      ...
      s->vrest = vstart;
      s->vrev = cvrev;
      s->elabl = (char *)regname[region-1];
      s->region = region;
      s->nodp1->region = region;
    }
      //at [ct][cn][nod1] sphere dia=cd1 vrest=vstart vrev=cvrev rm=crm
				//elabl regname[region-1]
    else {
      if (region!=PSOMA) {		/* if "point soma", then don't make cable */
        cable *c = (cable *)make_conn(ct, cn, nod1, ct, cn, nod2, CABLE);
        c->cplam = local_cplam;
        c->vrest = vstart;
        c->vrev = cvrev;
        c->Rm = crm;
        c->Ri = cri;
        c->elabl = (char *)regname[region-1];
        c->region = region;
        c->dia = cd1;
        c->dia2 = cd2;
        c->nodp1->region = region;
        c->nodp2->region = region;
      }
    }
  }
  else {
    ...
    if (ninfo >=5) {
      fprintf(stderr, "Make_celseg %s biophys %g region %s node %d %d %d ",
                      cname[ct], getn(ct,BIOPHYS), regname[region-1], ct, cn, nod1);
   ...
   /* if radial gradient set, find dist to soma */
    ...
    if (nod1==nod2) {
      ...
      if (ninfo >=5) {
        fprintf(stderr, "Make_celseg region is cable; make_conn nod1 %d %d %d to nod2 %d %d %d\n",
