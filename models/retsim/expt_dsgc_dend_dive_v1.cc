   ...
   switch (setscene) {
	default:
 	...
 	case 5: dend_layers = 1; space_time = 0; 
		...
		if (!notinit(stim_loc)) {sprintf (tbuf2,"Off layer, loc %d",stim_loc); dend_text1 = tbuf2;}
...
/* orig, for dens_dsgc_morph_ww.n */
/*  
  if (notinit(stim1)) stim1 = 2060;		// R2, red
  if (notinit(stim2)) stim2 = 4440;		// R2, red
  if (notinit(stim3)) stim3 = 2330;		// R5, green
  if (notinit(stim4)) stim4 = 4250;
  // if (notinit(stim5)) stim5 = 5980;

  if (!notinit(stim_loc)) stim5 = stim_loc;

  if (notinit(rec1)) rec1 = 490;		// R1, blue
  if (notinit(rec2)) rec2 = 1860;
  if (notinit(rec3)) rec3 = 4015;
  if (notinit(rec4)) rec4 = 2809;
/* */

  if (notinit(stim1)) stim1 = 3940;		// R2, red
  if (notinit(stim2)) stim2 = 5950;		// R2, red
  if (notinit(stim3)) stim3 = 6154;		// R5, green
  if (notinit(stim4)) stim4 = 3626;
  // if (notinit(stim5)) stim5 = 7610;

  if (!notinit(stim_loc)) stim5 = stim_loc;

  if (notinit(rec1)) rec1 = 800;		// R1, blue
  if (notinit(rec2)) rec2 = 2802;
  if (notinit(rec3)) rec3 = 4320;
  if (notinit(rec4)) rec4 = 3536;

  // if (!notinit(stim1)) label(nde(c,1,dendn_node(c,201)),yellow," -S1");  // label column 201
 
  // if (make_movie && setscene!=4) {
  if (setscene!=4) {
      if (!notinit(stim1)) label(nde(c,1,stim1),red,    " -Off_dive1");
      if (!notinit(stim2)) label(nde(c,1,stim2),magenta," -Off_dive2");
      if (!notinit(stim2)) label(nde(c,1,stim3),green,  " -Off3");
      if (!notinit(stim3)) label(nde(c,1,stim4),ltgreen," -Off4");
  }
  // if (!notinit(stim3)) label(nde(c,1,stim3),white," -S3");
  // if (!notinit(stim4)) label(nde(c,1,stim4),white," -S4");

  // if (make_movie && setscene!=3) {
  if (setscene!=3) {
...
//   };
   ...
   if (istim != 0) {
	  double totcur;
       ...
       t = 0;
       if (!notinit(stim_loc)) {
          if (!notinit(stim_loc)) cclamp(ndn(dsgc,1,stim_loc),   istim, start=stimtime+t*sinc, sdur);
       } else {
   ...
   if (!noplots) {
     ...
     */
     ...
     plot_v_nod(ct=dsgc,cn=1,stim5,Vmin,Vmax,c=green,  "Stim_loc", pl=10, 1.0*plsiz);
     ...
     plot_v_nod(ct=dsgc,cn=1,rec1,Vmin,Vmax,c=blue,    "On1", pl=10, 1.0*plsiz);
     plot_v_nod(ct=dsgc,cn=1,rec2,Vmin,Vmax,c=ltblue,  "On2", pl=10, 1.0*plsiz);
     plot_v_nod(ct=dsgc,cn=1,rec3,Vmin,Vmax,c=cyan,    "On3", pl=10, 1.0*plsiz);
     plot_v_nod(ct=dsgc,cn=1,rec4,Vmin,Vmax,c=ltcyan,  "On4", pl=10, 1.0*plsiz);
