...
void defparams(void) { 
  ...
  if (notinit(g_am_dbp1))  g_am_dbp1  = 4e-10; 	/* am cond, feedback to dbp1, nval_cbp_chirp.n */
  if (notinit(g_am_dbp2))  g_am_dbp2  = 4e-10; 	/* am cond, feedback to dbp2, nval_cbp_chirp.n */
  if (notinit(g_am2_dbp1)) g_am2_dbp1 = 4e-10; 	/* am2 cond, feedback to dbp1, nval_cbp_chirp.n */
  if (notinit(g_am2_dbp2)) g_am2_dbp2 = 4e-10; 	/* am2 cond, feedback to dbp2, nval_cbp_chirp.n */
  if (notinit(g_dbp1_am))  g_dbp1_am  = 5e-10;	/* dbp1 cond, output to am, nval_cbp_chirp.n */
  if (notinit(g_dbp1_am2)) g_dbp1_am2 = 5e-10; 	/* dbp1 cond, output to am2, nval_cbp_chirp.n */
  if (notinit(g_dbp2_am))  g_dbp2_am  = 5e-10; 	/* dbp2 cond, output to am, nval_cbp_chirp.n */
  if (notinit(g_dbp2_am2)) g_dbp2_am2 = 5e-10; 	/* dbp2 cond, output to am2, nval_cbp_chirp.n */
  if (notinit(presyn1)) presyn1 = 0;  		/* presynaptic locus for synaptic output (label in nval.n) */
  if (notinit(presyn2)) presyn2 = 0;  		/* presynaptic locus for dbp2 synaptic output (label in nval.n) */
  if (notinit(presyn3)) presyn3 = 0;  		/* presynaptic locus for dbp1 2nd synaptic output (label in nval.n) */
  // if (notinit(recpnt1)) recpnt1 = 0; 	 	/* recording point (label) */
  // if (notinit(recpnt2)) recpnt2 = 0; 	 	/* recording point (label) */
  if (notinit(dbp1_gtau)) dbp1_gtau = 1; 	/* tau multiplier for GABA channel in dbp1 */
  if (notinit(dbp2_gtau)) dbp2_gtau = 1; 	/* tau multiplier for GABA channel in dbp2 */
  if (notinit(am_atau))     am_atau  = 1; 	/* tau multiplier for AMPA channel in am */
  if (notinit(am2_atau))    am2_atau = 1; 	/* tau multiplier for AMPA channel in am2 */
  if (notinit(am_sdur))     am_sdur  = 2; 	/* sdur for am->dbp1 synapse */
  if (notinit(am_sdur2))    am_sdur2 = 2; 	/* sdur for am->dbp2 synapse */
  if (notinit(am2_sdur))    am2_sdur = 2; 	/* sdur for am2->dbp1 synapse */
  if (notinit(am2_sdur2))   am2_sdur2 = 2; 	/* sdur for am2->dbp2 synapse */
  if (notinit(am_sfall))    am_sfall  = 0; 	/* fall time const for am->dbp1 synapse */
  if (notinit(am_sfall2))   am_sfall2 = 0; 	/* fall time const for am->dbp2 synapse */
  if (notinit(am2_sfall))   am2_sfall = 0; 	/* fall time const for am2->dbp1 synapse */
  if (notinit(am2_sfall2))  am2_sfall2 = 0; 	/* fall time const for am2->dbp2 synapse */
  if (notinit(ampar))      ampar  = xampa5; 	/* AMPA response for dbp1->am synapse */
  if (notinit(ampar2))     ampar2 = xampa5; 	/* AMPA response for dbp2->am synapse */
...
int dbp1row (int ncells, double spacing, double rowtheta, double zrot, double xloc, double yloc, int center, int i, int s)
{
      double dist;		// radial distance from (xloc, yloc)

    return makrow (ncells, spacing, rowtheta, xloc, yloc, zrot, dbp1xarr, dbp1yarr, dbp1tharr, center, i, s);
}

/* - - - - - - - - - - - - - - - - - - - */

int dbp1row (int ncells, double spacing, double rowtheta, double zrot, int center, int i, int s)
{

    return makrow (ncells, spacing, rowtheta, 0, 0, zrot, dbp1xarr, dbp1yarr, dbp1tharr, center, i, s);
}

/* - - - - - - - - - - - - - - - - - - - */

int dbp2row (int ncells, double spacing, double rowtheta, double zrot, double xloc, double yloc, int center, int i, int s)
{
      double dist;		// radial distance from (xloc, yloc)

    return makrow (ncells, spacing, rowtheta, xloc, yloc, zrot, dbp2xarr, dbp2yarr, dbp2tharr, center, i,s);
}

/* - - - - - - - - - - - - - - - - - - - */

int dbp2row (int ncells, double spacing, double rowtheta, double zrot, int center, int i, int s)
{

    return makrow (ncells, spacing, rowtheta, 0, 0, zrot, dbp2xarr, dbp2yarr, dbp2tharr, center, i,s);
}

/* - - - - - - - - - - - - - - - - - - - */

int dbp1cent (int ncells, double theta, double zrot, int center, int i)
{
      int s;			// start
      double spacing;		// cell spacing along row (here, none)

    return makrow (ncells, spacing=0, theta, 0, 0, zrot, dbp1xarr, dbp1yarr, dbp1tharr, center, i, s=0);
}
/* - - - - - - - - - - - - - - - - - - - */

int dbp2cent (int ncells, double theta, double zrot, int center, int i)
{
      int s;			// start
      double spacing;		// cell spacing along row (here, none)

    return makrow (ncells, spacing=0, theta, 0, 0, zrot, dbp2xarr, dbp2yarr, dbp2tharr, center, i, s=0);
}
/* - - - - - - - - - - - - - - - - - - - */
...
#define AMARR  20
  ...
  dbp1cent (1,      theta=  0,   dbp1_zrot,   cent, i=0);
  ...
  switch (amarr) {						/* set up dbp1s */
	...
	break;
	 ...
	 i = dbp1row (dbp1_am, spacing, theta=  180, dbp1_zrot, cent,  i=1,s=0);
	break; 
	...
	break;
     ...
     case 11:	
	  ...
	  i = dbp1row (n_dbp1, spacing, theta=  0,   dbp1_zrot, cent,  i=1,s=1);
	  break;
 
     case 21:

	  if (n_dbp1 < 0 || n_dbp1 > 2*dbp1_am) n_dbp1 = 2*dbp1_am - 1; 	  /* limit to 21 dbp1s */
	  // i = dbp1cent (1,                  theta=  45, dbp1_zrot,  cent, i=0);	  /* set cell rot near both ams */
	  i = dbp1row (dbp1_am, spacing, theta=  90, dbp1_zrot,  cent, i=1,s=1);
	  i = dbp1row (dbp1_am, spacing, theta=   0, dbp1_zrot,  cent, i,  s=1);
	  setn(dbp1,MAXSDIST,15); 
	  break;
 
     case 22: 									/* use with dbp1, dbp2 */
	  if (n_dbp1 < 0 || n_dbp1 > dbp1_am) n_dbp1 = dbp1_am;			/* limit to 11 dbp1s */
	  i = dbp1cent (1,      theta=  0,   dbp1_zrot+315,         cent, i=0);
	  // i = dbp1row (dbp1_am, spacing, theta= 0, dbp1_zrot+55, cent, i=1,s=1);
     ...
     case 3:
	  ...
	  i = dbp1cent (1,                  theta=  45, dbp1_zrot, cent, i=0);  /* set cell rot near both ams */
	  i = dbp1row (dbp1_am, spacing, theta=  0,  dbp1_zrot, cent, i=1,s=1);
	  i = dbp1row (dbp1_am, spacing, theta=  60, dbp1_zrot, cent, i,  s=1);
	  i = dbp1row (dbp1_am, spacing, theta= -60, dbp1_zrot, cent, i,  s=1);
     ...
     case 4:
	  ...
	  i = dbp1cent (1,                  theta=  45, dbp1_zrot, cent, i=0);  /* set cell rot near both ams */
	  i = dbp1row (dbp1_am, spacing, theta=   0, dbp1_zrot, cent, i=1,s=1);
	  i = dbp1row (dbp1_am, spacing, theta=  45, dbp1_zrot, cent, i, s=1);
	  i = dbp1row (dbp1_am, spacing, theta= -45, dbp1_zrot, cent, i, s=1);
	  i = dbp1row (dbp1_am, spacing, theta=  90, dbp1_zrot, cent, i, s=1);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (amarr > 0) {
     ...
     // dbp2cent (1,      theta=  0,   dbp2_zrot,   cent, i=0);
  }
  ...
  if (n_dbp2 > 0) {
    switch (amarr) {						/* set up dbp2s */
     ...
     case 1:
	  ...
	  i = dbp2row (dbp2_am, spacing, theta=  180, dbp2_zrot, cent,  i=1,s=0);
	  break;

     case 11:
	  if (n_dbp2 > dbp2_am) n_dbp2 = dbp2_am;				/* limit to 11 dbp2s */
	  i = dbp2row (dbp2_am, spacing, theta=  0,   dbp2_zrot, cent,  i=0,s=1);
	  break;

     case 21: 
	  if (n_dbp2 > 2*dbp2_am) n_dbp2 = 2*dbp2_am - 1; 			   /* limit to 21 dbp2s */
	  i = dbp2cent (1,                  theta=  45, dbp2_zrot,  cent,  i=0); /* set cell rot near both ams */
	  i = dbp2row (dbp2_am, spacing, theta=  90, dbp2_zrot,  cent,  i=1,s=1);
	  i = dbp2row (dbp2_am, spacing, theta=   0, dbp2_zrot,  cent,  i,  s=1);
	 ...
	 i = dbp2row (dbp2_am, spacing, theta=  0,  dbp2_zrot, cent, i=0, s=1);
	 i = dbp2row (dbp2_am, spacing, theta=  90, dbp2_zrot, cent, i,   s=1);
	  setn(dbp2,MAXSDIST,15); 
	 break;
     ...
     case 3:
	  ...
	  i = dbp2cent (1,                  theta=  45, dbp2_zrot, cent, i=0);  /* set cell rot near both ams */
	  i = dbp2row (dbp2_am, spacing, theta=  0,  dbp2_zrot, cent, i=1,s=1);
	  i = dbp2row (dbp2_am, spacing, theta=  60, dbp2_zrot, cent, i,  s=1);
	  i = dbp2row (dbp2_am, spacing, theta= -60, dbp2_zrot, cent, i,  s=1);
     ...
     case 4:
	  ...
	  i = dbp2cent (1,                  theta=  45, dbp2_zrot, cent, i=0);  /* set cell rot near both ams */
	  i = dbp2row (dbp2_am, spacing, theta=   0, dbp2_zrot, cent, i=1,s=1);
	  i = dbp2row (dbp2_am, spacing, theta=  45, dbp2_zrot, cent, i,  s=1);
	  i = dbp2row (dbp2_am, spacing, theta= -45, dbp2_zrot, cent, i,  s=1);
	  i = dbp2row (dbp2_am, spacing, theta=  90, dbp2_zrot, cent, i,  s=1);
...
if (!make_cones) {
  ...
  if (notinit(nrepeats)) nrepeats = 1;
  ...
  Vmin = -0.06; Vmax = -0.02;
  if (make_cones > 0) {
    plot_v_lnod(ct, cn, soma, Vmin = -0.05,    Vmax = -0.02,    colr=blue,   "", 3, 1.0);
    // plot_v_lnod(ct2,findcell(ct2,0,0), soma,   Vmin = -0.05,    Vmax = -0.02,    colr=green,"", 3, 1.0);
    plot_v_lnod(ct2, 1, soma,   Vmin = -0.05,    Vmax = -0.02,    colr=white,"", 3, 1.0);
    plot_v_lnod(ct2, 2, soma,   Vmin = -0.05,    Vmax = -0.02,    colr=gray,"", 3, 1.0);
  } else {
  ...
  plot_synrate(findsyn(ct,1,dendn_node(ct,presyn1),gca),   fmin=0,fmax=400,        red,  8,"psyn1",0.5);
  plot_synrate(findsyn(ct,1,dendn_node(ct,presyn2),gca),  fmin=0,fmax=400,      green,  8,"psyn2",0.5);
  // plot_synrate(findsyn(ct,1,dendn_node(ct,presyn1),am),   fmin=0,fmax=400,        red,  8,"psyn1",0.5);
  // plot_synrate(findsyn(ct,1,dendn_node(ct,presyn2),am2),  fmin=0,fmax=400,      green,  8,"psyn2",0.5);
  // plot_synrate(findsyn(dbp2,1,dendn_node(dbp2,presyn3),am),   fmin=0,fmax=400,   magenta,  8,"psyn3",0.5);
  // plot_synrate(findsyn(dbp2,2,dendn_node(dbp2,presyn3),am2),  fmin=0,fmax=400,      blue,   8,"psyn4",0.5);
  ...
  if (vnoise > 0) 
   ...
   plot_syncond(findsyn(dbp1,1,dendn_node(dbp1,presyn1),am),   cmin=0,cmax=100e-12,  red,   10,"psyn1",0.5);
   plot_syncond(findsyn(dbp1,1,dendn_node(dbp1,presyn2),am2),  cmin=0,cmax=100e-12,  green, 10,"psyn2",0.5);
   plot_syncond(findsyn(dbp2,1,dendn_node(dbp2,presyn3),am),   cmin=0,cmax=100e-12,  magenta, 10,"psyn3",0.5);
   plot_syncond(findsyn(dbp2,2,dendn_node(dbp2,presyn3),am2),  cmin=0,cmax=100e-12,  ltgreen, 10,"psyn4",0.5);
   ...
   if (stimtype == 1) { 		// flashed spot and chirp
       s = 0;
       if (nspots==3) {
         stim_spot (spotdia, x=0, y=0, -cmult*scontrast, stimtime, spotdur); s++;
       }
       stim_spot (spotdia, x=0, y=0,  cmult*scontrast, stimtime+s*spotdur, spotdur); s++;
       stim_spot (spotdia, x=0, y=0, -cmult*scontrast, stimtime+s*spotdur, spotdur); s++;
  
       time2 = spot_chirp (spotdia, x=0, y=0, fstart, fincr, cmult, scontrast, stimtime+s*spotdur+prestimdur, stimdur);
       if (vcontrast>0) time2 = spot_vcontrast (spotdia, x=0, y=0, temp_freq, 
        					cmult, c1, c2, time2+prestimdur, stimdur);

       if (nrepeats>1) {
         for(n=1;n<nrepeats;n++) {
           s = 0;
           if (nspots==3) {
             stim_spot (spotdia, x=0, y=0, -cmult*scontrast, stimtime+time2, spotdur); s++;
           }
           stim_spot (spotdia, x=0, y=0,  cmult*scontrast, stimtime+time2+s*spotdur, spotdur); s++;
           stim_spot (spotdia, x=0, y=0, -cmult*scontrast, stimtime+time2+s*spotdur, spotdur); s++;

	   time2 = spot_chirp (spotdia, x=0, y=0, fstart, fincr, cmult, scontrast, stimtime+time2+s*spotdur+prestimdur, stimdur);
           if (vcontrast>0) time2 = spot_vcontrast (spotdia, x=0, y=0, temp_freq, 
        					    cmult, c1, c2, time2+prestimdur, stimdur);
         }
       }
   ...
   if (disp) {                        // display the stimulus
      ...
      display_size(500);
      disp_end = time2+0.05;
      fprintf(stderr,"simtime %.3g, time2 %.3g, disp_end %.3g\n",simtime,time2,disp_end);
  ...
  simtime = 0 - predur;
  if (vcontrast>0) {endexp = nrepeats*stimtime+nrepeats*2*stimdur+nrepeats*nspots*spotdur+nrepeats*2*prestimdur+poststimdur;}
  else             {endexp = nrepeats*stimtime+nrepeats*1*stimdur+nrepeats*nspots*spotdur+nrepeats*1*prestimdur+poststimdur;};

  fprintf(stderr,"# endexp %.3g\n",endexp);
