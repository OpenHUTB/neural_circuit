...
void setparams(void)
{
  ...
  if (notinit(caplot)) caplot = 1; 		/* make plot of [Ca] and cabuf */
...
void runexpt(void) {
  ...
  if (ivplot) {
     ...
     plot_v_nod(ct, cn=1, 20, Vmin = min(vhold,vstop),     Vmax = max(vhold,vstopx),    colr=magenta,    "", 11, 0.5);
     plot_v_nod(ct, cn=1, 11, Vmin = min(vhold,vstop),     Vmax = max(vhold,vstopx),    colr=blue,    "", 10, 0.5);
     plot_v_nod(ct, cn=1, 9, Vmin = min(vhold,vstop),     Vmax = max(vhold,vstopx),     colr=cyan,    "", 9, 0.5);
     plot_v_nod(ct, cn=1, soma, Vmin = min(vhold,vstop),     Vmax = max(vhold,vstopx),    colr=brown,    "", 8, 0.5);
     //plot_v_nod(ct,cn=2, soma, Vmin = min(vhold,vstop),     Vmax = 0.05,    colr=blue,    "", 6, 0.5);
     // plot_i_nod(ct,cn=1, soma, imin = -iscal, imax = iscal, colr=magenta, "", 5, 1); 
     // if (dbpair) plot_i_nod(ct,cn=2, soma, imin = -iscal, imax = iscal, colr=brown, "", 5, 1); 
     plot_var(&idiff,1,Imin,Imax);  				/* plot current minus cap transient */
        plot_param ("Idbp_soma", colr=brown, pl=5,plsize=1);
     plot_var(&current,1,Imin,Imax);  				/* plot current minus cap transient */
        plot_param ("Idbp_nonsub", colr=brown, pl=4,plsize=1);
     plot_var(&cond,1,Gmin,Gmax);                               /* plot cond=current/driving force */
        plot_param ("Gdbp_soma", colr=green, pl=3,plsize=1);

     if (caplot) {
        plot_ca_nod    (ct, cn=1, axnode, 10e-6, cyan, "Ca axnode",    1, 0.5); 
        plot_cabufb_nod(ct, cn=1, axnode, 10e-6, red,  "Cabuf axnode", 1, 0.5);
     }
  }
  ...
  dst = 0.0001;		// small time to allow voltage clamp to end
   ...
   if (!ivplot) flag = true;
  //for (i=0,vpulse=vstart; vpulse>=vstop; i++,vpulse += vstep) {
