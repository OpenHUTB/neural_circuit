...
double rad_dist2 (int ct, int cn, int tnod, int parent, int somanode) 
...
{
  return (dist3d (ndn(ct,cn,tnod), ndn(ct,cn,somanode)));
};
...
*/
    ...
    double dist,newdist,nlength,totlen, lfrac;
 ...
 totlen = sqrt (xrange*xrange + yrange*yrange);
 lfrac = (totlen - tiplen) / totlen; 
 nlength = totlen * lfrac;

 nsegs = int (nlength / 10) + 1 + (tiplen > 0);		// tip seg is extra
 xincr = xrange / nsegs * lfrac;
 yincr = yrange / nsegs * lfrac;
 zincr = zrange / nsegs * lfrac;

 drange = cd1 - cd2;

 if      (nsegs>=2)  dincr = drange / (nsegs);
 else                dincr = 0;
...
ncfprintf (stdout,"taperden: cable from node %d %d %d to %d %d %d dia %g\n\n",
 ...
 if (nsegs==1) {
    make_celseg(ct,cn,n1,n2,cd1,cd2,region,cplam);
 }
    ...
    npnt = nd(ct,cn,nden+1); npnt->xloc=x1+xincr; 
			     npnt->yloc=y1+yincr;
			     npnt->zloc=z1+zincr;
    make_celseg(ct,cn,n1,nden+1,cd1,cd1-dincr,region,cplam);

   for (i=1; i<nsegs-1; i++) {		/* middle segments */

     npnt = nd(ct,cn,nden+i+1); npnt->xloc= x1+(i+1)*xincr; 
			        npnt->yloc= y1+(i+1)*yincr; 
			        npnt->zloc= z1+(i+1)*zincr;

     make_celseg(ct,cn,nden+1,nden+i+1,cd1-dincr,cd1-dincr*i,region,cplam);

   }

   if (tiplen > 0) {
         make_celseg(ct,cn,nden+i,n2,tipdia,tipdia,  region,cplam);
   }
   else  make_celseg(ct,cn,nden+i,n2,cd1-dincr*i,cd2,region,cplam);
 };
...
*/
 ...
 dist = rad_dist2 (ct, cellnr, nod1, nod2, somanode);	/* find distance to soma */
 ...
 d2 = rad_diam (dist+newdist,diaspc,diabs);
 cplam = -1;

  return (taperden(ct,cellnr,nod1,nod2, d1,d2,tipdia,tiplen,region,nden)); /* */
...
/* Procedure to find a cell soma closest to (roffset, theta) from (0,0). */
...
/* Procedure to find a node in cell region to record from. */
/* Find node closest to (xoffset, yoffset) relative to soma. */
/* If region is between 1 and 20, it is assumed to be a region */
/*  If region is between 21 and 1000, it is assumed to be a node label */
/*  If region is between 1001 and 1005 it is assumed to be a set of regions */
/*   defined in the dens_xxx.n file in one of the rows _SREG1 ... _SREG5 */

{
   int i,n,nod,found,closenod, closenod_region;
   double dx, dy, dist, mindist;
   node *npnt, *somanode;
  ...
  for (npnt=nodepnt; npnt=foreach(npnt, ct, cn, -1, NULL,NULL,&nod); npnt=npnt->next) {
      ...
      if (check_region(ct, cn, npnt->region, region, nod, "") <= 0) continue;
      dx = npnt->xloc - xoffset;
      dy = npnt->yloc - yoffset;
      dist = sqrt(dx*dx+dy*dy);
      if (mindist>dist) {
        mindist = dist;
        closenod = nod;
	closenod_region = npnt->region;
        found = 1;
      }
  }
  if (!found) closenod = -1;
  // fprintf (stderr,"closenod %d closenod_region %d region %d\n",closenod,closenod_region,region);
  return closenod;
}
...
/* Procedure to find a node in cell to record from. */
/* Find node closest to (xoffset, yoffset) from its soma, but nearer than maxdist. */
{
   int i,n,nod,found,closenod;
   double dx, dy, dist, mindist;
   node *npnt, *somanode;
...
/* Find node closest to (roffset, theta) from its soma. */
{
  ...
  xoffset = roffset *  cos (theta*PI/180.0);
  yoffset = roffset * -sin (theta*PI/180.0);
  return findnodlocr(ct, cn, xoffset, yoffset, region);
}
...
/* Procedure to find a node in cell to record from. */
/* Find node closest to (roffset, theta) from its soma, but nearer than maxdist. */
{
   double xoffset, yoffset;
...

{
  ...
  closesyn = NULL;
  mindist = 1e10;
  for (epnt=*lepnt; epnt=foreach(epnt, SYNAPSE, ct, cn, nod, NULL,&ncn,&nnod); epnt=epnt->next) {
     if (cn2<0) { 
        if (epnt->node1a==ct  && epnt->node1b==ncn && epnt->node1c==nnod && 
            epnt->node2a==ct2) {
            closesyn = (synapse *)epnt;
...

{
  ...
  return (findsyn(ct, cn, nod, ct2, cn2, &lepnt));
}
...
/* Procedure to find a synapse in either of 2 cell types with a synapse to record from. */
/* Find synapse closest to (xoffset, yoffset) from (0,0). */
{
   int i,n,found;
   double dx, dy, dist, mindist;
  ...
  closesyn = NULL;
  mindist = 1e10;
  for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct1, -1, -1); epnt=epnt->next) {
     if (epnt->node1a==ct1) {
       dx = epnt->nodp1->xloc - xoffset;
       dy = epnt->nodp1->yloc - yoffset;
       dist = sqrt(dx*dx+dy*dy);
  ...
  for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct2, -1, -1); epnt=epnt->next) {
     if (epnt->node1a==ct2) {
       dx = epnt->nodp1->xloc - xoffset;
       dy = epnt->nodp1->yloc - yoffset;
       dist = sqrt(dx*dx+dy*dy);
...
/* Procedure to find a synapse in any of 3 cell types with a synapse to record from. */
/* Find synapse closest to (xoffset, yoffset) from (0,0). */
{
   int i,n,found;
   double dx, dy, dist, mindist;
  ...
  for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct3, -1, -1); epnt=epnt->next) {
     if (epnt->node1a==ct3) {
       dx = epnt->nodp1->xloc - xoffset;
       dy = epnt->nodp1->yloc - yoffset;
       dist = sqrt(dx*dx+dy*dy);
...
/*-------------------------------------------------*/
// 
//  Don't use because it gets confused with:
...
//   for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct, cn, -1); epnt=epnt->next) {
//      s = (synapse *)epnt;
//      if ( ((ct<0) || epnt->node1a==ct) && ((cn<0) || epnt->node1b==cn) && (abs(s->vrev-vrev) < 0.001)) {
//        dx = epnt->nodp1->xloc - xoffset;
//        dy = epnt->nodp1->yloc - yoffset;
//        dist = sqrt(dx*dx+dy*dy);
//        if (mindist>dist) {
...
/* Find synapse closest to (xoffset, yoffset). */
{
  ...
  closesyn = NULL;
  mindist = 1e10;
  for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct1, -1, ct3, cn3, 0); epnt=epnt->next) {
       dx = epnt->nodp1->xloc - xoffset;
       dy = epnt->nodp1->yloc - yoffset;
       dist = sqrt(dx*dx+dy*dy);
       if (mindist>dist) {
  ...
  for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct2, -1, ct3, cn3, 0); epnt=epnt->next) {
       dx = epnt->nodp1->xloc - xoffset;
       dy = epnt->nodp1->yloc - yoffset;
       dist = sqrt(dx*dx+dy*dy);
       if (mindist>dist) {
...
/* Find synapse closest to (xoffset, yoffset). */
{
  ...
  closesyn = NULL;
  mindist = 1e10;
  for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct1, -1, ct4, cn4, 0); epnt=epnt->next) {
       dx = epnt->nodp1->xloc - xoffset;
       dy = epnt->nodp1->yloc - yoffset;
       dist = sqrt(dx*dx+dy*dy);
       if (mindist>dist) {
  ...
  for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct2, -1, ct4, cn4, 0); epnt=epnt->next) {
       dx = epnt->nodp1->xloc - xoffset;
       dy = epnt->nodp1->yloc - yoffset;
       dist = sqrt(dx*dx+dy*dy);
       if (mindist>dist) {
  ...
  for (epnt=elempnt; epnt=foreach(epnt, SYNAPSE, ct3, -1, ct4, cn4, 0); epnt=epnt->next) {
       dx = epnt->nodp1->xloc - xoffset;
       dy = epnt->nodp1->yloc - yoffset;
       dist = sqrt(dx*dx+dy*dy);
       if (mindist>dist) {
...
/* Procedure to find a synapse in a cell with a synapse to record from. */
/* Find synapse closest to (xoffset, yoffset) from (0,0) with vrev. */

{
   double xoffset, yoffset, maxdist;
   
  xoffset = roffset *  cos (theta*PI/180.0);
  yoffset = roffset * -sin (theta*PI/180.0);

  return findsynloc(ct, -1, ct2, cn2, xoffset, yoffset, vrev, maxdist=100);
}
...
/* Find synapse closest to (xoffset, yoffset) from (0,0). */
{
  ...
  xoffset = roffset *  cos (theta*PI/180.0);
  yoffset = roffset * -sin (theta*PI/180.0);
  return findsynloct(ct1, ct2, ct3, cn3, xoffset, yoffset);
}
...
/* Find synapse closest to (xoffset, yoffset) from (0,0). */
{
  ...
  xoffset = roffset *  cos (theta*PI/180.0);
  yoffset = roffset * -sin (theta*PI/180.0);
  return findsynloct(ct1, ct2, ct3, ct4, cn4, xoffset, yoffset);
}
...

{
    ...
    for (epnt=elempnt; epnt=foreach(epnt, ELEMENT, ct, cn, -1, -1, 
					      NULL,NULL,&n,NULL); epnt=epnt->next) {  
        if (gctype(ct) &&  ((epnt->region!=HILLOCK) &&
             		   (epnt->region!=AXON_THIN) && 
			   (epnt->region!=AXON_PROX) && 
			   (epnt->region!=AXON_DIST)))
...
void printsyns(int n1a, int n1b, int n2a, int n2b)
 ...
 /* Print synapses from presynaptic to postsynaptic cell */
 /*  To match any, call with -1 in args */

{
   int i;	
   elem *e;

  if (cname[n1a] == NULL || cname[n2a] == NULL) return;
  for (i=0, e=elempnt; e=foreach (e, SYNAPSE, n1a,n1b, n2a,n2b, 0); e=e->next, i++) {
         ncfprintf (stderr,"#syn %s %4d %4d to %s %4d %4d\n", 
		 cname[n1a],e->node1b,e->node1c,
		 cname[n2a],e->node2b,e->node2c);
  }
  fprintf (stderr,"# total synapses from %s %d to %s %d: %d\n",
		  cname[n1a], n1b,
		  cname[n2a], n2b, i);
}
...

{
  ...
  if (cname[n1a] == NULL || cname[n2a] == NULL) return;
  for (i=0, e=elempnt; e=foreach (e, SYNAPSE, n1a,n1b,n1c, n2a,n2b,n2c, 0); e=e->next, i++) {
         ncfprintf (stderr,"#syn %s %4d %4d to %s %4d %4d\n", 
		 cname[n1a],e->node1b,e->node1c,
		 cname[n2a],e->node2b,e->node2c);
  }
  fprintf (stderr,"# total synapses from %s %d %d to %s %d %d: %d\n",
		  cname[n1a], n1b, n1c, 
		  cname[n2a], n2b, n2c, i);
}
