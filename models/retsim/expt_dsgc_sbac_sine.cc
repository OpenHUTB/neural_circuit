...

{
  ...
  setptr("r_dbp1_sbac",	 &r_dbp1_sbac);
  setptr("idist",	 &idist);
  setptr("mglur2",	 &mglur2);
  setptr("g_ams_sbac",	 &g_ams_sbac);
  setptr("f_ams_sbac",	 &f_ams_sbac);
  ...
  setptr("db1_caax",	 &db1_caax);
  setptr("db2_caax",	 &db2_caax);
  ...
  if (sbac_species==1) {		 // parameter values in nval file specific to mouse
      ...
      if (notinit (db1_caax)) db1_caax = 1e-3;
      if (notinit (db2_caax)) db2_caax = 1e-3;
  ...
  nvalfile = "nval_dsgc_sbac_sine.n";
  if (notinit(sbac_densfile))  sbac_densfile   = "dens_sbac_sine.n";
  if (notinit(sbac_densfile2)) sbac_densfile2  = "dens_sbac_sine2.n";
  ...
  chanparamsfile  = "chanparams_sbac_sine";
...

{
  ...
  if (notinit(disp_incr)) disp_incr = 0.001;		/* time step for stimulus display */
...
#define SBARR 100
   ...
   if (notinit(cbplam)) cbplam = 0.2;
   // setn(dbp1,NCOLOR,RCOLOR);  			/* set dbp1 display color from region */
 ...
 if (!notinit(sbarr)) {		// array of sbac locations
   ...
   if (sbarr==3) {                    	/* 3 aligned */
       sbxarr[0] =  0;       sbyarr[0] = 0; sbtharr[0] = 0  + sbac_r + sbrarr[0] * sbac_rnd; sbnarr[0]=1;
       sbxarr[1] =  -sbspac; sbyarr[1] = 0; sbtharr[1] = 30 + sbac_r + sbrarr[1] * sbac_rnd; sbnarr[1]=2;
       sbxarr[2] =   sbspac; sbyarr[2] = 0; sbtharr[2] = 60 + sbac_r + sbrarr[2] * sbac_rnd; sbnarr[2]=3;
       n_sbac = 3;
   }
  ...
  if (!notinit(r_dbp1_sbac)) {					/* dbp1 -> sbac synaptic depression */
	  		      setsv(dbp1,SMRRPOOL,3,100);
	  		      setsv(dbp1,SMAXRATE,3,r_dbp1_sbac);
  }
  ...
  if (!notinit(r_sbac_sbac)) {					/* sbac - sbac synaptic depression */
	  		      setsv(sbac,SMRRPOOL,4,100);
	  		      setsv(sbac,SMAXRATE,4,r_sbac_sbac);
  }
  ...
  amsarrsiz = 600;		/* make narrow-field amacrines to contact sbac near soma */
...

{
    ...
    if (sbarr== -2) {		    // set cn 2 to use sbac_densfile2
       ...
       ndens[sbac][cn=2] = 1;         // set cells to use sbac_densfile2, inhib input cond set by sb1mul
       ...
       ndens[sbac][cn=5] = 1;         // set cells to use sbac_densfile2
...
double move_stim(double stimtime,double barwidth,double theta,double velocity, double ncycles, double scontrast, int direction, double mask)
{
 ...
 if (stimtype==1) {			// move bar to & fro
  ...
  dur = ncycles / tfreq;
  movesineann (0, 0, direction=1, annrad, 0,   0, barwidth, tfreq, inten_add=0, inten_mult=1.0, minten, scontrast, makenv, waveshape, stimtime,dur);
  movesineann (0, 0, direction=2, annrad, 0, 180, barwidth, tfreq, inten_add=0, inten_mult=1.0, minten, scontrast, makenv, waveshape, 2*stimtime+dur,dur);
  mvbrt1 = dur;
  ...
  dur = ncycles / tfreq;
  movegrating (0, 0, theta, sphase=0, barwidth, tfreq, drift=1,  inten_add=0, inten_mult=1.0, scontrast, waveshape, stimtime,dur);
  movegrating (0, 0, theta, sphase=0, barwidth, tfreq, drift=-1, inten_add=0, inten_mult=1.0, scontrast, waveshape, 2*stimtime+dur,dur);
  mvbrt1 = stimtime+dur;
  mvbrt2 =2*stimtime + 2*dur;
...
void addlabels(void)
{
    ...
    int nsynapsb2t1, nsynapsb5t1, nsynapdbp1t1;
    int nsynapams, nsynapams2, nsynapams3;
    node *npnt;
    photorec *p;
  ...
  /*  - - - - - - - - - - - - - - - - - - - */
   ...
   nsynapdbp1t1 = synapse_add (k=5,dbp1,-1,-1,sbac,c1,3,50,0,15); /* make list of excit syns from dbp1s to sbac 1 */
   ...
   nsynapsb2t1 = synapse_add (k=38,sbac,c2,-1,sbac,c1,4);     /* make list of inhib synapses from sbac 2 to sbac 1 */
   nsynapsb5t1 = synapse_add (k=39,sbac,c5,-1,sbac,c1,4);     /* make list of inhib synapses from sbac 5 to sbac 1 */

   nsynapam  = synapse_add  (k=51,am,  -1,-1,dsgc,-1,2);      /* make list of inhib synapses from am's to dsgcs */
   nsynapam2 = synapse_add  (k=52,am2, -1,-1,dsgc,-1,2);      /* make list of inhib synapses from am2's to dsgcs */

   nsynapams  = synapse_add  (k=55,ams, -1,-1,sbac,c1,2);      /* make list of inhib syns from prox ams's to sbac 1 */
   nsynapams2 = synapse_add  (k=56,ams, -1,-1,sbac,c5,2);      /* make list of inhib syns from dist ams's to sbac 2 */
   nsynapams3 = synapse_add  (k=57,ams, -1,-1,sbac,c2,2);      /* make list of inhib syns from dist ams's to sbac 3 */
...
//   k = number of synapse lists up to here
   ...
   nsynap = nsynapa + nsynapb + nsynapc + nsynapg + nsynapds + nsynapsb + nsynapam + nsynapam2 + 
	    nsynapams + nsynapams2 + nsynapams3 + nsynapsb2t1 + nsynapsb5t1 + nsynapdbp1t1;
   ...
   ncfprintf (stderr,"# nsyn_dbp1->sbac1  %4d\n",nsynapdbp1t1);		// dbp1 synapses near 80,0
   ...
   ncfprintf (stderr,"# nsyn_sbac2->sbac1 %4d\n",nsynapsb2t1);
   ncfprintf (stderr,"# nsyn_sbac5->sbac1 %4d\n",nsynapsb5t1);
   ncfprintf (stderr,"# nsyn_ams->sbac    %4d\n",nsynapams);
   ncfprintf (stderr,"# nsyn_ams2->sbac   %4d\n",nsynapams2);
   ncfprintf (stderr,"# nsyn_ams3->sbac   %4d\n",nsynapams3);
...
//  if (notinit(stimdur))   stimdur=0.45;	/* used for non-moving stimuli */
  ...
  /*  - - - - - - - - - - - - - - - - - - - */
   ...
   //     sprintf (savefile,"dsgc_sbac_sine%06d",getpid());       // add pid to file name
   // }
   ...
   amss[0]  = findmid(ams,20,0);
   amss[1]  = findmid(ams,150,0);
  ...
  if (strcmp(sbac_file,"morph_sb1")==0) {
   ...
   npnt = findnode (sbac,c1,soma);			// find central cell
  ...
  stimdur = move_stim(stimtime, barwidth, stim_theta, velocity, ncycles, scontrast, direction, mask=0);
   ...
   if (disp & DSTIM) {
	...
	dispsize=maxsize*(1+disp_margin);
     ...
     for (starttime=simtime,t=0; t<disp_end; starttime = t, t+= disp_incr) {
   ...
   if (!notinit(make_movie) && make_movie) {
    ...
    plot_syncond(findsynloca(dbp1,sbac,-1,-6*r,rstim_theta),cmin=0,cmax, blue,   46,"",0.5);
    plot_syncond(findsynloca(dbp1,sbac,-1,-4*r,rstim_theta),cmin=0,cmax, green,  46,"",0.5);
    plot_syncond(findsynloca(dbp1,sbac,-1,-2*r,rstim_theta),cmin=0,cmax, cyan,   46,"",0.5);
    plot_syncond(findsynloca(dbp1,sbac,-1,0,0),             cmin=0,cmax, red,    46,"",0.5);
    plot_syncond(findsynloca(dbp1,sbac,-1,2*r,rstim_theta), cmin=0,cmax, magenta,46,"",0.5);
    plot_syncond(findsynloca(dbp1,sbac,-1,4*r,rstim_theta), cmin=0,cmax, brown,  46,"",0.5);
    plot_syncond(findsynloca(dbp1,sbac,-1,6*r,rstim_theta), cmin=0,cmax, white,  46,"",0.5);
    ...
    if (nsbac==1 || sbarr==-2) { 
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1,   0,0), Vmin_c1,Vmaxg,blue,  "",pl=37,0.8);
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1,  40,0), Vmin_c1,Vmaxg,gray,  "",pl=37,0.8);
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1,  80,0), Vmin_c1,Vmaxg,cyan,  "",pl=37,0.8);
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 120,0), Vmin_c1,Vmaxg,ltblue,"",pl=37,0.8);
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 150,0), Vmin_c1,Vmaxg,magenta,"",pl=37,0.8);
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, -40,0), Vmin_c1,Vmaxg,brown, "",pl=37,0.8);
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, -80,0), Vmin_c1,Vmaxg,white, "",pl=37,0.8);
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, -120,0), Vmin_c1,Vmaxg,green,"",pl=37,0.8);
       plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, -150,0), Vmin_c1,Vmaxg,red,  "",pl=37,0.8);
    }

    // look at distal dendrites at increments of "a" (30) deg
   
   if (stimtype!=7 && stimtype !=8) { 
    plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,0*a), Vmin_c1,Vmaxg,blue,  "",pl=36,0.8);
    plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,2*a), Vmin_c1,Vmaxg,green, "",pl=36,0.8);
    plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,4*a), Vmin_c1,Vmaxg,cyan,  "",pl=36,0.8);
    plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,6*a), Vmin_c1,Vmaxg,red,   "",pl=36,0.8);
    plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,8*a), Vmin_c1,Vmaxg,magenta,"",pl=36,0.8);
    plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,10*a), Vmin_c1,Vmaxg,brown, "",pl=36,0.8);
    // plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,12*a), Vmin_c1,Vmaxg,white, "",pl=36,0.8);
    // plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,9*a), Vmin_c1,Vmaxg,gray,  "",pl=36,0.8);
   }

    plot_v_nod(ct=ams,amss[0],soma, Vmin_c1,Vmaxg,brown, "",pl=35,0.5);
    // plot_syncond(findsynlocr(ams,amss[0],0,0), cmin=0,1e-10, green,  34,"Gams_sbac    ",0.5);
    // plot_syncond(findsynlocr(ams,amss[1],0,0), cmin=0,1e-10, cyan,   34,"Gams_sbacd   ",0.5);

    sprintf (sbuf, "Gams_sbac_%d  ",c1);
    plot_func(gsyn_tot,55,rmax=1e-9,rmin=0); plot_param(sbuf,green,  34,0.5);
    sprintf (sbuf, "Gams_sbac_%d  ",c5);
    plot_func(gsyn_tot,56,rmax=1e-9,rmin=0); plot_param(sbuf,cyan,  34,0.5);
    sprintf (sbuf, "Gams_sbac_%d  ",c2);
    plot_func(gsyn_tot,57,rmax=1e-9,rmin=0); plot_param(sbuf,brown,  34,0.5);

    plot_ca_nod(ct=sbac,c1,findnodlocra(sbac,c1, 150,0), 1,5e-6,red,  "",pl=30,0.8);
    plot_v_nod (ct=sbac,c1,findnodlocra(sbac,c1, 150,0), Vmin_c1,Vmaxg,magenta,  "",pl=10,0.8);

    if (sbarr!=-2) {
     if (nsbac > 1) {
      plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,0*a), Vmin,Vmaxg,blue,  "",pl=27,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,3*a), Vmin,Vmaxg,green, "",pl=27,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,4*a), Vmin,Vmaxg,cyan,  "",pl=27,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,5*a), Vmin,Vmaxg,red,   "",pl=27,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,6*a), Vmin,Vmaxg,magenta,"",pl=27,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,7*a), Vmin,Vmaxg,brown, "",pl=27,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,8*a), Vmin,Vmaxg,white, "",pl=27,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,9*a), Vmin,Vmaxg,gray,  "",pl=27,0.8);/* V at dist nod */

      plot_v_nod(ct=sbac,c5,findnodlocra(sbac,c5, 110,0*a), Vmin,Vmaxg,blue,  "",pl=26,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c5,findnodlocra(sbac,c5, 110,3*a), Vmin,Vmaxg,green, "",pl=26,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c5,findnodlocra(sbac,c5, 110,4*a), Vmin,Vmaxg,cyan,  "",pl=26,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c5,findnodlocra(sbac,c5, 110,5*a), Vmin,Vmaxg,red,   "",pl=26,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c5,findnodlocra(sbac,c5, 110,6*a), Vmin,Vmaxg,magenta,"",pl=26,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c5,findnodlocra(sbac,c5, 110,7*a), Vmin,Vmaxg,brown, "",pl=26,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c5,findnodlocra(sbac,c5, 110,8*a), Vmin,Vmaxg,white, "",pl=26,0.8);/* V at dist nod */
      plot_v_nod(ct=sbac,c5,findnodlocra(sbac,c5, 110,9*a), Vmin,Vmaxg,gray,  "",pl=26,0.8);/* V at dist nod */
     }
    }
    if (ivplot) {
    ...
    // plot_v_nod(ct=sbac,c1,findnodlocra(sbac,c1, 110,0*a), Vmin,Vmaxg,blue,  "",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c2,findnodlocra(sbac,c2, 110,3*a), Vmin,Vmaxg,green, "",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c3,findnodlocra(sbac,c3, 110,4*a), Vmin,Vmaxg,cyan,  "",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c8,findnodlocra(sbac,c8, 110,5*a), Vmin,Vmaxg,red,   "",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c9,findnodlocra(sbac,c9, 110,6*a), Vmin,Vmaxg,magenta,"",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c10,findnodlocra(sbac,c10, 110,7*a), Vmin,Vmaxg,brown, "",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c11,findnodlocra(sbac,c11, 110,8*a), Vmin,Vmaxg,white, "",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c12,findnodlocra(sbac,c12, 110,9*a), Vmin,Vmaxg,gray,  "",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c13,findnodlocra(sbac,c13, 110,9*a), Vmin,Vmaxg,ltblue,"",pl=25,0.5);/* V at dist nod */
    // plot_v_nod(ct=sbac,c14,findnodlocra(sbac,c14, 110,9*a), Vmin,Vmaxg,ltmag,"",pl=25,0.5);/* V at dist nod */

    // if (n_sbac >= 23) {
    //   plot_v_nod(ct=sbac,c1,soma,Vmin,Vmaxg,blue,    "",pl=24,0.5);/* V at soma */
    //   plot_v_nod(ct=sbac,c10,soma,Vmin,Vmaxg,magenta,"",pl=24,0.5);/* V at soma */
    //   plot_v_nod(ct=sbac,c16,soma,Vmin,Vmaxg,green,  "",pl=24,0.5);/* V at soma */
    //   if (n_sbac >= 45)
    //       plot_v_nod(ct=sbac,c25,soma,Vmin,Vmaxg,ltblue, "",pl=24,0.5);/* V at soma */
    //   if (n_sbac >= 57) 
    //       plot_v_nod(ct=sbac,c53,soma,Vmin,Vmaxg,red,  "",pl=24,0.5);/* V at soma */
    // }
    // else if (nsbac > 1 && sbarr != -2) {
    //   plot_v_nod(ct=sbac,c1,soma,Vmin,Vmaxg,blue,   "",pl=24,0.5);/* V at soma */
    //   plot_v_nod(ct=sbac,c2,soma,Vmin,Vmaxg,green,  "",pl=24,0.5);/* V at soma */
    //   plot_v_nod(ct=sbac,c3,soma,Vmin,Vmaxg,cyan,   "",pl=24,0.5);/* V at soma */
    //   plot_v_nod(ct=sbac,c4,soma,Vmin,Vmaxg,red,    "",pl=24,0.5);/* V at soma */
    //   plot_v_nod(ct=sbac,c5,soma,Vmin,Vmaxg,magenta,"",pl=24,0.5);/* V at soma */
    //   plot_v_nod(ct=sbac,c6,soma,Vmin,Vmaxg,yellow, "",pl=24,0.5);/* V at soma */
    //   plot_v_nod(ct=sbac,c7,soma,Vmin,Vmaxg,gray,   "",pl=24,0.5);/* V at soma */
    // }

    // plot_func(gsyn_tot,34,rmax=1e-8,rmin=0); plot_param("Gsbac_sbac2",blue,  23,0.5);
    // plot_func(gsyn_tot,35,rmax=1e-8,rmin=0); plot_param("Gsbac_sbac5",red,   23,0.5);

    if (sbarr==100) {
      sprintf (sbuf, "Gsbac_sbac%d",c1);
      plot_func(gsyn_tot,33,rmax=1e-8,rmin=0); plot_param(sbuf,blue,   20,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c2);
      plot_func(gsyn_tot,34,rmax=1e-8,rmin=0); plot_param(sbuf,magenta,20,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c22);
      plot_func(gsyn_tot,35,rmax=1e-8,rmin=0); plot_param(sbuf,green, 20,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c23);
      plot_func(gsyn_tot,36,rmax=1e-8,rmin=0); plot_param(sbuf,ltblue,20,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c50);
      plot_func(gsyn_tot,37,rmax=1e-8,rmin=0); plot_param(sbuf,red,   20,0.5);
    } else if (nsbac <= 7 && make_sbac_sbac==1) {
      sprintf (sbuf, "Gsbac_sbac%d",c1);
      plot_func(gsyn_tot,33,rmax=1e-9,rmin=0); plot_param(sbuf,blue,   20,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c2);
      plot_func(gsyn_tot,31,rmax=1e-9,rmin=0); plot_param(sbuf,magenta,20,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c5);
      plot_func(gsyn_tot,32,rmax=1e-9,rmin=0); plot_param(sbuf,green, 20,0.5);

      sprintf (sbuf, "Gsbac%d_sbac%d   ",c2,c1);
      plot_func(gsyn_tot,38,rmax=1e-9,rmin=0); plot_param(sbuf,magenta, 21,0.5);
      sprintf (sbuf, "Gsbac%d_sbac%d   ",c5,c1);
      plot_func(gsyn_tot,39,rmax=1e-9,rmin=0); plot_param(sbuf,green, 21,0.5);

      // sprintf (sbuf, "Gdbp1_sbac%d @50 ",c1);
      // plot_func(isyn_tot,5,rmax=2e-10,rmin=0); plot_param(sbuf,blue, 22,0.5);
      sprintf (sbuf, "Idbp1_sbac%d @50 ",c1);
      plot_func(isyn_tot,5,rmax=0,rmin= -3e-12); plot_param(sbuf,blue, 22,0.5);

    } else if (nsbac > 1 && make_sbac_sbac==1) {
      sprintf (sbuf, "Gsbac_sbac%d",c1);
      plot_func(gsyn_tot,33,rmax=1e-8,rmin=0); plot_param(sbuf,blue,   15,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c10);
      plot_func(gsyn_tot,34,rmax=1e-8,rmin=0); plot_param(sbuf,magenta,15,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c16);
      plot_func(gsyn_tot,35,rmax=1e-8,rmin=0); plot_param(sbuf,green, 15,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c25);
      plot_func(gsyn_tot,36,rmax=1e-8,rmin=0); plot_param(sbuf,ltblue,15,0.5);
      sprintf (sbuf, "Gsbac_sbac%d",c53);
      plot_func(gsyn_tot,37,rmax=1e-8,rmin=0); plot_param(sbuf,red,   15,0.5);
    }
    ...
    if (run_vclamp_sbac>0) {
	        plot_i_nod (ct=sbac,c1,soma,   Imin=-100e-12,Imax=100e-12,blue,"", 3, 0.5);
    }
  ...
  if (stimtype==7) {
	...
	plot_i_nod   (ct=sbac,c1,soma,   Imin=-200e-12,Imax=200e-12,magenta,"", 5, 0.5);
	if (sbarr== -2) {
	  plot_i_nod (ct=sbac,c2,soma,   Imin=-200e-12,Imax=200e-12,cyan,"", 5, 0.5);
	  plot_var (&isub,               1, Imin=-100e-12,Imax=000e-12);
	  plot_param ("Isbac sub",magenta, 4, 0.5);
	}
        endexp=prestimdur+sbac_vpulse_dur+tailcurdur+poststimdur;
  }
  else if (stimtype==5) endexp=stimdur+poststimdur;
  else endexp=stimtime+stimdur+poststimdur;

  step (1e-4);		// short step before run to create model with channels
  if (mglur2 > 0) set2ndmsg(CA,-1,dbp1,sbac,AMPA,sb_mglur_maxdist,mglur2);    // set glut to modulate Ca chans
