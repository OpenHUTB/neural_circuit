...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - */
			...
			/* use thetax,y,z to rotate cell */

 mxrot     = 0;		/* X rotation for display */
 myrot     = 0;		/* Y rotation for display */
 mzrot     = 0;		/* Z rotation for display */
...
if (strcmp(expt,"help")==0 && !silent) {
  ...
  fprintf (stderr,"                | cbp_flash    | cbp_sine\n");
  fprintf (stderr,"                | gc_Rin       | gc_cbp_flash\n");
  fprintf (stderr,"                | dsgc_cbp_bar | dsgc_dend_dive\n");
  fprintf (stderr,"                | sbac_chans   | sbac_vclamp\n");
  fprintf (stderr,"                | rbp_aii_a17  | aii_dbp\n");
...
if (!notinit(am2_biophys))   setn(AM2,BIOPHYS,am2_biophys);
if (!notinit(am3_biophys))   setn(AM3,BIOPHYS,am3_biophys);
if (!notinit(am4_biophys))   setn(AM4,BIOPHYS,am4_biophys);
...
if (make_sbac) {
  ...
  if (!notinit(sbarrsiz)) {
      sbxarrsiz = sbarrsiz;
      sbyarrsiz = sbarrsiz;
      nsbac = setupcells(sbac,n_sbac,sbxarrsiz,sbyarrsiz,sbac_first_cent);
  }
      ...
      nsbac = setupcells(sbac,n_sbac,sbac_first_cent);
...
if (make_ha) {		/* HA cells */
  if (!notinit(haarrsiz)) {
      haxarrsiz = haarrsiz;
      hayarrsiz = haarrsiz;
      nha = setupcells(ha,n_ha,haxarrsiz,hayarrsiz);
  }
  else if (!notinit(haxarrsiz)) {
      nha = setupcells(ha,n_ha,haxarrsiz,hayarrsiz);
  } 
  else if (haxarr!=NULL) {
      nha = setupcells(ha,n_ha, haxarr, hayarr, hatharr);
  }
  else
...
if (make_hb) {		/* HB cells */
  if (!notinit(hbarrsiz)) {
      hbxarrsiz = hbarrsiz;
      hbyarrsiz = hbarrsiz;
      nhb = setupcells(hb,n_hb,hbxarrsiz,hbyarrsiz);
  }
  else if (!notinit(hbxarrsiz)) {
      nhb = setupcells(hb,n_hb,hbxarrsiz,hbyarrsiz);
  } 
  else if (hbxarr!=NULL) {
      nhb = setupcells(hb,n_hb, hbxarr, hbyarr, hbtharr);
  }
  else
      nhb=setupcells(hb,n_hb);
};

if (make_hbat) {		/* rod Hz cells (HB axon terminals) */
  if (!notinit(hbatarrsiz)) {
      hbatxarrsiz = hbatarrsiz;
      hbatyarrsiz = hbatarrsiz;
      nhbat = setupcells(hbat,n_hbat,hbatxarrsiz,hbatyarrsiz);
  }
  else if (!notinit(hbatxarrsiz)) {
      nhbat = setupcells(hbat,n_hbat,hbatxarrsiz,hbatyarrsiz);
  } 
  else if (hbatxarr!=NULL) {
      nhbat = setupcells(hbat,n_hbat, hbatxarr, hbatyarr, hbattharr);
  }
  else
...
/* Before making bps and photoreceptors,
   ...
   This section enlarges the xarrsiz, yarrsiz variables to provide extra
   space for horizontal cells and bipolar cells. One consequence of this
   is that, for example, adding additional arrays of bipolar cells will 
   enlarge the array, which will affect the (semi-random) locations of all
   the bipolar cell types and photoreceptors. To avoid having the addition
   of a bipolar cell type affect the placement of other cell types, set
   the "arrsiz" or "xarrsiz,yarrsiz" variables. This will keep the array
   size you set, and will prevent interaction between the different bipolar
   cell arrays.
*/
...
if (setarrsiz==0) { 
 ...
 if(notinit(xarrsiz)) xarrsiz = 10;	/* if none of the above cells are made, set arrsiz now */
...
if (make_rods) {
  ...
  if (rod_rect_arr) {
    ...
    for (j= -rwid; j<rwid; j++)
      for (i= -rwid; i<rwid; i++) {
         makcell (xrod, ++rodnum, 0, i*rodspac, j*rodspac,0,0,0);
      };
    nrods = rodnum;
...
if (make_cones) {
  ...
  if (cone_rect_arr) {				/* make rectangular array */
     ...
     for (j= -rwid; j<rwid; j++)
        for (i= -rwid; i<rwid; i++) {
          makcell (xcone, ++conenum, 0, i*conespac, j*conespac, 0, 0, 0);
        };
     ncones = conenum;
    ...
    if (!notinit(conearrsiz) || !notinit(conexarrsiz)) {  // if cone array size defined
...
if (make_one_dbp1) {	 /* make one cone bipolar at specified location */
   ...
   makcell (dbp1, 1, 0, dbp1_xloc, dbp1_yloc, 0, 0, flip );
   ndbp1 = 1;
   setn (dbp1,MAKE,1);
...
if (make_cone_rod)  connect_types (xcone,xrod); /* */
...
if (make_rod_hbat)  connect_types (xrod,hbat, make_hbat_rod, dyadc=0); /* */

if (make_rod_hb)    connect_types (xrod,hb, nalt_hb, make_hb_rod, dyadc=0); /* */

if (make_cone_ha)   connect_types (xcone,ha, make_ha_cone, make_ha_dbp1); /* */

if (make_cone_hb)   connect_types (xcone,hb, 0, make_hb_cone, dyadc=0); /* */
...
if (make_dbp1_ams)   connect_types (dbp1,ams); /* */
...
if (make_dbp1_sbac)  connect_types (dbp1,sbac); /* */

if (make_dbp2_sbac)  connect_types (dbp2,sbac); /* */

if (make_dbp3_sbac)  connect_types (dbp3,sbac); /* */

if (make_dbp4_sbac)  connect_types (dbp4,sbac); /* */
...
if (make_sbac_dbp1)  connect_types (sbac,dbp1); /* makercr=0 because sbac outputs made from distal dends */
						/*        which have no bp inputs, */
if (make_sbac_dbp2)  connect_types (sbac,dbp2); /* */

if (make_sbac_dbp3)  connect_types (sbac,dbp3); /* */

if (make_sbac_dbp4)  connect_types (sbac,dbp4); /* */

if (make_sbac_dsgc)  connect_types (sbac,dsgc); /* */

if (make_sbac_sbac)  connect_types (sbac,sbac,make_rcr=0,dyadc=0);   /* */
...
if (make_ams_sbac)   connect_types (ams,sbac); /* */
...
if (make_am_dsgc)   connect_types (am,dsgc); /* */

if (make_am2_dsgc)   connect_types (am2,dsgc); /* */
...
(*addsyns)();		/* User defined additions of synapses */
			/*  defined in the experiment file */
...
/* check amacrine cells */
 ...
 if (getn(ams,NMADE)>0) {		/* check small-field amacrine connections */
   ...
   if (make_dbp1 && make_dbp1_ams) checkcellin(ams,dbp1);
   if (make_sbac && make_ams_sbac) checkcellout(ams,sbac);
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[ams]);
   setn(ams,NMADE,ncells[ams]-ncell_erased[ams]);
 }
 ...
 if (getn(amhs,NMADE)>0) {		/* check small-field amacrine connections */
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[amhs]);
   setn(amhs,NMADE,ncells[amhs]-ncell_erased[amhs]);
 }
 ...
 if (getn(amh,NMADE)>0) {		/* check wide-field amacrine connections */
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[amh]);
   setn(amh,NMADE,ncells[amh]-ncell_erased[amh]);
 }
 if (getn(am,NMADE)>0) {		/* check wide-field amacrine connections */
   if (ninfo>=2) ncfprintf (stderr,"# %s's ",cname[am]);
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[am]);
   setn(am,NMADE,ncells[am]-ncell_erased[am]);
 }
 if (getn(am2,NMADE)>0) {		/* check wide-field amacrine connections */
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[am2]);
   setn(am2,NMADE,ncells[am2]-ncell_erased[am2]);
 }
 if (getn(am3,NMADE)>0) {		/* check wide-field amacrine connections */
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[am3]);
   setn(am3,NMADE,ncells[am3]-ncell_erased[am3]);
 }
 if (getn(am4,NMADE)>0) {		/* check wide-field amacrine connections */
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[am4]);
   setn(am4,NMADE,ncells[am4]-ncell_erased[am4]);
 }
 if (getn(amh2,NMADE)>0) {		/* check wide-field amacrine connections */
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[amh2]);
   setn(amh2,NMADE,ncells[amh2]-ncell_erased[amh2]);
 }
 ...
 if (getn(sbac,NMADE)>0) {			/* check starburst connections */
   ...
   if (make_dsgc && make_sbac_dsgc) { 
	  if (make_sbac_sbac) checkcellout(sbac,sbac,sbac,dsgc);
	  else checkcellout(sbac);
   }
   else if (nsbac>1 && make_sbac_sbac) checkcellout(sbac,sbac);
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[sbac]);
   setn(sbac,NMADE,ncells[sbac]-ncell_erased[sbac]);
 }
 ...
 if (getn(aii,NMADE)>0) {		/* check aii amacrine connections */
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[aii]);
   setn(aii,NMADE,ncells[aii]-ncell_erased[aii]);
 }
 if (getn(a17,NMADE)>0) {		/* check a17 amacrine connections */
   ...
   if (ninfo >= 2) ncfprintf (stderr," %d erased\n",ncell_erased[a17]);
   setn(a17,NMADE,ncells[a17]-ncell_erased[a17]);
 }

 /* check cone bipolar cells */
 ...
 if (getn(dbp1,NMADE)>0) {		/* check on-bipolar connections */
   ...
   if (make_gca && make_dbp1_gca && !made_gc_comps) {
	  ...
	  if (make_dbp1_dbp1) checkcellout(dbp1,dbp1,gca);
	  else checkcellout(dbp1);
   ...
   if (make_dsgc && make_dbp1_dsgc) {
      ...
      // else checkcellout(dbp1);
      checkcellout(dbp1);
      // if (make_sbac & make_dbp1_sbac && make_sbac_sbac && make_sbac_dsgc) checkcellout(dbp1,sbac,sbac,dsgc);
   }
   else if (make_sbac) {
	  // if (make_dbp1_sbac) checkcellout(dbp1,sbac);
	  if      (make_dbp1_sbac && !make_sbac_dbp1) checkcellout (dbp1,sbac);
	  else if (!make_dbp1_sbac && make_sbac_dbp1) checkcellin  (dbp1,sbac);
	  else if (make_dbp1_sbac && make_sbac_dbp1)  checkcellout (dbp1);	// must have some output
   }
   if (make_ams) {						/* after checking dbp1's check ams's */
	if (ninfo>=2) ncfprintf (stderr,"\n# %s's ",cname[ams]);
	if (make_dbp1_ams && make_ams_sbac) checkcell_in_or_out(ams,dbp1,sbac);
	if (ninfo >= 2) {
	     if (ncell_erased[ams]>0) ncfprintf (stderr,"%d erased\n#",ncell_erased[ams]);
        }
	setn(ams,NMADE,ncells[ams]-ncell_erased[ams]);
	if      (make_dbp1_ams) checkcellout (dbp1);
   }
   if (ninfo >= 2) {
	   if (ncell_erased[dbp1]>0) 
	        ncfprintf (stderr,"\n# dbp1s %d erased\n",ncell_erased[dbp1]);
   }
   setn(dbp1,NMADE,ncells[dbp1]-ncell_erased[dbp1]);
 }
 ...
 if (getn(dbp2,NMADE)>0) {		/* check on-bipolar connections */
   ...
   if (make_gca && make_dbp2_gca) {
	  if (make_dbp2_dbp2) checkcellout(dbp2,dbp2,gca);
	  else checkcellout(dbp2);
   ...
   if (make_dsgc & make_dbp2_dsgc) {
      ...
      // if (make_sbac & make_dbp2_sbac && make_sbac_sbac && make_sbac_dsgc) checkcellout(dbp2,sbac,sbac,dsgc);
   }
   ...
   setn(dbp2,NMADE,ncells[dbp2]-ncell_erased[dbp2]);
 }
 ...
 if (getn(dbp3,NMADE)>0) {		/* check on-bipolar connections */
   ...
   if (make_gca && make_dbp3_gca) {
	  if (make_dbp3_dbp3) checkcellout(dbp3,dbp3,gca);
	  else checkcellout(dbp3);
   ...
   setn(dbp3,NMADE,ncells[dbp3]-ncell_erased[dbp3]);
 }
 ...
 if (getn(dbp4,NMADE)>0) {		/* check on-bipolar connections */
   ...
   if (make_gca && make_dbp4_gca) {
	  if (make_dbp4_dbp4) checkcellout(dbp4,dbp4,gca);
	  else checkcellout(dbp4);
   ...
   setn(dbp4,NMADE,ncells[dbp4]-ncell_erased[dbp4]);
 }
 ...
 if (getn(hbp1,NMADE)>0) {		/* check off-bipolar connections */
   ...
   if (make_gcaoff && make_hbp1_gcaoff) {
	  if (make_aii && make_aii_hbp1) {
          ...
          if (!make_aii) {
	      if (make_hbp1_hbp1) checkcellout(hbp1,hbp1,gcaoff);
	      else checkcellout(hbp1);
	  }
   }
   ...
   setn(hbp1,NMADE,ncells[hbp1]-ncell_erased[hbp1]);
 }
 ...
 if (getn(hbp2,NMADE)>0) {		/* check off-bipolar connections */
   ...
   if (make_gcaoff && make_hbp2_gcaoff) {
	  if (make_aii && make_aii_hbp2) {
          ...
          if (!make_aii) {
	      if (make_hbp2_hbp2) checkcellout(hbp2,hbp2,gcaoff);
	      else checkcellout(hbp2);
	  }
   }
   ...
   setn(hbp2,NMADE,ncells[hbp2]-ncell_erased[hbp2]);
 }
...
/* next check rod bipolars */
 ...
 if (getn(rbp,NMADE)>0) {		/* check rod bipolar connections */
   ...
   setn(rbp,NMADE,ncells[rbp]-ncell_erased[rbp]);
 }
 ...
 if (getn(ha,NMADE)>0) {		/* check ha connections */
   ...
   // if (make_cones && make_cone_ha) checkcellin(ha,xcone);
   ...
   setn(ha,NMADE,ncells[ha]-ncell_erased[ha]);
 }
 ...
 if (getn(hb,NMADE)>0) {		/* check hb connections */
   ...
   // if (make_cones && make_cone_hb) checkcellin(hb,xcone);
   ...
   setn(hb,NMADE,ncells[hb]-ncell_erased[hb]);
 }
 ...
 if (getn(xcone,NMADE)>0) {		/* check cone connections */
   ...
   setn(xcone,NMADE,ncells[xcone]-ncell_erased[xcone]);
 }

 if (getn(xrod,NMADE)>0) {		/* check rod connections */
   if (ninfo>=2) ncfprintf (stderr,"# %ss ",cname[xrod]);
   if ((make_rbp && make_rod_rbp)     ||
       (make_hb && make_rod_hb)       ||
       (make_hbat && make_rod_hbat)   ||
       (make_cones && make_cone_rod)) checkcellout(xrod);
   if (ninfo >= 2) {
	if (ncell_erased[xrod]>0) ncfprintf (stderr,"\n# rods ");
   	ncfprintf (stderr,"%d erased\n",ncell_erased[xrod]);
   }
   setn(xrod,NMADE,ncells[xrod]-ncell_erased[xrod]);
 }
...
if (ninfo>=2) {
	  if (dispsize >= 1e3) ncfprintf (stderr,"# dispsize %3.4g um\n",dispsize);
          else                 ncfprintf (stderr,"# dispsize %3.3g um\n",dispsize);
}
...
if (notinit(disp_calib_len)) {
   disp_calib_len = dispsize/7; 
   disp_calib_len = set_int_val(disp_calib_len);
}

if (notinit(disp_calib_x)) disp_calib_x = 0.97;
if (notinit(disp_calib_y)) disp_calib_y = 0.05;

if (disp_calib_len > 0) disp_calib (disp_calib_x,disp_calib_y,disp_calib_len,dispsize,white);
...
if (make_cones) {
   ...
   if (make_rods && make_cone_rod) dispcelltype(xcone,1,ncones,xrod,blue,0, scal=0.8,nscal);
   else                            dispcelltype(xcone,1,ncones,scal=0.8,nscal);
}
...
if (make_hb) {
   ...
   if (make_hb_hb) dispcelltype(hb,1,nhb,hb,41,9, scal=0.8,nscal);
   else            dispcelltype(hb,1,nhb,scal=1,nscal);
}
