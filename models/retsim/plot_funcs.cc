...
double spike_plot(double cct, double cnn, double nodenum, double thresh, double time);
...
#define SPIKTHRESH (-0.04)
  ...
  if ((npnt=nde(ct,cn,n))==NULL) return;
   ...
   plot_func((double(*)(double, double, double, double, double))spike_plot,
		   ct,cn,n,thresh=SPIKTHRESH,fmax=500,fmin=0);
...
void plot_synrate(synapse *s, int prate, double rmin, double rmax, int pves, double fmin, double fmax, int pmglu, double mmin, double mmax, int pcond, double cmin, double cmax, int pcolor, int plotnum, const char *plname, double plsize)

{
    int t, ct, cn, nod, col;
    elem *cpnt;
    char name[CSIZ], vesnam[CSIZ], mglunam[CSIZ], condnam[CSIZ];
    attrib *apnt;
    synapse *tc;
    ...
    if (plname==NULL || streq(plname,"")) {
        ...
        sprintf(mglunam, "mglur2 %s_%d_%d",cname[ct],cn,nod);
        sprintf(condnam, "G%s_%d_%d",      cname[ct],cn,nod);
    } else {
        sprintf(name,   "%s", plname);
        sprintf(vesnam, "%s", plname);
        sprintf(mglunam, "%s",plname);
        sprintf(condnam,"%s", plname);
    }
    ...
    if (pmglu) { plot(FD1,s->elnum,mmax,mmin); plot_param(mglunam,col,plotnum,plsize); }
    if (pcond) {
       if (tc==NULL) {
...

{
   ...
   double mmin, mmax;
    ...
    plot_synrate(s, prate, rmin, rmax, pves, fmin, fmax, pmglu=0, mmin=0, mmax=0, pcond, cmin, cmax, pcolor, plotnum, plname, plsize);

}

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

void plot_synratep(synapse *s, int prate, double rmin, double rmax, int pves, double fmin, double fmax, int pmglu, double mmin, double mmax, int pcond, double cmin, double cmax, int pcolor, int plotnum, const char *plname, double plsize)
...

{
    ...
    if (plname==NULL || streq(plname,"")) {
        ...
        sprintf(mglunam,"mglur2 %s_%d %s_%d",cname[ct],nod,cname[ct2],nod2);
...

{
   ...
   double mmin, mmax;
    ...
    plot_synratep(s, prate, rmin, rmax, pves, fmin, fmax, pmglu=0, mmin=0, mmax=0, pcond, cmin, cmax, pcolor, plotnum, plname, plsize);
...
{
   int prate,pves,pcond,pmglu;

    plot_synrate(s, prate=0, 0, 0, pves=0, 0, 0, pmglu=1, mmax, mmin, pcond=0, 0, 0, pcolor, plotnum, plname, plsize);

}

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

void plot_mglur2p(synapse *s, double mmin, double mmax, int pcolor, int plotnum, const char *plname, double plsize)

{
   int prate,pves,pcond,pmglu;

    plot_synratep(s, prate=0, 0, 0, pves=0, 0, 0, pmglu=1, mmax, mmin, pcond=0, 0, 0, pcolor, plotnum, plname, plsize);
  ...
  if (script==0) {
    ...
    if (plname==NULL || streq(plname,"")) {
	   ...
	   sprintf(name,   "rate %s_%d_%d",    cname[ct],cn,nod);
           ...
           sprintf(mglunam, "mglur2 %s_%d",cname[ct],cn);
...

{
   ...
   double mmin, mmax;
    ...
    plot_synrate(ct, cn, nod, prate, rmin, rmax, pves, fmin, fmax, pmglu=0, mmin=0, mmax=0, pcond, cmin, cmax, pcolor, plotnum, plname, plsize);
...
{
     int prate,pves,pcond,pmglu;
   plot_synrate(ct, cn, nod, prate=0, 0, 0, pves=0, 0, 0, pmglu=1, mmin, mmax, pcond=0, 0, 0, pcolor, plotnum, plname, plsize);
}
...

{
  ...
  if (script==0) {
    ...
    if (!t) return;
    if (plname) sprintf(name,"%s",plname);
    else        sprintf(name,"rate %s_%d",cname[ct],cn);
    if (plname) sprintf(vesnam,"%s",plname);
    else        sprintf(vesnam,"ves  %s_%d",cname[ct],cn);
    if (plname) sprintf(condnam,"%s",plname);
...
synapse_list *make_synapse_list(int synlist)
{
   ...
   if (synap_rec_list[synlist].list==NULL) {
       synap_rec_list[synlist].list = (synapse **)emalloc(SLISTSIZ*sizeof(synapse *));
       synap_rec_list[synlist].index = 0;
       synap_rec_list[synlist].length = SLISTSIZ;
   }
   if (synap_rec_list[synlist].list==NULL) {
...
synapse_list *lengthen_slist (synapse_list *s)
{
     int i, new_length;
     synapse **tmp;
     synapse_list *retval;
   ...
   else if (s->list==NULL) retval = NULL;
   else if (s->index >= s->length) {
       new_length = s->length + SLISTSIZ;
       tmp = (synapse **)emalloc(new_length *sizeof(synapse *));
   ...
   if (retval==NULL) ncfprintf (stderr,"lengthen_slist: not enough space\n");
   return (retval);
}
   ...
   for (nsyn=0,epnt=elempnt; epnt=foreach(epnt,SYNAPSE,ct,cn,nod,ct2,cn2,-1,0); epnt=epnt->next) {
     ...
     //             epnt->node2a,epnt->node2b,epnt->node2c); /* */
      ...
      if (cn2 < 0 || epnt->node2b==cn2) {
         ...
         nsyn++;
         if (++s->index >= s->length) {
		if (lengthen_slist(s)==NULL) break;
	 }
      } else continue;
   }
   return nsyn;
...
/*  Originally from expt_dsgc_sbac_bar.cc
   ...
   usage:  synapse_add (1, sbac, 1, 1215, sbac, 2, 10, 10);          // add synapse for rate plot
*/ 
...
/*  Originally from expt_dsgc_sbac_bar.cc
   ...
   usage:  synapse_add (1, sbac, 1, 1215, sbac, 2, 10, 10, 120, 0);    // add synapse for rate plot
*/ 
...
/*  Originally from expt_dsgc_sbac_bar.cc
   ...
   usage:  synapse_add (1, sbac, 1, 1215, sbac, 2, 4);          // add synapse for rate plot
*/ 
...
/* Make array of synapse elements for recording at runtime, 
 * within disto of absolute location (x,y) from onto ct2, cn2. */

/* Synlist defines which list */

/*  Originally from expt_dsgc_sbac_bar.cc
 
   usage:  synapse_add (1, sbac, 1, 1215, sbac, 2, 120, 0, 20);          // add synapse for rate plot
*/ 
   ...
   for (nsyn=0,epnt=elempnt; epnt=foreach(epnt,SYNAPSE,ct,cn,nod,ct2,cn2,-1,0); epnt=epnt->next) {
	 ...
	 xr = epnt->nodp2->xloc - x;
	 yr = epnt->nodp2->yloc - y;
         dist = sqrt(xr*xr + yr*yr);
	 if (dist <= disto) {		// if within distance limits 
		 // fprintf (stderr,"syn loc %g %g\n",epnt->nodp2->xloc,epnt->nodp2->yloc);
             s->list[s->index] = (synapse *)epnt;	// save pointer to synapse element
             nsyn++;
             if (++s->index >= s->length) {
		    if (lengthen_slist(s)==NULL) break;
...
double rsyn_avg (double sl, double time)
{
   ...
   synlist = int(sl);
   if (synlist>=SLISTLEN) synlist = SLISTLEN-1;
