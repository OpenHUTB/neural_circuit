...
extern int node_color;			/* node color in nc/src/control.h */
...
void setmorphfiles(void)
{
  ...
  anatfiles [0][xrod]  = (char *)"";		/* set the names */
  anatfiles [0][xcone] = (char *)"";
  anatfiles [0][hbat]  = (char *)hbat_file;
  anatfiles [0][ha]   = (char *)ha_file;
  anatfiles [0][hb]   = (char *)hb_file;
  anatfiles [0][rbp]  = (char *)rbp_file;
  anatfiles [0][dbp1] = (char *)dbp1_file;
  anatfiles [0][dbp2] = (char *)dbp2_file;
  anatfiles [0][dbp3] = (char *)dbp3_file;
  anatfiles [0][dbp4] = (char *)dbp4_file;
  anatfiles [0][hbp1] = (char *)hbp1_file;
  anatfiles [0][hbp2] = (char *)hbp2_file;
  anatfiles [0][aii]  = (char *)aii_file;
  anatfiles [0][sbac] = (char *)sbac_file;
  anatfiles [0][am]   = (char *)am_file;
  anatfiles [0][am2]  = (char *)am2_file;
  anatfiles [0][am3]  = (char *)am3_file;
  anatfiles [0][am4]  = (char *)am4_file;
  anatfiles [0][amh]  = (char *)amh_file;
  anatfiles [0][amh2] = (char *)amh2_file;
  anatfiles [0][ams]  = (char *)"";
  anatfiles [0][amhs] = (char *)"";
  anatfiles [0][a17]  = (char *)a17_file;
  anatfiles [0][gca]  = (char *)gca_file;
  anatfiles [0][gcb]  = (char *)gcb_file;
  anatfiles [0][dsgc] = (char *)dsgc_file;
  anatfiles [0][gcaoff] = (char *)gcaoff_file;
  anatfiles [0][gcboff] = (char *)gcboff_file;

  anatfiles [1][sbac] = (char *)sbac_file2;
}
  ...
  for (ct=0; ct<nceltypes; ct++) {      /* get density file names */
    if (getn(ct, MAKE)) {
       if ((morph=int(getn(ct,MORPH)))==0) {
         if (anatfiles[0][ct]!="") {
                 ncfprintf (filout,"#   %-5s morph:       %-8s", cname[ct],anatfiles[0][ct]);
	 }
         else {
       ...
       ncfprintf (filout,"\n");
       if ((morph=int(getn(ct,MORPH)))==0) {
          if (anatfiles[1][ct]!=NULL) 
             ncfprintf (filout,"#   %-5s morph2:      %-8s\n", cname[ct],anatfiles[1][ct]);
       }
    };
  };
...

{
 ...
 if (notinit(first_center)) first_center = 0;
  ...
  if (notinit(morph_frac)) morph_frac = 0;	/* set morphology file to first morphology */

      /* Set up random state for morphfil, and makcell()  */
      /*  note that this also sets random state for all subroutines: drand(), rrange() */

  if (anatfiles[1][ct] != NULL) {		/* check to see if 2nd morphology file is set */
      mrseed = noduniq(ct,ncells,0,0, rseed);
      mstate = (char *)makrand (RNDSIZ,"setupcells",ct);
      if (mstate) {
	  ncinitstate(mrseed,mstate,RNDSIZ);
	  drand_setstate(mstate);
      }
  }
  ...
  if (ncells==1) {
    ...
    if (anatfiles[1][ct] != NULL) {		/* check to see if 2nd morphology file is set */
        morphfil = (drand() < morph_frac);	/* find random morph with prob morph_frac */
    } else morphfil = 0;

    if (ninfo>=3)
        ncfprintf (stderr,"# %s #%d, r=%.3g m=%d\n",
                cname[ct],cn,thetaz,morphfil);

    if (syarr !=NULL)  makcell (ct, cn, morphfil, xarr[i], yarr[i],thetay,thetaz,flip);
    else	       makcell (ct, cn, morphfil, arrcentx, arrcenty,thetay,thetaz,flip);
  }
  else
    for (m=i=0; i<ncells; i++) {
      if (stharr!=NULL)
	   thetaz = stharr[i];
      ...
      if (ninfo>=3)
        ncfprintf (stderr,"# %s #%2d, x= %5.3g  y= %5.3g r= %4.3g  m=%d\n",
                cname[ct],cn,xarr[i],yarr[i],thetaz,morphfil);

      makcell (ct, cn, morphfil, xarr[i], yarr[i],thetay,thetaz,flip);
  }
  if (mstate) restorstate();

  if (ninfo>=2 && anatfiles[1][ct]!=NULL)
        ncfprintf (stderr,"# %s total %4d %s %4d %s\n", cname[ct],ncells-m,anatfiles[0][ct], m, anatfiles[1][ct]);
...
int setupcells(int ct,int ncells, double lxarrsiz, double lyarrsiz, int first_center) 

{
    double larrcentx, larrcenty;

   if (notinit(arrcentx)) larrcentx = 0; 
   else                   larrcentx = arrcentx;
   if (notinit(arrcenty)) larrcenty = 0;
   else                   larrcenty = arrcenty;

 return setupcells(ct, ncells, lxarrsiz, lyarrsiz, larrcentx, larrcenty, NULL, NULL, NULL, NULL, NULL, first_center);
}
...
{
 return setupcells(ct, ncells, lxarrsiz, lyarrsiz, 0);
}
...
int setupcells(int ct,int ncells, double lxarrsiz, double lyarrsiz, 
				double arrcentx, double arrcenty, int first_center) 

{
 return setupcells(ct, ncells, lxarrsiz, lyarrsiz, arrcentx, arrcenty, NULL, NULL, NULL, NULL, NULL, first_center);
}
...
/* first_center=1 => sets first cell at (0,0) */

int setupcells(int ct,int ncells, int first_center) 

{
 return setupcells(ct, ncells, LARGENUM, LARGENUM, arrcentx, arrcenty, NULL, NULL, NULL, NULL, NULL, first_center);
}
...
{
     node *np,*npn;
     static int conn_cnt1;
     static int conn_cnt2;
     static int conn_cnt3;
     static int oldct=0;
     static int t;
...
/* Erase cells that have no inputs and no outputs */

{
    int cn;
    int ncell_in,ncell_out;
    node *npnt,*np,*tnpnt;
 ...
 for (npnt=nodepnt; npnt=foreach (npnt, ct,-1,soma, NULL,&cn,NULL); npnt=tnpnt) {
    ...
    if (ninfo >= 3) ncfprintf (stderr,"checkcellin_out: %s %d: %d %d\n",cname[ct],cn,ncell_in,ncell_out);
    if (ncell_in == 0 && ncell_out == 0) {
       erase_cell(ct,cn);			/* erase unconnected cells */
    }
  }
}
...
/* Erase cells that have no inputs from one type and no outputs to another type */
...
/* Erase cells that have no inputs or no outputs */
...
/* Erase cells that have no inputs from one type or outputs to another type */
...
{
  rcolors = celdens[ct][ndens[ct][cn]][_COL];
}
...
void displaycell(int ct, int cn, int colval, int cmap, int ct2, int colval2, int cmap2, double scal, double nscal)
{
    int color,ns,d,cmapp;
    double dscale;


    ns = int(nscal);
    display (SYNAPSE,MATCHING,ndt(ct,cn,-1), color=magenta,cmapp=0,dscale=synap_scale);    /* show synapses magenta*/
    display (SPHERE,MATCHING, ndt(ct,cn,-1), color=colval, cmap,   dscale=scal);
    display (CABLE,MATCHING,  ndt(ct,cn,-1), color=colval, cmap,   dscale=scal);
    if (ct2 >= 0) {
           display (GJ,MATCHING, ndt(ct,cn,-1), ndt(ct,-1,-1),  color=colval,  cmap,  dscale=gj_scale);
           display (GJ,MATCHING, ndt(ct,cn,-1), ndt(ct2,-1,-1), color=colval2, cmap2, dscale=gj_scale);
    } else display (GJ,MATCHING, ndt(ct,cn,-1), color=colval2, cmap2,  dscale=gj_scale);

    display (ROD,MATCHING,    ndt(ct,cn,-1), color=colval, cmap,   dscale=scal);
    display (CONE,MATCHING,   ndt(ct,cn,-1), color=colval, cmap,   dscale=scal);
    display (COMPS,MATCHING,  ndt(ct,cn,-1), color=1,      cmapp=0,dscale=scal);
    if (ns<0) {
      switch (ns) {
       case -1: display (NODE, MATCHING,  ndt(ct,cn,soma), node_color, dscale=nscal); break;
       case -2: display (NODE, MATCHING,  ndt(ct,cn,soma), node_color, dscale=nscal); break;
       case -3: display (NODE, MATCHING,  ndt(ct,cn,-1),   node_color, dscale=nscal); break;
       case -4: display (NODE, MATCHING,  ndt(ct,cn,-1),   node_color, dscale=nscal); break;
       case -5: display (NODE, MATCHING,  ndt(-1,-1,soma), node_color, dscale=nscal); break;
       case -6: display (NODE, MATCHING,  ndt(ct,cn,-1),   node_color, dscale=nscal); break;
       case -7: display (NODE, MATCHING,  ndt(ct,cn,-1),   node_color, dscale=nscal); break;
       case -9: break;
       default:
                display (NODE, MATCHING, ndt(ct,cn,soma), node_color, dscale=nscal); /* display soma node */
      }
    } 
    else display (NODE, MATCHING, ndt(ct,cn,-1), node_color, dscale=nscal);  /* positive nscal */
    // dscale = abs(nscal);
    // d = int(dscale);
    ...
    // display (LABEL, MATCHING, ndt(ct,cn,-1), color=white, dscale);  /* always display labels */
      ...
      switch (ns) {
       case -1: display (LABEL, MATCHING,  ndt(ct,cn,soma), node_color, dscale=nscal); break;
       case -2: display (LABEL, MATCHING,  ndt(ct,cn,soma), node_color, dscale=nscal); break;
       case -3: display (LABEL, MATCHING,  ndt(ct,cn,-1),   node_color, dscale=nscal); break;
       case -4: display (LABEL, MATCHING,  ndt(ct,cn,-1),   node_color, dscale=nscal); break;
       case -5: display (LABEL, MATCHING,  ndt(-1,-1,soma), node_color, dscale=nscal); break;
       case -6: display (LABEL, MATCHING,  ndt(ct,cn,-1),   node_color, dscale=nscal); break;
       case -7: display (LABEL, MATCHING,  ndt(ct,cn,-1),   node_color, dscale=nscal); break;
       case -9: break;
       default:
...
void displaycell(int ct, int cn, int colval, int cmap, double scal, double nscal)

{
      int ct2, colval2, cmap2;

  displaycell(ct, cn, colval, cmap, ct2 = ct, colval2=brown, cmap2=0, scal, nscal);
}
...

{
 ...
 if (disp>0 && disp<DMOVIE)
  for (i=start; i<=stop; i++) {
    cmap=int(getn(ct,NCMAP));
    if ((colval=int(getn(ct,NCOLOR)))<0) {cmap=0;  colval = i % brtwht + 0;}
    ...
    if (!notinit(disp_ct)) {		/* disp_ct and disp_cn override node displays */
	 ...
	 if (ct==disp_ct && i==disp_cn) 
	    displaycell(disp_ct,i,colval,cmap,scal,nscal);
      }
      else if (ct==disp_ct) 
	    displaycell(disp_ct,i,colval,cmap,scal,nscal);
    }
    else {  displaycell(ct,i,colval,cmap,scal,nscal);
    }
  }
...

{
 ...
 if (disp>0 && disp<DMOVIE)
  for (i=start; i<=stop; i++) {
    ...
    if ((colval=int(getn(ct,NCOLOR)))<0) {cmap=0;  colval = i % brtwht + 0;}
    if (colval == RCOLOR) set_rcolors(ct,i);		    /* set region colors */
    displaycell(ct,i,colval,cmap,ct2,colval2,cmap2,scal,nscal);
  }
  unset_rcolors();					/* unset region colors */
...
//printf ("loc %g %gi\n", xden, yden);
     ...
     celnode[ctype][cn] = taperdistden (ctype,cn,0,celnode[ctype][cn],
				diabs,diaspc,diabs,0,DEND_PROX, celnode[ctype][cn],soma);
	...
	if (shortlen+leng > rad) shortlen = rad - leng;
        ...
        celnode[ctype][cn] = taperdistden(ctype,cn,sn,celnode[ctype][cn],
				diabs,diaspc,diabs,0,DEND_DIST,celnode[ctype][cn],soma);
...
void mak_real_cel(int ct, int cn, int morphfil, double xpos, double ypos, double thetax, double thetay, double thetaz, int flip, int dend1, int dend2, int dend3, int dend4, int dend5, double dend_cplam, int taper)
...

{
    ...
    int p, par, par_indx;
    double somaoffsetx=0, somaoffsety=0, somaoffsetz=0, somaz;
  ...
  if      (morphfil==0) {
      arbor_scale = getn(ct,ARBSCALE);	     /* xy scale for dend tree */
       dia_factor = getn(ct,DENDDIA);	     /* dend dia (thickness) factor */
            somaz = getn(ct,SOMAZ);	     /* soma Z loc */
  } 
  else if (morphfil==1) {		     /* morphology parameters for 2nd morph */
      arbor_scale = getn(ct,ARBSCALE2);	     /* xy scale for dend tree */
       dia_factor = getn(ct,DENDDIA2);	     /* dend dia (thickness) factor */
            somaz = getn(ct,SOMAZ2);	     /* soma Z loc */
  ...
  if (fp = fopen(anatfiles[morphfil][ct], "r")) {   /* Check for user-specified anatomy file */
    anatfname = anatfiles[morphfil][ct];
    aflag = 0;
    fclose(fp);
  } else {

    //prepend confdir, check again
    tanatfname = emalloc(min(FILNAMSIZ,strlen(confdir)) + min(FILNAMSIZ,strlen(anatfiles[morphfil][ct])) + 2);
    aflag = 1;
    sprintf(tanatfname, "%.50s/%.50s", confdir, anatfiles[morphfil][ct]);
  ...
  for (i=0; i<clines; i++) {				/* locate nodes first */
    ...
    np = loc(nd(ct,cn,nod),xpos+dx,ypos+dy,somaz+dz,region,dend); /*locate the nodes */
   ...
   /*   but only for sb1 for now. */
    ...
    if (ct==sbac) {
      ...
      if (notinit(dia_prox_factor)) dia_prox_factor = 1;
    ...
    if ((region==SOMA)      || 
	  ...
	  d_factor = 1.0;       
    else  d_factor = dia_factor;  /* scale dendrites only, note that R1-R10 are included */

    if (ninfo >=5) fprintf(stderr, "# Mak_real_cel %d %d %d region %d dia_factor %g d_factor %g\n", 
		    			ct, cn, nod, region, dia_factor, d_factor);
...
int make_axon_branch (int ct, int cn, int startnode, int newnode, double zoffs, double zlen, double angle, double zangle,
			double brdia, double len, double vdia, int nsegs, int reg1, int reg2, double cplam) 

{
  ...
  for (i=1; i<=nsegs; i++) {
      ...
      make_celseg (ct, cn, parent,  newnode, brdia, brdia, reg1, cplam); /* make 1 seg of axon branch */
      if (vdia>0)
	make_celseg (ct, cn, newnode, newnode, vdia,  0,   reg2, cplam); /* make varicosity */
      parent = newnode++;
  } 
  return parent;
}
...
{
   int i,j,sn,pd,dendr,nsegs,naxbr,axnode,oldnode,naxseg;
   int next,next2;
   int lob_append;
  ...
  switch (morph) {
    ...
    case MORPH_REAL:  	 	  /* realistic morphology */
      ...
      mak_real_cel(ct,cn,0,xpos,ypos,thetax,thetay,thetaz,flip,
		    bp_dend1,bp_dend1,bp_dend1,bp_dend1,bp_dend1,cplam,taper);
      break;

    case MORPH_A1: 		/* bp-like artif morphology, soma, axon, and dendrite */
      ...
      np = loc(nd(ct,cn,dendr), xpos,ypos,getn(ct,DENDARBZ));
      make_celseg (ct, cn, soma, dendr, cd1=0.5 ,cd2=0.3, DEND,cplam); /* connect to soma */
      celnode[ct][cn] = 2;
      break;
  
    case MORPH_A2: 		/* just soma and axon, dendrites grow, artif morphology */
      ...
      make_celseg (ct, cn, soma, axtrm, cd1=0.5 ,cd2=0.4, AXON,cplam); /* connect to soma */
      make_celseg (ct, cn, axtrm, axtrm, cd1=1,cd2=0, AXON,cplam);   /* sphere at tip */
      np = loc(nd(ct,cn,dendr), xpos,ypos,getn(ct,SOMAZ)+0.1);     /* prevents bug */
      make_celseg (ct, cn, soma, dendr, cd1=0.1 ,cd2=0.1, DEND,cplam); /* connect to soma */
      celnode[ct][cn] = dendr;
      break;

    case MORPH_A3: 		/* just soma and axon, dendrites grow, artif morphology */
				/* make 4 axon branches */
      np = loc(nd(ct,cn,soma), xpos,ypos,getn(ct,SOMAZ));
      make_celseg (ct, cn, soma, soma, cd1=getn(ct,SOMADIA),cd2=0,SOMA,cplam);
      np = loc(nd(ct,cn,axtrm), xpos,ypos,getn(ct,AXARBZ));	       /* just above axtrm */
      make_celseg (ct, cn, soma, axtrm, cd1=0.5 ,cd2=0.4, AXON,cplam); /* connect to soma */

      seglen = 5; cplam = 0.5;
      np = loc(nd(ct,cn,axtrm+1), xpos+seglen,ypos,getn(ct,AXARBZ));
      make_celseg (ct, cn, axtrm, axtrm+1, cd1=0.2 ,cd2=0.2, AXON,cplam); /* connect to prox ax */

      np = loc(nd(ct,cn,axtrm+2), xpos-seglen,ypos,getn(ct,AXARBZ));
      make_celseg (ct, cn, axtrm, axtrm+2, cd1=0.2 ,cd2=0.2, AXON,cplam); /* connect to soma */

      np = loc(nd(ct,cn,axtrm+3), xpos,ypos+seglen,getn(ct,AXARBZ));
      make_celseg (ct, cn, axtrm, axtrm+3, cd1=0.2 ,cd2=0.2, AXON,cplam); /* connect to soma */

      np = loc(nd(ct,cn,axtrm+4), xpos,ypos-seglen,getn(ct,AXARBZ));
      make_celseg (ct, cn, axtrm, axtrm+4, cd1=0.2 ,cd2=0.2, AXON,cplam); /* connect to soma */

      // make_celseg (ct, cn, axtrm+1, axtrm+1, cd1=1,cd2=0, AXON,cplam);   /* sphere at tip */
      np = loc(nd(ct,cn,dendr=axtrm+5), xpos,ypos,getn(ct,SOMAZ)+0.1);     /* prevents bug */
      make_celseg (ct, cn, soma, dendr, cd1=0.1 ,cd2=0.1, DEND,cplam); /* connect to soma */
      celnode[ct][cn] = axtrm+5;
      break;

    case MORPH_A4: 		/* artif morphology with soma and segmented axon, for db4 primate */
				/*  4 axon branches, but only 1 axon terminal compartment */

      somadia = getn(ct,SOMADIA);
      axbrzoffs = 0;		// Z offset for start of branches
      somaz = getn(ct,SOMAZ);	// zloc of soma
      axarbz = getn(ct,AXARBZ);	// bottom of proximal axon

      naxbr = 4; 		// number of axon branches
      nsegs = 1;		// number of segments per branch
      seglen = 5;		// length of each segment
      start_angle = thetaz;	// angle of first branch
      axbrdia = 0.5;		// diameter of axon branches
      ...
      np = loc(nd(ct,cn,dendr=2), xpos,ypos,getn(ct,SOMAZ)+0.1);     /* prevents bug */
      make_celseg (ct, cn, soma, dendr, cd1=0.1 ,cd2=0.1, R2,cplam); /* connect to soma */
      celnode[ct][cn] = 2;

      np = loc(nd(ct,cn,axtrm), xpos,ypos,somaz-1);
      make_celseg (ct, cn, soma, axtrm, cd1=0.3, cd2=0.3, R4, cplam); /* make first prox axon, connect to soma */
      oldnode = axtrm;
      axnode = 3;
      naxseg = 5;
      axnodedist = (somaz - axarbz - axbrzoffs) / 5;  // make 6 nodes along proximal axon to just above terminal
      axloc = somaz-axnodedist;
      for (i=0; i<naxseg; i++) {
        np = loc(nd(ct,cn,axnode), xpos,ypos,axloc);
        make_celseg (ct, cn, oldnode, axnode, cd1=0.3, cd2=0.3, R4+i, cplam); /* make prox axon */
        axloc -= axnodedist;
        celnode[ct][cn] = axnode;
        oldnode = axnode++;
      ...
      if (naxbr > 1) {
         ...
         for (j=1; j<=naxbr; j++) {
            next  = make_axon_branch (ct,cn,oldnode, oldnode+j, -axbrzoffs, 0, sumangle, zangle= 0, axbrdia, seglen=5, vdia=0, nsegs=1, R9, R10, cplam);
            sumangle += dtheta;
            celnode[ct][cn] = oldnode+j;
         }
      }
      break;

    case MORPH_A5: 		/* like MORPH_A2,but regions R1-R9 like MORPH_A4
				/*   just soma and axon, dendrites grow, artif morphology */

      np = loc(nd(ct,cn,soma), xpos,ypos,getn(ct,SOMAZ));
      make_celseg (ct, cn, soma, soma, cd1=getn(ct,SOMADIA),cd2=0,SOMA,cplam);
      np = loc(nd(ct,cn,axtrm), xpos,ypos,getn(ct,AXARBZ));
      make_celseg (ct, cn, soma, axtrm, cd1=0.5 ,cd2=0.4, R7,cplam); /* connect to soma */
      make_celseg (ct, cn, axtrm, axtrm, cd1=1,cd2=0, R9,cplam);   /* sphere at tip */
      np = loc(nd(ct,cn,dendr), xpos,ypos,getn(ct,SOMAZ)+0.1);     /* prevents bug */
      make_celseg (ct, cn, soma, dendr, cd1=0.1 ,cd2=0.1, R2,cplam); /* connect to soma */
      celnode[ct][cn] = dendr;
      break;
    ...
    case MORPH_T4: 		/* artif morphology with branched axon, sparse, stratified, t4 */
      ...
      sumangle = start_angle;
      for (i=1; i<=naxbr; i++) {
         next  = make_axon_branch (ct,cn,oldnode, i*100, -axbrzoffs, 0, sumangle, zangle= -20, axbrdia, seglen=5, vdia=2.0, nsegs=1, R9, R10, cplam);
         next2 = make_axon_branch (ct,cn,next, next+1,  0, 0, sumangle, zangle= -87, axbrdia, seglen=5, vdia=2.0, nsegs=2, R9, R10, cplam);
         next  = make_axon_branch (ct,cn,next, next2+1, 0, 0, sumangle, zangle= -20, axbrdia, seglen=5, vdia=2.0, nsegs=1, R9, R10, cplam);
         next2 = make_axon_branch (ct,cn,next, next+1,  0, -1, sumangle, zangle= -87, axbrdia, seglen=5, vdia=2.0, nsegs=2, R9, R10, cplam);
         sumangle += dtheta;
    ...
    case MORPH_T8: 		/* artif morphology with branched axon, sparse, stratified, t8 */
      ...
      sumangle = start_angle;
      for (i=1; i<=naxbr; i++) {
         next  = make_axon_branch (ct,cn,oldnode, i*100, -axbrzoffs, 0, sumangle, zangle, axbrdia, seglen, vdia=2.0, nsegs, R9, R10, cplam);
         next2 = make_axon_branch (ct,cn,next, next+1, 0,  0, sumangle - brangle, zangle, axbrdia, seglen, vdia=2.0, nsegs+1, R9, R10, cplam);
         next2 = make_axon_branch (ct,cn,next, next2+1, 0, 0, sumangle + brangle, zangle, axbrdia, seglen, vdia=2.0, nsegs+1, R9, R10, cplam);
         sumangle += dtheta;
         celnode[ct][cn] += nsegs*3+2;
      }
      break;
      case MORPH_SOMA: 			/* simple sphere */
            double somadia;
      somadia = getn(ct,SOMADIA);
      np = loc(nd(ct,cn,soma),xpos,ypos,getn(ct,SOMAZ));
      make_celseg(ct,cn,soma, soma, somadia, somadia, SOMA, cplam);
      break;

      case MORPH_SIMP: 			/* simple dendrite */
          double ddia;
      np = loc(nd(ct, cn, 1),xpos,ypos,getn(ct,AXARBZ));
      make_celseg(ct, cn, 1, 1, ddia=2, ddia=2, AXOND, cplam);
      break;
...
node *generate_hz(int ctype, int cn, int nden, double xpos, double ypos, 
	double thetax, double thetay, double thetaz, int flip, 
	double dtreedia, double diabs, double diaspc, double somadia, int nregion)

{
  int i,local_soma,region,sn;
  double angl,sumangl;
  double xden, yden;
  double leng, thisangl;
  double rad,cplam;
  ...
  node *np, *somanode;
    ...
    local_soma = celnode[ctype][cn];
    np = loc(nd(ctype,cn,local_soma),xpos,ypos,getn(ctype,SOMAZ));
    somanode = np;
    make_celseg(ctype,cn,local_soma,local_soma,somadia,0,region=SOMA,cplam);
    angl = 360 / nden;
    sumangl = thetaz;
    for (i=1; i<=nden; i++) {
      thisangl = angl * rrange (0.8,1.2);
...
//printf ("loc %g %gi\n", xden, yden);
      ...
      sn = celnode[ctype][cn];		/* sequence node */   
      celnode[ctype][cn] = taperdistden (ctype,cn,local_soma,celnode[ctype][cn],
				diabs,diaspc,diabs,0, nregion, celnode[ctype][cn],local_soma);
      if (leng < rad * 0.5) {                  /* make first bifurcation */
        double shortang, shortlen, shsumang;
        double xden1, yden1;
...
{  
  int i,region;
  static int first=1;
  double dtreedia,rad,somadia,diabs,diaspc,cplam;
  double xpos_at, ypos_at, xlen, x, axon_dia;
  sphere *s;
  node *np;
  node *axnode1, *axnode2;

  dtreedia = getn(ctype,DTREEDIA);
  diabs  = getn(ctype,TAPERABS);	/* starting taper diameter */
  diaspc = getn(ctype,TAPERSPC);	/* taper space constant */
  somadia=getn(ctype,SOMADIA);
  ...
  if (morph==MORPH_REAL) { 		/* realistic morphology */
    if (notinit(den_dia)) den_dia = 0.3;   /* set dendrite thin dia in file */
    if (notinit(taper)) taper = 1;	  /* make cables with taper */
    mak_real_cel(ctype,cn,0,xpos,ypos,thetax,thetay,thetaz,flip,
		    hz_dend1,hz_dend2,hz_dend3,hz_dend4,hz_dend5,cplam,taper);
  }
  else if (morph==MORPH_A1) {
       generate_hz(ctype,  cn, nden, xpos, ypos, thetax, thetay, thetaz, flip, 
	 				dtreedia, diabs, diaspc, somadia, DEND_MED);
  }
  else if (morph==MORPH_A2 && ctype==hb) {  // make hb with axon terminal
         double at_dtreedia, at_diabs, at_diaspc, at_somadia;

   /* refs for mouse horizontal cell morphology: */
   /*  Axon length ranges from 80-200 */

   /* Hombach S, Janssen-Bienhold H, Söh G, et al 22004 Functional expression of*/
   /*  connexin57 in horizontal cells of the mouse retina. EJN 19:2633-2640. */
   
   /* He, S, Weiler R, Vaney DI (2000) Endogenous dopaminergic regulation of  */
   /*  horizontal cell coupling in the mammalian retina. JCN 418:33-40 */

   /* Weiler R, Pottek M, He S, Vaney DI (2000) Modulation of coupling between retinal */
   /*  horizontal cells by retinoic acid and endogenous dopamine. Br Res Rev. 32:121-129 */

   /* Badea TC, Nathans J (2004) Quantitative analysis of neuronal morphologies in the */
   /*   mouse retina visualized by using a genetically directed reporter. JCN 480:331-351 */


      at_dtreedia = getn(hbat,DTREEDIA);
        at_diabs  = getn(hbat,TAPERABS);	/* starting taper diameter */
        at_diaspc = getn(hbat,TAPERSPC);	/* taper space constant */
       at_somadia = getn(hbat,SOMADIA);

         axon_dia = getn(hbat,AXTIPDIA);	/* dia of axon (not tip) */
             xlen = getn(hbat,STRATDIA); 	/* length of axon */

       axnode1 = generate_hz (hb, cn, nden, xpos, ypos, thetax, thetay, -thetaz, flip, 
      	 			dtreedia, diabs, diaspc, somadia, DEND_MED);

      celnode[hb][cn]++;	/* next node is first node (soma) of hbat */
    //   celnode[hb][cn] = 0; 
    
      if (first) {
	      alt_hb_somas = (node **)emalloc(sizeof(node**)*getn(ctype,MAXNUM));
	      nalt_hb = 0;
	      first = 0;
      }
      xpos_at = cos(-thetaz/DEG) * xlen + xpos;
      ypos_at = sin(-thetaz/DEG) * xlen + ypos;
      axnode2 =  generate_hz (hb, cn, nden, xpos_at, ypos_at, thetax, thetay, -thetaz, flip, 
		      	  at_dtreedia, at_diabs, at_diaspc, at_somadia, AXOND);
      alt_hb_somas[nalt_hb++] = axnode2;
      make_celseg(ctype,cn,axnode1->nodenm3,axnode2->nodenm3,axon_dia,axon_dia,region=AXONT,cplam);
  }
  return;
}
...

{
   ...
   if      (morph==MORPH_REAL) {
              mak_real_cel (ctype,n,0,xpos,ypos,thetax,thetay,thetaz,flip,
...
//printf ("loc %g %gi\n", xden, yden);
      ...
      if (leng < rad * 0.5) {                  /* make secondary dendrite */
	   mak_am_branch (ctype,n,sn, leng,angl,sumangl,xden,yden,2,edia);
        ...
        double somadia = getn(ctype,SOMADIA);
    np = loc(nd(ctype,n,soma),xpos,ypos,getn(ctype,SOMAZ));
    make_celseg(ctype, n, soma, soma, somadia, somadia, SOMA, cplam);
  }
  else if (morph==MORPH_SIMP) {		/* simple varicosity */
      double vdia;
    np = loc(nd(ctype,n,1),xpos,ypos,getn(ctype,DENDARBZ));
    make_celseg(ctype, n, 1, 1, vdia=2, vdia=2, VARICOS, cplam);
  }
...

{
  ...
  if (morph==MORPH_REAL) { 		/* realistic gc morphology */
    mak_real_cel(ctype,n,0,xpos,ypos,thetax,thetay,thetaz,flip,gc_dend1,
    ...
    celnode[ctype][n] = taperdistden (ctype,n,0,pd,diabs,diaspc,diabs,0,DEND_PROX,pd,soma);
...
*/
  ...
  else if (morph==MORPH_SIMP) {			/* simple dendrite */
      double ddia;
    np = loc(nd(ctype,n,1),xpos,ypos,getn(ctype,DENDARBZ));
    make_celseg(ctype, n, 1, 1, ddia=2, ddia=2, DEND, cplam);
  }
  else if (morph==MORPH_SIMP2) {		/* simple straight cable */
      double somadia;
...
void makcell (int ctype, int cn, int morphfil, double xpos, double ypos, double thetay, double thetaz, int flip)
...

{
  ...
  if (ctype==xrod) { 		/* procedure to make rod */
	...
	p->maxcond=rod_maxcond; p->photnoise=pnoise; p->darknoise=dnoise; p->linit=1;
	if (!notinit(rod_timec)) p->timec1=rod_timec; 
	if (!notinit(rod_loopg)) p->loopg=rod_loopg;
	if (!notinit(rod_stimchan)) p->stimchan=rod_stimchan;
    ...
    if (getn(ct,MORPH) == 1) {		/* single comp cone, cplam = 1 */
      cplam = 1;
      np = loc(nd(xcone,cn,soma), xpos,ypos,getn(xcone,SOMAZ));
      make_celseg (xcone, cn, soma, soma, cd1=getn(ctype,SOMADIA),cd2=0,SOMA,cplam);
      p = make_cone(np,dia=2.5); p->xpos=xpos; p->ypos=ypos;
	p->pigm = cone_pigm; p->pathl = 7.5; p->attf = 0.9;
	...
	if (!notinit(cone_stimchan)) p->stimchan=cone_stimchan;
    ...
    else if (getn(ct,MORPH) == 2) {		/* single comp cone, cplam = 1, transducer */
      cplam = 1;
      np = loc(nd(xcone,cn,soma), xpos,ypos,getn(xcone,SOMAZ));
      make_celseg (xcone, cn, soma, soma, cd1=getn(ctype,SOMADIA),cd2=0,SOMA,cplam);
    ...
    else if (getn(ct,MORPH) == 3) {		/* single comp cone, cplam = 1, itransducer */
      cplam = 1;
      np = loc(nd(xcone,cn,soma), xpos,ypos,getn(xcone,SOMAZ));
      make_celseg (xcone, cn, soma, soma, cd1=getn(ctype,SOMADIA),cd2=0,SOMA,cplam);
	...
	if (!notinit(cone_loopg)) p->loopg=cone_loopg;
        if (!notinit(cone_stimchan)) p->stimchan=cone_stimchan;
      np = loc(nd(xcone,cn,axtrm), xpos,ypos,getn(xcone,AXARBZ)+getn(xcone,SOMAZ));
      make_celseg (xcone, cn, soma, axtrm, cd1=0.3,cd2=0.4,AXON,cplam);
      make_celseg (xcone, cn, axtrm, axtrm, cd1=2,cd2=0,AXON_DIST,cplam);
      celnode[ctype][cn] = axtrm;
    }
  }
    ...
    mak_sbac(ctype,cn,morphfil,aden,xpos,ypos,thetax,thetay,thetaz,flip,int(getn(ctype,MORPH)));
  }
...
#define RECPNT 10000
  ...
  //make_conn (nd(ct,soma),nd(ct,recpnt3),VBUF); 
  //cpnt = (capac*)at(nd(ct,recpnt3),GNDCAP); cpnt->c = elec_capac1, cpnt->vrest=vstart;
  rpnt = (resistor*)make_conn (nd(ct,soma),nd(ct,recpnt),RESISTOR); rpnt->r = elec_resist1;
  cpnt = (capac*)at(nd(ct,recpnt),GNDCAP); cpnt->c = elec_capac1, cpnt->vrest=vstart;
  //dpnt = make_conn (nd(gc,recpnt3),nd(gc,recpnt),DIODE); rpnt->r = elec_resist1;

  rpnt = (resistor*)make_conn(nd(ct,recpnt),nd(ct,recpnt2),RESISTOR); rpnt->r = elec_resist2;
