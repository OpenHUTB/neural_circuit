...

{
   ...
   static TEXT labl[1000], str[100], scomnd[80],  comnd[80], comch[10];
   int  ch,dashnum,flsize,fill,aspect;
   char delim,filnam[20];
   FILE *tempin;

 iflag = 0;
 istat = (long int) signal (2, (sighandler_t)1);	/* save original status */
 if (istat != 1)
   signal (2,(sighandler_t)onintr);		/* go to "onintr" at ^C */
 ...
 while ((ch = fscanf (instream,"%s",comnd)) > 0)
    ...
    *labl = 0; *str = 0;
    ...
    case '!':	ctext();		/* change back to text screen */
	 	fflush(stdout);
	  	fgets(scomnd,80,stdin);		/* run system command */
		sprintf (str,"csh -c '%s'>/dev/tty",scomnd);
